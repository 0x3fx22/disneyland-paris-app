...
public class MyersDiff<T> implements DiffAlgorithm<T> {
    ...
    public PathNode buildPath(List<T> list, List<T> list2) throws DifferentiationFailedException {
        ...
        while (i5 < i2) {
            ...
            while (i7 <= i5) {
                ...
                int i9 = i8 + 1;
                int i10 = i8 - 1;
                if (i7 == i6 || (i7 != i5 && pathNodeArr[i10].f3827i < pathNodeArr[i9].f3827i)) {
    ...
    public Patch<T> buildRevision(PathNode pathNode, List<T> list, List<T> list2) {
        ...
        while (pathNode != null) {
            ...
            if (pathNode2 == null || pathNode2.f3828j < 0) {
                break;
            }
            if (pathNode.isSnake()) {
                throw new IllegalStateException("bad diffpath: found snake when looking for diff");
            }
            int i = pathNode.f3827i;
            int i2 = pathNode.f3828j;
            pathNode = pathNode.prev;
            int i3 = pathNode.f3827i;
            int i4 = pathNode.f3828j;
