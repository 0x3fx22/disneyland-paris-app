...
public class TableDiffer {
    ...
    public DataTableDiff calculateUnorderedDiffs() {
        ...
        arrayList2.addAll(this.f3799to.cells());
        for (List<String> list : this.from.cells()) {
            if (!this.f3799to.cells().contains(list)) {
    ...
    private Map createDeltasByLine() {
        List<Delta> deltas = DiffUtils.diff(getDiffableRows(this.from), getDiffableRows(this.f3799to)).getDeltas();
