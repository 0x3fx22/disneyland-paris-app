...
public final class CompletableConcatIterable extends Completable {
    ...
    public void subscribeActual(CompletableObserver completableObserver) {
        try {
            ...
            completableObserver.onSubscribe(concatInnerObserver.f3844sd);
    ...
    static final class ConcatInnerObserver extends AtomicInteger implements CompletableObserver {
        ...
        final SequentialDisposable f3844sd = new SequentialDisposable();
        ...
        public void onSubscribe(Disposable disposable) {
            this.f3844sd.replace(disposable);
        }
        ...
        void next() {
            if (!this.f3844sd.isDisposed() && getAndIncrement() == 0) {
                Iterator it = this.sources;
                while (!this.f3844sd.isDisposed()) {
                    try {
                        if (!it.hasNext()) {
