...
public class MediaController implements Player {
    ...
    MediaController(Context context, SessionToken sessionToken, Bundle bundle, Listener listener, Looper looper, ConnectionCallback connectionCallback, androidx.media3.common.util.BitmapLoader bitmapLoader) {
        ...
        Log.m228i("MediaController", "Init " + Integer.toHexString(System.identityHashCode(this)) + " [" + MediaLibraryInfo.VERSION_SLASHY + "] [" + Util.DEVICE_DEBUG_INFO + "]");
        this.window = new Timeline.Window();
        this.timeDiffMs = C0740C.TIME_UNSET;
    ...
    public final void stop() {
        ...
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring stop().");
        } else {
            this.impl.stop();
        }
    }
    ...
    public final void release() {
        ...
        Log.m228i("MediaController", "Release " + Integer.toHexString(System.identityHashCode(this)) + " [" + MediaLibraryInfo.VERSION_SLASHY + "] [" + Util.DEVICE_DEBUG_INFO + "] [" + MediaLibraryInfo.registeredModules() + "]");
        this.released = true;
        this.applicationHandler.removeCallbacksAndMessages(null);
        try {
            this.impl.release();
        } catch (Exception e) {
            Log.m225d("MediaController", "Exception while releasing impl", e);
        }
    ...
    public static void releaseFuture(Future<? extends MediaController> future) {
        ...
        try {
            ...
            Log.m231w("MediaController", "MediaController future failed (so we couldn't release it)", e);
        }
    }
    ...
    public final void play() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring play().");
        } else {
            this.impl.play();
        }
    }
    ...
    public final void pause() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring pause().");
        } else {
            this.impl.pause();
        }
    }
    ...
    public final void prepare() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring prepare().");
        } else {
            this.impl.prepare();
        }
    }
    ...
    public final void seekToDefaultPosition() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring seekTo().");
        } else {
    ...
    public final void seekBack() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring seekBack().");
        } else {
            this.impl.seekBack();
        }
    }
    ...
    public final void seekForward() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring seekForward().");
        } else {
    ...
    public final long getDuration() {
        verifyApplicationThread();
        return isConnected() ? this.impl.getDuration() : C0740C.TIME_UNSET;
    }
    ...
    @Override // androidx.media3.common.Player
    @IntRange(from = 0, m4to = WebToNativeBridgeBase.CLOSE_WAIT_DURATION_MILLISECONDS)
    ...
    public final long getCurrentLiveOffset() {
        verifyApplicationThread();
        return isConnected() ? this.impl.getCurrentLiveOffset() : C0740C.TIME_UNSET;
    }

    @Override // androidx.media3.common.Player
    public final long getContentDuration() {
        verifyApplicationThread();
        return isConnected() ? this.impl.getContentDuration() : C0740C.TIME_UNSET;
    }
    ...
    public final void setPlaybackParameters(PlaybackParameters playbackParameters) {
        ...
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setPlaybackParameters().");
        } else {
    ...
    public final void setPlaybackSpeed(float f) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setPlaybackSpeed().");
        } else {
    ...
    public final ImmutableList<CommandButton> getCustomLayout() {
        verifyApplicationThread();
        return isConnected() ? this.impl.getCustomLayout() : ImmutableList.m1516of();
    }
    ...
    public final void setMediaItem(MediaItem mediaItem) {
        ...
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setMediaItem().");
        } else {
    ...
    public final void setMediaItem(MediaItem mediaItem, boolean z) {
        ...
        Assertions.checkNotNull(mediaItem, "mediaItems must not be null");
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setMediaItems().");
        } else {
    ...
    public final void setPlaylistMetadata(MediaMetadata mediaMetadata) {
        ...
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setPlaylistMetadata().");
        } else {
    ...
    public final void addMediaItem(MediaItem mediaItem) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring addMediaItem().");
        } else {
    ...
    public final void addMediaItems(List<MediaItem> list) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring addMediaItems().");
        } else {
    ...
    public final void removeMediaItem(int i) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring removeMediaItem().");
        } else {
    ...
    public final void removeMediaItems(int i, int i2) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring removeMediaItems().");
        } else {
    ...
    public final void clearMediaItems() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring clearMediaItems().");
        } else {
    ...
    public final void moveMediaItem(int i, int i2) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring moveMediaItem().");
        } else {
    ...
    public final void moveMediaItems(int i, int i2, int i3) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring moveMediaItems().");
        } else {
    ...
    public final void replaceMediaItem(int i, MediaItem mediaItem) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring replaceMediaItem().");
        } else {
    ...
    public final void replaceMediaItems(int i, int i2, List<MediaItem> list) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring replaceMediaItems().");
        } else {
    ...
    public final void seekToPreviousMediaItem() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring seekToPreviousMediaItem().");
        } else {
    ...
    public final void seekToNextMediaItem() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring seekToNextMediaItem().");
        } else {
    ...
    public final void seekToPrevious() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring seekToPrevious().");
        } else {
    ...
    public final void seekToNext() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring seekToNext().");
        } else {
    ...
    public final void setRepeatMode(int i) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setRepeatMode().");
        } else {
    ...
    public final void setShuffleModeEnabled(boolean z) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setShuffleMode().");
        } else {
    ...
    public final void clearVideoSurface() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring clearVideoSurface().");
        } else {
    ...
    public final void setVideoSurface(@Nullable Surface surface) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setVideoSurface().");
        } else {
    ...
    public final void setVideoSurfaceHolder(@Nullable SurfaceHolder surfaceHolder) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setVideoSurfaceHolder().");
        } else {
    ...
    public final void clearVideoSurfaceHolder(@Nullable SurfaceHolder surfaceHolder) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring clearVideoSurfaceHolder().");
        } else {
    ...
    public final void setVideoSurfaceView(@Nullable SurfaceView surfaceView) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setVideoSurfaceView().");
        } else {
    ...
    public final void clearVideoSurfaceView(@Nullable SurfaceView surfaceView) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring clearVideoSurfaceView().");
        } else {
    ...
    public final void setVideoTextureView(@Nullable TextureView textureView) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setVideoTextureView().");
        } else {
    ...
    public final void clearVideoTextureView(@Nullable TextureView textureView) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring clearVideoTextureView().");
        } else {
    ...
    @Override // androidx.media3.common.Player
    @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 1.0d)
    ...
    @Override // androidx.media3.common.Player
    public final void setVolume(@FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 1.0d) float f) {
        verifyApplicationThread();
        Assertions.checkArgument(f >= BitmapDescriptorFactory.HUE_RED && f <= 1.0f, "volume must be between 0 and 1");
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setVolume().");
        } else {
    ...
    public final void setDeviceVolume(@IntRange(from = 0) int i) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setDeviceVolume().");
        } else {
    ...
    public final void increaseDeviceVolume() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring increaseDeviceVolume().");
        } else {
    ...
    public final void decreaseDeviceVolume() {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring decreaseDeviceVolume().");
        } else {
    ...
    public final void setDeviceMuted(boolean z) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setDeviceMuted().");
        } else {
    ...
    public final void setAudioAttributes(AudioAttributes audioAttributes, boolean z) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setAudioAttributes().");
        } else {
    ...
    public final void setTrackSelectionParameters(TrackSelectionParameters trackSelectionParameters) {
        verifyApplicationThread();
        if (!isConnected()) {
            Log.m230w("MediaController", "The controller is not connected. Ignoring setTrackSelectionParameters().");
        }
