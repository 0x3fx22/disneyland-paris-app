...
abstract class MediaUtils {
    ...
    public static int calculateBufferedPercentage(long j, long j2) {
        if (j == C0740C.TIME_UNSET || j2 == C0740C.TIME_UNSET) {
            return 0;
        }
        if (j2 == 0) {
            return 100;
    ...
    public static long getUpdatedCurrentPositionMs(PlayerInfo playerInfo, long j, long j2, long j3) {
        ...
        if (!playerInfo.isPlaying) {
            return (z || j == C0740C.TIME_UNSET) ? playerInfo.sessionPositionInfo.positionInfo.positionMs : j;
        }
        if (!z && j != C0740C.TIME_UNSET) {
            return j;
        }
        ...
        return j5 != C0740C.TIME_UNSET ? Math.min(j4, j5) : j4;
    }
}
