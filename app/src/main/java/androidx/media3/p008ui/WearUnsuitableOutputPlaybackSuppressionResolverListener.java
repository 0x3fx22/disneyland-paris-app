...
public final class WearUnsuitableOutputPlaybackSuppressionResolverListener implements Player.Listener {
    ...
    WearUnsuitableOutputPlaybackSuppressionResolverListener(Context context, long j, Clock clock) {
        ...
        this.clock = clock;
        this.unsuitableOutputPlaybackSuppressionStartRealtimeMs = C0740C.TIME_UNSET;
    }
    ...
    public void onEvents(Player player, Player.Events events) {
        if (Util.isWear(this.applicationContext)) {
            ...
            if (!events.contains(6) || player.getPlaybackSuppressionReason() != 0 || this.unsuitableOutputPlaybackSuppressionStartRealtimeMs == C0740C.TIME_UNSET || this.clock.elapsedRealtime() - this.unsuitableOutputPlaybackSuppressionStartRealtimeMs >= this.autoResumeTimeoutAfterUnsuitableOutputSuppressionMs) {
                return;
            }
            this.unsuitableOutputPlaybackSuppressionStartRealtimeMs = C0740C.TIME_UNSET;
            player.play();
