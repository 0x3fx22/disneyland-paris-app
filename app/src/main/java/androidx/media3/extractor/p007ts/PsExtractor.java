...
public final class PsExtractor implements Extractor {
    ...
    public void seek(long j, long j2) {
        boolean z = this.timestampAdjuster.getTimestampOffsetUs() == C0740C.TIME_UNSET;
        if (!z) {
            long firstSampleTimestampUs = this.timestampAdjuster.getFirstSampleTimestampUs();
            z = (firstSampleTimestampUs == C0740C.TIME_UNSET || firstSampleTimestampUs == 0 || firstSampleTimestampUs == j2) ? false : true;
        }
        if (z) {
    ...
    private void maybeOutputSeekMap(long j) {
        ...
        if (this.durationReader.getDurationUs() != C0740C.TIME_UNSET) {
