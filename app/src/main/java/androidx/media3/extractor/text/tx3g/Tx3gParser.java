...
public final class Tx3gParser implements SubtitleParser {
    ...
    public Tx3gParser(List<byte[]> list) {
        int size = list.size();
        String str = C0740C.SANS_SERIF_NAME;
        if (size == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            byte[] bArr = list.get(0);
            this.defaultFontFace = bArr[24];
            this.defaultColorRgba = ((bArr[26] & 255) << 24) | ((bArr[27] & 255) << 16) | ((bArr[28] & 255) << 8) | (bArr[29] & 255);
            this.defaultFontFamily = "Serif".equals(Util.fromUtf8Bytes(bArr, 43, bArr.length - 43)) ? C0740C.SERIF_NAME : str;
        ...
        this.defaultFontFamily = C0740C.SANS_SERIF_NAME;
    ...
    public void parse(byte[] bArr, int i, int i2, SubtitleParser.OutputOptions outputOptions, Consumer<CuesWithTiming> consumer) {
        ...
        if (subtitleText.isEmpty()) {
            consumer.accept(new CuesWithTiming(ImmutableList.m1516of(), C0740C.TIME_UNSET, C0740C.TIME_UNSET));
            return;
        }
        ...
        consumer.accept(new CuesWithTiming(ImmutableList.m1517of(new Cue.Builder().setText(spannableStringBuilder).setLine(fConstrainValue, 0).setLineAnchor(0).build()), C0740C.TIME_UNSET, C0740C.TIME_UNSET));
    }
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) {
        ...
        if (unsignedShort2 > spannableStringBuilder.length()) {
            Log.m230w("Tx3gParser", "Truncating styl end (" + unsignedShort2 + ") to cueText.length() (" + spannableStringBuilder.length() + ").");
            unsignedShort2 = spannableStringBuilder.length();
        }
        if (unsignedShort >= unsignedShort2) {
            Log.m230w("Tx3gParser", "Ignoring styl with start (" + unsignedShort + ") >= end (" + unsignedShort2 + ").");
            return;
        }
