...
public final class CuesWithTimingSubtitle implements Subtitle {
    ...
    public CuesWithTimingSubtitle(List<CuesWithTiming> list) {
        if (list.size() == 1) {
            ...
            if (cuesWithTiming.durationUs == C0740C.TIME_UNSET) {
                this.eventCues = ImmutableList.m1517of(cuesWithTiming.cues);
                this.eventTimesUs = new long[]{jNormalizeUnsetStartTimeToZero};
                return;
            } else {
                this.eventCues = ImmutableList.m1518of((ImmutableList) cuesWithTiming.cues, ImmutableList.m1516of());
        ...
        for (int i2 = 0; i2 < immutableListSortedCopyOf.size(); i2++) {
            ...
            if (i != 0) {
                ...
                if (j2 < jNormalizeUnsetStartTimeToZero2) {
                    ...
                    Log.m230w("CuesWithTimingSubtitle", "Truncating unsupported overlapping cues.");
            ...
            if (cuesWithTiming2.durationUs != C0740C.TIME_UNSET) {
                this.eventTimesUs[i] = j;
                arrayList.add(ImmutableList.m1516of());
                i++;
            }
        }
    ...
    public ImmutableList<Cue> getCues(long j) {
        ...
        return iBinarySearchFloor == -1 ? ImmutableList.m1516of() : (ImmutableList) this.eventCues.get(iBinarySearchFloor);
    }
}
