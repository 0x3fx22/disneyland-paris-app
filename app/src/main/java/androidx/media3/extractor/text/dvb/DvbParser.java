...
public final class DvbParser implements SubtitleParser {
    ...
    private static final byte[] defaultMap2To4 = {0, 7, 8, Ascii.f3531SI};
    ...
    private CuesWithTiming parse(ParsableBitArray parsableBitArray) {
        ...
        if (pageComposition == null) {
            return new CuesWithTiming(ImmutableList.m1516of(), C0740C.TIME_UNSET, C0740C.TIME_UNSET);
        }
        ...
        for (int i2 = 0; i2 < sparseArray.size(); i2++) {
            ...
            int i5 = 0;
            for (SparseArray sparseArray2 = regionComposition.regionObjects; i5 < sparseArray2.size(); sparseArray2 = sparseArray2) {
        ...
        return new CuesWithTiming(arrayList, C0740C.TIME_UNSET, C0740C.TIME_UNSET);
    }
    ...
    private static void parseSubtitlingSegment(ParsableBitArray parsableBitArray, SubtitleService subtitleService) {
        ...
        if (bits3 * 8 > parsableBitArray.bitsLeft()) {
            Log.m230w("DvbParser", "Data field length exceeds limit");
        ...
        switch (bits) {
            ...
            case 17:
                ...
                if (bits2 == subtitleService.subtitlePageId && pageComposition3 != null) {
                    ...
                    if (pageComposition3.state == 0 && (regionComposition = (RegionComposition) subtitleService.regions.get(regionComposition2.f139id)) != null) {
                        regionComposition2.mergeFrom(regionComposition);
                    }
                    subtitleService.regions.put(regionComposition2.f139id, regionComposition2);
                }
                break;
            case 18:
                if (bits2 == subtitleService.subtitlePageId) {
                    ClutDefinition clutDefinition = parseClutDefinition(parsableBitArray, bits3);
                    subtitleService.cluts.put(clutDefinition.f137id, clutDefinition);
                } else if (bits2 == subtitleService.ancillaryPageId) {
                    ClutDefinition clutDefinition2 = parseClutDefinition(parsableBitArray, bits3);
                    subtitleService.ancillaryCluts.put(clutDefinition2.f137id, clutDefinition2);
                }
                break;
            case 19:
                if (bits2 == subtitleService.subtitlePageId) {
                    ObjectData objectData = parseObjectData(parsableBitArray);
                    subtitleService.objects.put(objectData.f138id, objectData);
                } else if (bits2 == subtitleService.ancillaryPageId) {
                    ObjectData objectData2 = parseObjectData(parsableBitArray);
                    subtitleService.ancillaryObjects.put(objectData2.f138id, objectData2);
                }
                break;
            case 20:
                if (bits2 == subtitleService.subtitlePageId) {
