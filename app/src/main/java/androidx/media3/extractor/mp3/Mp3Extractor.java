...
public final class Mp3Extractor implements Extractor {
    ...
    public Mp3Extractor(int i, long j) {
        ...
        this.basisTimeUs = C0740C.TIME_UNSET;
    ...
    private int readSample(ExtractorInput extractorInput) throws IOException {
        if (this.sampleBytesRemaining == 0) {
            ...
            if (this.basisTimeUs == C0740C.TIME_UNSET) {
                this.basisTimeUs = this.seeker.getTimeUs(extractorInput.getPosition());
                if (this.forcedFirstSampleTimestampUs != C0740C.TIME_UNSET) {
    ...
    private Seeker maybeReadSeekFrame(ExtractorInput extractorInput) throws IOException {
        ...
        if (extractorInput.getLength() != -1 && xingFrame.dataSize != -1 && extractorInput.getLength() != xingFrame.dataSize + position) {
            Log.m228i("Mp3Extractor", "Data size mismatch between stream (" + extractorInput.getLength() + ") and Xing frame (" + (xingFrame.dataSize + position) + "), using Xing value.");
        }
        extractorInput.skipFully(this.synchronizedHeader.frameSize);
        if (seekFrameHeader == 1483304551) {
    ...
    private Seeker getConstantBitrateSeeker(long j, XingFrame xingFrame, long j2) {
        ...
        if (j5 != -1) {
            long j6 = j + j5;
            j3 = j5 - ((long) xingFrame.header.frameSize);
            j4 = j6;
        } else {
            if (j2 == -1) {
                return null;
        ...
        return new ConstantBitrateSeeker(j4, j + ((long) xingFrame.header.frameSize), Ints.checkedCast(Util.scaleLargeValue(j7, 8000000L, jComputeDurationUs, roundingMode)), Ints.checkedCast(LongMath.divide(j7, xingFrame.frameCount, roundingMode)), false);
    }
    ...
    private static long getId3TlenUs(Metadata metadata) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (entry instanceof TextInformationFrame) {
                ...
                if (textInformationFrame.f131id.equals("TLEN")) {
