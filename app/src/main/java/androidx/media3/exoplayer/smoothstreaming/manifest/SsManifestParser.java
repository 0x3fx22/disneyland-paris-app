...
public class SsManifestParser implements ParsingLoadable.Parser<SsManifest> {
    ...
    private static class StreamIndexParser extends ElementParser {
        ...
        private void parseStreamFragmentStartTag(XmlPullParser xmlPullParser) throws ParserException {
            ...
            long jLongValue = parseLong(xmlPullParser, "t", C0740C.TIME_UNSET);
            int i = 1;
            if (jLongValue == C0740C.TIME_UNSET) {
                if (size == 0) {
                    jLongValue = 0;
            ...
            this.lastChunkDuration = parseLong(xmlPullParser, "d", C0740C.TIME_UNSET);
            long j = parseLong(xmlPullParser, "r", 1L);
            if (j > 1 && this.lastChunkDuration == C0740C.TIME_UNSET) {
    ...
    private static class QualityLevelParser extends ElementParser {
        ...
        public void parseStartTag(XmlPullParser xmlPullParser) throws ParserException {
            ...
            if (iIntValue == 2) {
                builder.setContainerMimeType(MimeTypes.VIDEO_MP4).setWidth(parseRequiredInt(xmlPullParser, "MaxWidth")).setHeight(parseRequiredInt(xmlPullParser, "MaxHeight")).setInitializationData(buildCodecSpecificData(xmlPullParser.getAttributeValue(null, "CodecPrivateData")));
            } else if (iIntValue == 1) {
