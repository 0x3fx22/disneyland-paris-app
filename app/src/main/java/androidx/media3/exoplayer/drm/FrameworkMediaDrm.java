...
public final class FrameworkMediaDrm implements ExoMediaDrm {
    ...
    public static /* synthetic */ ExoMediaDrm lambda$static$0(UUID uuid) {
        try {
            ...
            Log.m226e("FrameworkMediaDrm", "Failed to instantiate a FrameworkMediaDrm for uuid: " + uuid + InstructionFileId.DOT);
    ...
    private FrameworkMediaDrm(UUID uuid) {
        ...
        if (C0740C.WIDEVINE_UUID.equals(uuid) && needsForceWidevineL3Workaround()) {
    ...
    public void setPlayerIdForSession(byte[] bArr, PlayerId playerId) {
        if (Util.SDK_INT >= 31) {
            try {
                ...
                Log.m230w("FrameworkMediaDrm", "setLogSessionId failed.");
            }
        }
    }
    ...
    public byte[] provideKeyResponse(byte[] bArr, byte[] bArr2) throws DeniedByServerException, NotProvisionedException {
        if (C0740C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    private boolean shouldForceAllowInsecureDecoderComponents() {
        return Util.SDK_INT < 21 && C0740C.WIDEVINE_UUID.equals(this.uuid) && "L3".equals(getPropertyString("securityLevel"));
    }

    private boolean isMediaDrmRequiresSecureDecoderImplemented() {
        if (this.uuid.equals(C0740C.WIDEVINE_UUID)) {
            String propertyString = getPropertyString("version");
        ...
        return this.uuid.equals(C0740C.CLEARKEY_UUID);
    }

    private static DrmInitData.SchemeData getSchemeData(UUID uuid, List list) {
        if (!C0740C.WIDEVINE_UUID.equals(uuid)) {
    ...
    private static UUID adjustUuid(UUID uuid) {
        return (Util.SDK_INT >= 27 || !C0740C.CLEARKEY_UUID.equals(uuid)) ? uuid : C0740C.COMMON_PSSH_UUID;
    }
    ...
    private static byte[] adjustRequestInitData(UUID uuid, byte[] bArr) {
        ...
        UUID uuid2 = C0740C.PLAYREADY_UUID;
        ...
        if (Util.SDK_INT >= 23 || !C0740C.WIDEVINE_UUID.equals(uuid)) {
    ...
    private static String adjustRequestMimeType(UUID uuid, String str) {
        return (Util.SDK_INT < 26 && C0740C.CLEARKEY_UUID.equals(uuid) && (MimeTypes.VIDEO_MP4.equals(str) || MimeTypes.AUDIO_MP4.equals(str))) ? C0740C.CENC_TYPE_cenc : str;
    }

    private static byte[] adjustRequestData(UUID uuid, byte[] bArr) {
        return C0740C.CLEARKEY_UUID.equals(uuid) ? ClearKeyUtil.adjustRequestData(bArr) : bArr;
    }
    ...
    private static byte[] addLaUrlAttributeIfMissing(byte[] bArr) {
        ...
        if (littleEndianShort != 1 || littleEndianShort2 != 1) {
            Log.m228i("FrameworkMediaDrm", "Unexpected record count or type. Skipping LA_URL workaround.");
            return bArr;
        }
        ...
        if (iIndexOf == -1) {
            Log.m230w("FrameworkMediaDrm", "Could not find the </DATA> tag. Skipping LA_URL workaround.");
        }
    ...
    private static class Api31 {
        ...
        public static void setLogSessionIdOnMediaDrmSession(MediaDrm mediaDrm, byte[] bArr, PlayerId playerId) {
            ...
            FrameworkMediaDrm$Api31$$ExternalSyntheticApiModelOutline1.m331m(Assertions.checkNotNull(mediaDrm.getPlaybackComponent(bArr))).setLogSessionId(logSessionId);
        }
    }
}
