...
public class DefaultDrmSessionManager implements DrmSessionManager {
    ...
    public static final class Builder {
        ...
        private UUID uuid = C0740C.WIDEVINE_UUID;
        ...
        public Builder setSessionKeepaliveMs(long j) {
            Assertions.checkArgument(j > 0 || j == C0740C.TIME_UNSET);
    ...
    private DefaultDrmSessionManager(UUID uuid, ExoMediaDrm.Provider provider, MediaDrmCallback mediaDrmCallback, HashMap map, boolean z, int[] iArr, boolean z2, LoadErrorHandlingPolicy loadErrorHandlingPolicy, long j) {
        ...
        Assertions.checkArgument(!C0740C.COMMON_PSSH_UUID.equals(uuid), "Use C.CLEARKEY_UUID instead");
        this.uuid = uuid;
    ...
    public final void prepare() {
        ...
        } else if (this.sessionKeepaliveMs != C0740C.TIME_UNSET) {
    ...
    public final void release() {
        ...
        if (this.sessionKeepaliveMs != C0740C.TIME_UNSET) {
    ...
    public DrmSession acquireSession(Looper looper, DrmSessionEventListener.EventDispatcher eventDispatcher, Format format, boolean z) {
        ...
        if (this.offlineLicenseKeySetId == null) {
            ...
            if (schemeDatas.isEmpty()) {
                ...
                Log.m227e("DefaultDrmSessionMgr", "DRM error", missingSchemeDataException);
    ...
    private DrmSession maybeAcquirePlaceholderSession(int i, boolean z) {
        ...
        if (defaultDrmSession == null) {
            DefaultDrmSession defaultDrmSessionCreateAndAcquireSessionWithRetry = createAndAcquireSessionWithRetry(ImmutableList.m1516of(), true, null, z);
            this.sessions.add(defaultDrmSessionCreateAndAcquireSessionWithRetry);
    ...
    private boolean canAcquireSession(DrmInitData drmInitData) {
        ...
        if (getSchemeDatas(drmInitData, this.uuid, true).isEmpty()) {
            if (drmInitData.schemeDataCount != 1 || !drmInitData.get(0).matches(C0740C.COMMON_PSSH_UUID)) {
                return false;
            }
            Log.m230w("DefaultDrmSessionMgr", "DrmInitData only contains common PSSH SchemeData. Assuming support for: " + this.uuid);
        }
        String str = drmInitData.schemeType;
        if (str == null || C0740C.CENC_TYPE_cenc.equals(str)) {
            return true;
        }
        if (C0740C.CENC_TYPE_cbcs.equals(str)) {
            return Util.SDK_INT >= 25;
        }
        return (C0740C.CENC_TYPE_cbc1.equals(str) || C0740C.CENC_TYPE_cens.equals(str)) ? false : true;
    }
    ...
    private void verifyPlaybackThread(boolean z) {
        if (z && this.playbackLooper == null) {
            Log.m231w("DefaultDrmSessionMgr", "DefaultDrmSessionManager accessed before setPlayer(), possibly on the wrong thread.", new IllegalStateException());
            return;
        }
        if (Thread.currentThread() != ((Looper) Assertions.checkNotNull(this.playbackLooper)).getThread()) {
            Log.m231w("DefaultDrmSessionMgr", "DefaultDrmSessionManager accessed on the wrong thread.\nCurrent thread: " + Thread.currentThread().getName() + "\nExpected thread: " + this.playbackLooper.getThread().getName(), new IllegalStateException());
        }
    }
    ...
    private static List getSchemeDatas(DrmInitData drmInitData, UUID uuid, boolean z) {
        ...
        for (int i = 0; i < drmInitData.schemeDataCount; i++) {
            ...
            if ((schemeData.matches(uuid) || (C0740C.CLEARKEY_UUID.equals(uuid) && schemeData.matches(C0740C.COMMON_PSSH_UUID))) && (schemeData.data != null || z)) {
    ...
    class ReferenceCountListenerImpl implements DefaultDrmSession.ReferenceCountListener {
        ...
        public void onReferenceCountIncremented(DefaultDrmSession defaultDrmSession, int i) {
            if (DefaultDrmSessionManager.this.sessionKeepaliveMs != C0740C.TIME_UNSET) {
        ...
        public void onReferenceCountDecremented(final DefaultDrmSession defaultDrmSession, int i) {
            if (i == 1 && DefaultDrmSessionManager.this.prepareCallsCount > 0 && DefaultDrmSessionManager.this.sessionKeepaliveMs != C0740C.TIME_UNSET) {
