...
public final class DownloadManager {
    ...
    static final class InternalHandler extends Handler {
        ...
        private void initialize(int i) {
            ...
            try {
                try {
                    ...
                    Log.m227e("DownloadManager", "Failed to load index.", e);
        ...
        private void setStopReason(String str, int i) {
            if (str == null) {
                ...
                try {
                    ...
                    Log.m227e("DownloadManager", "Failed to set manual stop reason", e);
                }
            } else {
                ...
                if (download != null) {
                    ...
                    try {
                        ...
                        Log.m227e("DownloadManager", "Failed to set manual stop reason: " + str, e2);
                    }
                }
            }
            syncTasks();
        ...
        private void addDownload(DownloadRequest downloadRequest, int i) {
            Download download = getDownload(downloadRequest.f117id, true);
        ...
        private void removeDownload(String str) {
            ...
            if (download == null) {
                Log.m226e("DownloadManager", "Failed to remove nonexistent download: " + str);
                return;
            }
        ...
        private void removeAllDownloads() {
            ...
            try {
                ...
                Log.m226e("DownloadManager", "Failed to load downloads.");
            }
            ...
            try {
                ...
                Log.m227e("DownloadManager", "Failed to update index.", e);
            }
        ...
        private void syncTasks() {
            ...
            for (int i2 = 0; i2 < this.downloads.size(); i2++) {
                ...
                Task taskSyncQueuedDownload = (Task) this.activeTasks.get(download.request.f117id);
                int i3 = download.state;
                if (i3 == 0) {
        ...
        private Task syncQueuedDownload(Task task, Download download) {
            if (task == null) {
                ...
                this.activeTasks.put(downloadPutDownloadWithState.request.f117id, task2);
        ...
        private void syncRemovingDownload(Task task, Download download) {
            if (task == null) {
                ...
                this.activeTasks.put(download.request.f117id, task2);
        ...
        private void onContentLengthChanged(Task task, long j) {
            Download download = (Download) Assertions.checkNotNull(getDownload(task.request.f117id, false));
        ...
        private void onTaskStopped(Task task) {
            String str = task.request.f117id;
            ...
            if (exc != null) {
                Log.m227e("DownloadManager", "Task failed: " + task.request + ", " + z, exc);
            }
        ...
        private void onDownloadTaskStopped(Download download, Exception exc) {
            ...
            this.downloads.remove(getDownloadIndex(download2.request.f117id));
            try {
        ...
        private void onRemoveTaskStopped(Download download) {
            if (download.state == 7) {
                ...
                this.downloads.remove(getDownloadIndex(download.request.f117id));
                try {
                    this.downloadIndex.removeDownload(download.request.f117id);
                } catch (IOException unused) {
                    Log.m226e("DownloadManager", "Failed to remove from database");
                }
        ...
        private Download putDownload(Download download) {
            ...
            Assertions.checkState((i == 3 || i == 4) ? false : true);
            int downloadIndex = getDownloadIndex(download.request.f117id);
            if (downloadIndex == -1) {
        ...
        private Download getDownload(String str, boolean z) {
            ...
            try {
                ...
                Log.m227e("DownloadManager", "Failed to load download: " + str, e);
                return null;
            }
        }
        ...
        private int getDownloadIndex(String str) {
            for (int i = 0; i < this.downloads.size(); i++) {
                if (((Download) this.downloads.get(i)).request.f117id.equals(str)) {
                    return i;
                }
            }
            return -1;
