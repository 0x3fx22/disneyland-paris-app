...
final class HlsSampleStreamWrapper implements Loader.Callback, Loader.ReleaseCallback, SequenceableLoader, ExtractorOutput, SampleQueue.UpstreamFormatChangedListener {
        ...
        if (this.enabledTrackGroupCount == 0) {
            ...
            if (this.mediaChunks.isEmpty() || Util.areEqual(exoTrackSelection, trackSelection)) {
                ...
                if (!this.seenFirstTrackSelection) {
                    ...
                    exoTrackSelection.updateSelectedTrack(j, j2, C0740C.TIME_UNSET, this.readOnlyMediaChunks, this.chunkSource.createMediaChunkIterators(lastMediaChunk, j));
    ...
    public boolean onPlaylistError(Uri uri, LoadErrorHandlingPolicy.LoadErrorInfo loadErrorInfo, boolean z) {
        ...
        return this.chunkSource.onPlaylistError(uri, j) && j != C0740C.TIME_UNSET;
    }
    ...
    public boolean continueLoading(LoadingInfo loadingInfo) {
        ...
        if (z) {
            this.pendingResetPositionUs = C0740C.TIME_UNSET;
    ...
    public Loader.LoadErrorAction onLoadError(Chunk chunk, long j, long j2, IOException iOException, int i) {
        ...
        if (zMaybeExcludeTrack) {
            ...
            if (retryDelayMsFor != C0740C.TIME_UNSET) {
    ...
    private TrackGroupArray createTrackGroupArrayWithDrmInfo(TrackGroup[] trackGroupArr) {
        for (int i = 0; i < trackGroupArr.length; i++) {
            ...
            trackGroupArr[i] = new TrackGroup(trackGroup.f107id, formatArr);
        }
    ...
    private boolean isPendingReset() {
        return this.pendingResetPositionUs != C0740C.TIME_UNSET;
    }
    ...
    private static DiscardingTrackOutput createDiscardingTrackOutput(int i, int i2) {
        Log.m230w("HlsSampleStreamWrapper", "Unmapped track with id " + i + " of type " + i2);
            ...
            if (!Util.areEqual(this.format.sampleMimeType, this.delegateFormat.sampleMimeType)) {
                if (MimeTypes.APPLICATION_EMSG.equals(this.format.sampleMimeType)) {
                    ...
                    if (!emsgContainsExpectedWrappedFormat(eventMessageDecode)) {
                        Log.m230w("HlsSampleStreamWrapper", String.format("Ignoring EMSG. Expected it to contain wrapped %s but actual wrapped format: %s", this.delegateFormat.sampleMimeType, eventMessageDecode.getWrappedMetadataFormat()));
                        return;
                    }
                    sampleAndTrimBuffer = new ParsableByteArray((byte[]) Assertions.checkNotNull(eventMessageDecode.getWrappedMetadataBytes()));
                } else {
                    Log.m230w("HlsSampleStreamWrapper", "Ignoring sample for unsupported format: " + this.format.sampleMimeType);
                    return;
                }
            }
