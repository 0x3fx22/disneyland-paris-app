...
class SampleDataQueue {
    ...
    private static AllocationNode readEncryptionData(AllocationNode allocationNode, DecoderInputBuffer decoderInputBuffer, SampleQueue.SampleExtrasHolder sampleExtrasHolder, ParsableByteArray parsableByteArray) {
        ...
        byte[] bArr = cryptoInfo.f110iv;
        if (bArr == null) {
            cryptoInfo.f110iv = new byte[16];
        } else {
            Arrays.fill(bArr, (byte) 0);
        }
        AllocationNode data2 = readData(data, j2, cryptoInfo.f110iv, i);
        long j3 = j2 + ((long) i);
        if (z) {
        ...
        cryptoInfo.set(i2, iArr2, iArr4, cryptoData.encryptionKey, cryptoInfo.f110iv, cryptoData.cryptoMode, cryptoData.encryptedBlocks, cryptoData.clearBlocks);
