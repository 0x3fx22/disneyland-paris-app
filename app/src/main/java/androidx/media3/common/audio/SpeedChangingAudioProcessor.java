...
public final class SpeedChangingAudioProcessor extends BaseAudioProcessor {
    ...
    public SpeedChangingAudioProcessor(SpeedProvider speedProvider) {
        ...
        this.speedAdjustedTimeAsyncInputTimeUs = C0740C.TIME_UNSET;
        resetState();
    }
    ...
    public void queueInput(ByteBuffer byteBuffer) {
        ...
        if (nextSpeedChangeTimeUs != C0740C.TIME_UNSET) {
            ...
            iScaleLargeValue = (int) Util.scaleLargeValue(j2, ((long) audioFormat2.sampleRate) * ((long) audioFormat2.bytesPerFrame), 1000000L, RoundingMode.CEILING);
        ...
        synchronized (this.lock) {
            try {
                if (isUsingSonic()) {
                    ...
                    long j = this.bytesRead;
                    AudioProcessor.AudioFormat audioFormat2 = this.inputAudioFormat;
                    this.lastProcessedInputTimeUs = Util.scaleLargeTimestamp(j, 1000000L, ((long) audioFormat2.sampleRate) * ((long) audioFormat2.bytesPerFrame));
                }
            } catch (Throwable th) {
                throw th;
            }
