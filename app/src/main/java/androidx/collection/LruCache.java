...
public class LruCache<K, V> {
    ...
    public LruCache(@IntRange(from = 1, m4to = Long.MAX_VALUE) int i) {
        this.maxSize = i;
        if (i <= 0) {
    ...
    public void resize(@IntRange(from = 1, m4to = Long.MAX_VALUE) int maxSize) {
        if (maxSize <= 0) {
            throw new IllegalArgumentException("maxSize <= 0");
        }
