...
public class FlatBufferBuilder {
    ...
    public FlatBufferBuilder(int i, ByteBufferFactory byteBufferFactory, ByteBuffer byteBuffer, Utf8 utf8) {
        ...
        if (byteBuffer != null) {
            this.f92bb = byteBuffer;
            byteBuffer.clear();
            this.f92bb.order(ByteOrder.LITTLE_ENDIAN);
        } else {
            this.f92bb = byteBufferFactory.newByteBuffer(i);
        }
        this.utf8 = utf8;
        this.space = this.f92bb.capacity();
    }
    ...
    public int offset() {
        return this.f92bb.capacity() - this.space;
    }

    public void pad(int i) {
        for (int i2 = 0; i2 < i; i2++) {
            ByteBuffer byteBuffer = this.f92bb;
            int i3 = this.space - 1;
            this.space = i3;
    ...
    public ByteBuffer createUnintializedVector(int i, int i2, int i3) {
        ...
        ByteBuffer byteBufferOrder = this.f92bb.slice().order(ByteOrder.LITTLE_ENDIAN);
    ...
    public int createString(CharSequence charSequence) {
        ...
        this.utf8.encodeUtf8(charSequence, this.f92bb);
        return endVector();
    }
    ...
    public int createString(ByteBuffer byteBuffer) {
        ...
        ByteBuffer byteBuffer2 = this.f92bb;
        int i = this.space - iRemaining;
        this.space = i;
        byteBuffer2.position(i);
        this.f92bb.put(byteBuffer);
        return endVector();
    }
    ...
    public int endTable() {
        ...
        loop2: while (true) {
            ...
            int iCapacity = this.f92bb.capacity() - this.vtables[i5];
            int i6 = this.space;
            short s = this.f92bb.getShort(iCapacity);
            if (s == this.f92bb.getShort(i6)) {
                int i7 = 2;
                while (true) {
                    if (i7 < s) {
                        if (this.f92bb.getShort(iCapacity + i7) != this.f92bb.getShort(i6 + i7)) {
                            break;
                        }
                        i7 += 2;
                    } else {
        ...
        if (i != 0) {
            int iCapacity2 = this.f92bb.capacity() - iOffset;
            this.space = iCapacity2;
            this.f92bb.putInt(iCapacity2, i - iOffset);
        } else {
    ...
    public void required(int i, int i2) {
        int iCapacity = this.f92bb.capacity() - i;
        if (this.f92bb.getShort((iCapacity - this.f92bb.getInt(iCapacity)) + i2) != 0) {
            return;
        }
    ...
    protected void finish(int i, boolean z) {
        ...
        addOffset(i);
        if (z) {
            addInt(this.f92bb.capacity() - this.space);
        }
        this.f92bb.position(this.space);
        this.finished = true;
    }
    ...
    public byte[] sizedByteArray(int i, int i2) {
        ...
        this.f92bb.position(i);
        this.f92bb.get(bArr);
        return bArr;
    }

    public byte[] sizedByteArray() {
        return sizedByteArray(this.space, this.f92bb.capacity() - this.space);
    }

    public InputStream sizedInputStream() {
        finished();
        ByteBuffer byteBufferDuplicate = this.f92bb.duplicate();
        byteBufferDuplicate.position(this.space);
        byteBufferDuplicate.limit(this.f92bb.capacity());
