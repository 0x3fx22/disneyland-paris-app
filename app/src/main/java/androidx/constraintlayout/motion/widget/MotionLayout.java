...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    void setState(TransitionState transitionState) {
        ...
        int i = C03915.f69xabc7e4ac[transitionState3.ordinal()];
        if (i != 1 && i != 2) {
            ...
            for (int i3 = 1; i3 < i2 - 1; i3++) {
                if (i != 4 || this.mPathMode[i3 - 1] != 0) {
                    ...
                    if (i == 4) {
                        ...
                        if (i6 == 1) {
                            drawPathRelativeTicks(canvas, f - BitmapDescriptorFactory.HUE_RED, f2 - BitmapDescriptorFactory.HUE_RED);
                        } else if (i6 == 0) {
                            drawPathCartesianTicks(canvas, f - BitmapDescriptorFactory.HUE_RED, f2 - BitmapDescriptorFactory.HUE_RED);
                        } else {
                            if (i6 == 2) {
                                drawPathScreenTicks(canvas, f - BitmapDescriptorFactory.HUE_RED, f2 - BitmapDescriptorFactory.HUE_RED, width, height);
                            }
  ...
  0x00e2: PHI (r3v50 float) = (r3v49 float), (r3v51 float), (r3v51 float) binds: [B:47:0x00ab, B:58:0x00d6, B:60:0x00da] A[DONT_GENERATE, DONT_INLINE]] */
    ...
    void evaluate(boolean z) {
        ...
        if (this.mKeepAnimating || (this.mInTransition && (z || this.mTransitionGoalPosition != f))) {
            ...
            if ((fSignum <= BitmapDescriptorFactory.HUE_RED || f3 < this.mTransitionGoalPosition) && (fSignum > BitmapDescriptorFactory.HUE_RED || f3 > this.mTransitionGoalPosition)) {
                z2 = false;
            } else {
                f3 = this.mTransitionGoalPosition;
                this.mInTransition = false;
            ...
            if (interpolator3 != null && !z2) {
                if (this.mTemporalInterpolator) {
                    ...
                    if (interpolator5 instanceof MotionInterpolator) {
                        ...
                        if (velocity > BitmapDescriptorFactory.HUE_RED && interpolation2 >= 1.0f) {
                            this.mTransitionLastPosition = 1.0f;
                            this.mInTransition = false;
                            interpolation2 = 1.0f;
                        }
                        if (velocity >= BitmapDescriptorFactory.HUE_RED || interpolation2 > BitmapDescriptorFactory.HUE_RED) {
                            f3 = interpolation2;
                        } else {
                            this.mTransitionLastPosition = BitmapDescriptorFactory.HUE_RED;
                            this.mInTransition = false;
                ...
                boolean z5 = (fSignum <= BitmapDescriptorFactory.HUE_RED && f3 >= this.mTransitionGoalPosition) || (fSignum <= BitmapDescriptorFactory.HUE_RED && f3 <= this.mTransitionGoalPosition);
                ...
                if (f3 <= BitmapDescriptorFactory.HUE_RED && (i4 = this.mBeginState) != -1 && this.mCurrentState != i4) {
                    this.mCurrentState = i4;
                ...
                if (!this.mKeepAnimating || this.mInTransition) {
                    invalidate();
                } else if ((fSignum > BitmapDescriptorFactory.HUE_RED && f3 == 1.0f) || (fSignum < BitmapDescriptorFactory.HUE_RED && f3 == BitmapDescriptorFactory.HUE_RED)) {
                    setState(TransitionState.FINISHED);
                }
                if (!this.mKeepAnimating && !this.mInTransition && ((fSignum > BitmapDescriptorFactory.HUE_RED && f3 == 1.0f) || (fSignum < BitmapDescriptorFactory.HUE_RED && f3 == BitmapDescriptorFactory.HUE_RED))) {
                    onNewStateAttachHandlers();
                }
            } else {
                this.mLastVelocity = f2;
