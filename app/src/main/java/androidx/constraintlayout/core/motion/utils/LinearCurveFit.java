...
public class LinearCurveFit extends CurveFit {
    ...
    private double[] f45mT;
    private double mTotalLength;

    /* JADX INFO: renamed from: mY */
    private double[][] f46mY;
    ...
    public void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f45mT;
        int length = dArr2.length;
        int i = 0;
        int length2 = this.f46mY[0].length;
        if (this.mExtrapolate) {
            double d2 = dArr2[0];
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = this.f46mY[0][i2] + ((d - this.f45mT[0]) * this.mSlopeTemp[i2]);
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                ...
                while (i < length2) {
                    dArr[i] = this.f46mY[i3][i] + ((d - this.f45mT[i3]) * this.mSlopeTemp[i]);
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            if (d == this.f45mT[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    dArr[i7] = this.f46mY[i6][i7];
                }
            }
            double[] dArr3 = this.f45mT;
            int i8 = i6 + 1;
            double d4 = dArr3[i8];
            if (d < d4) {
                double d5 = dArr3[i6];
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (this.mExtrapolate) {
            ...
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (this.f46mY[0][i2] + ((d - this.f45mT[0]) * this.mSlopeTemp[i2]));
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                getSlope(d3, this.mSlopeTemp);
                while (i < length2) {
                    fArr[i] = (float) (this.f46mY[i3][i] + ((d - this.f45mT[i3]) * this.mSlopeTemp[i]));
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            if (d == this.f45mT[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    fArr[i7] = (float) this.f46mY[i6][i7];
                }
            }
            double[] dArr2 = this.f45mT;
            int i8 = i6 + 1;
    ...
    public double getPos(double d, int i) {
        ...
        if (this.mExtrapolate) {
            double d2 = dArr[0];
            if (d <= d2) {
                return this.f46mY[0][i] + ((d - d2) * getSlope(d2, i));
            }
            int i3 = length - 1;
            double d3 = dArr[i3];
            if (d >= d3) {
                return this.f46mY[i3][i] + ((d - d3) * getSlope(d3, i));
            }
        } else {
            if (d <= dArr[0]) {
                return this.f46mY[0][i];
            }
            int i4 = length - 1;
            if (d >= dArr[i4]) {
                return this.f46mY[i4][i];
            }
        }
        while (i2 < length - 1) {
            double[] dArr2 = this.f45mT;
