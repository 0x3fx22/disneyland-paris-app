...
public class MonotonicCurveFit extends CurveFit {
    ...
    public void getPos(double d, double[] dArr) {
        ...
        int i = 0;
        int length2 = this.f48mY[0].length;
        if (this.mExtrapolate) {
            double d2 = dArr2[0];
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = this.f48mY[0][i2] + ((d - this.f47mT[0]) * this.mSlopeTemp[i2]);
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                ...
                while (i < length2) {
                    dArr[i] = this.f48mY[i3][i] + ((d - this.f47mT[i3]) * this.mSlopeTemp[i]);
                    i++;
                }
                return;
            }
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (this.mExtrapolate) {
            ...
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (this.f48mY[0][i2] + ((d - this.f47mT[0]) * this.mSlopeTemp[i2]));
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                getSlope(d3, this.mSlopeTemp);
                while (i < length2) {
                    fArr[i] = (float) (this.f48mY[i3][i] + ((d - this.f47mT[i3]) * this.mSlopeTemp[i]));
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            if (d == this.f47mT[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    fArr[i7] = (float) this.f48mY[i6][i7];
                }
            }
            double[] dArr2 = this.f47mT;
            int i8 = i6 + 1;
