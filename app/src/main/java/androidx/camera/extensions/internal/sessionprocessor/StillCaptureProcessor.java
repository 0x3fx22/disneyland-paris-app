...
class StillCaptureProcessor {
    ...
    void startCapture(final boolean z, @NonNull final List<Integer> list, @NonNull final OnCaptureResultCallback onCaptureResultCallback) {
        Logger.m28d(TAG, "Start the capture: enablePostview=" + z);
        ...
        synchronized (this.mLock) {
            try {
                if (this.mIsClosed) {
                    ...
                    Logger.m28d(TAG, "Ignore image in closed state");
                    return;
                }
                Logger.m28d(TAG, "onImageReferenceIncoming  captureStageId=" + i);
                this.mCaptureResults.put(Integer.valueOf(i), new Pair<>(imageReference, totalCaptureResult));
                Logger.m28d(TAG, "mCaptureResult has capture stage Id: " + this.mCaptureResults.keySet());
        ...
        synchronized (this.mLock) {
            try {
                try {
                    try {
                        if (this.mIsClosed) {
                            Logger.m28d(TAG, "Ignore process() in closed state.");
                            Logger.m28d(TAG, "CaptureProcessorImpl.process() finish");
                        ...
                        Logger.m28d(TAG, "CaptureProcessorImpl.process() begin");
                        Version version = Version.VERSION_1_4;
                        ...
                        Logger.m31e(TAG, "mCaptureProcessorImpl.process exception ", e);
                        this.mOnCaptureResultCallback = null;
    ...
    void close() {
        synchronized (this.mLock) {
            Logger.m28d(TAG, "Close the StillCaptureProcessor");
            this.mIsClosed = true;
            clearCaptureResults();
