...
public final class ImageUtil {
    ...
    @NonNull
    public static Rational getRotatedAspectRatio(@IntRange(from = 0, m4to = 359) int i, @NonNull Rational rational) {
        if (i == 90 || i == 270) {
    ...
    @NonNull
    public static byte[] jpegImageToJpegByteArray(@NonNull ImageProxy imageProxy, @NonNull Rect rect, @IntRange(from = 1, m4to = WebToNativeBridgeBase.CLOSE_WAIT_DURATION_MILLISECONDS) int i) throws CodecFailedException {
        if (!isJpegFormats(imageProxy.getFormat())) {
            throw new IllegalArgumentException("Incorrect image format of the input image proxy: " + imageProxy.getFormat());
        }
    ...
    @NonNull
    public static byte[] yuvImageToJpegByteArray(@NonNull ImageProxy imageProxy, @Nullable Rect rect, @IntRange(from = 1, m4to = WebToNativeBridgeBase.CLOSE_WAIT_DURATION_MILLISECONDS) int i, int i2) throws CodecFailedException {
    ...
    public static Rect computeCropRectFromAspectRatio(@NonNull Size size, @NonNull Rational rational) {
        ...
        if (!isAspectRatioValid(rational)) {
            Logger.m34w("ImageUtil", "Invalid view ratio.");
            return null;
        }
