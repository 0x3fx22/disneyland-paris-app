...
public class SurfaceProcessorNode implements Node<AbstractC0278In, Out> {
    final CameraInternal mCameraInternal;
    private AbstractC0278In mInput;
    private Out mOutput;
    ...
    @NonNull
    @MainThread
    public Out transform(@NonNull AbstractC0278In abstractC0278In) {
        Threads.checkMainThread();
        this.mInput = abstractC0278In;
        this.mOutput = new Out();
        SurfaceEdge surfaceEdge = abstractC0278In.getSurfaceEdge();
        for (OutConfig outConfig : abstractC0278In.getOutConfigs()) {
    ...
    private void sendSurfaceRequest(SurfaceEdge surfaceEdge) {
        try {
            ...
            Logger.m31e("SurfaceProcessorNode", "Failed to send SurfaceRequest to SurfaceProcessor.", e);
        }
    }
        ...
        Futures.addCallback(surfaceEdge2.createSurfaceOutputFuture(((OutConfig) entry.getKey()).getFormat(), SurfaceOutput.CameraInputInfo.m38of(surfaceEdge.getStreamSpec().getResolution(), ((OutConfig) entry.getKey()).getCropRect(), surfaceEdge.hasCameraTransform() ? this.mCameraInternal : null, ((OutConfig) entry.getKey()).getRotationDegrees(), ((OutConfig) entry.getKey()).isMirroring()), null), new FutureCallback() { // from class: androidx.camera.core.processing.SurfaceProcessorNode.1
            ...
            public void onSuccess(SurfaceOutput surfaceOutput) {
                ...
                try {
                    ...
                    Logger.m31e("SurfaceProcessorNode", "Failed to send SurfaceOutput to SurfaceProcessor.", e);
                }
            }
            ...
            public void onFailure(Throwable th) {
                if (surfaceEdge2.getTargets() == 2 && (th instanceof CancellationException)) {
                    Logger.m28d("SurfaceProcessorNode", "Downstream VideoCapture failed to provide Surface.");
                    return;
                }
                Logger.m35w("SurfaceProcessorNode", "Downstream node failed to provide Surface. Target: " + TargetUtils.getHumanReadableName(surfaceEdge2.getTargets()), th);
            }
    ...
    @AutoValue
    public static abstract class AbstractC0278In {
        @NonNull
        ...
        public static AbstractC0278In m60of(@NonNull SurfaceEdge surfaceEdge, @NonNull List<OutConfig> list) {
