...
public class DualSurfaceProcessorNode implements Node<AbstractC0280In, Out> {
    private AbstractC0280In mInput;
    private Out mOutput;
    ...
    @NonNull
    @MainThread
    public Out transform(@NonNull AbstractC0280In abstractC0280In) {
        Threads.checkMainThread();
        this.mInput = abstractC0280In;
    ...
    private void sendSurfaceRequest(CameraInternal cameraInternal, SurfaceEdge surfaceEdge, Map map, boolean z) {
        try {
            ...
            Logger.m31e("DualSurfaceProcessorNode", "Failed to send SurfaceRequest to SurfaceProcessor.", e);
        }
    }
    ...
    public void lambda$sendSurfaceOutputs$0(CameraInternal cameraInternal, CameraInternal cameraInternal2, SurfaceEdge surfaceEdge, SurfaceEdge surfaceEdge2, Map.Entry entry) {
        ...
        SurfaceOutput.CameraInputInfo cameraInputInfoM38of = SurfaceOutput.CameraInputInfo.m38of(resolution, cropRect, cameraInternal, ((DualOutConfig) entry.getKey()).getPrimaryOutConfig().getRotationDegrees(), ((DualOutConfig) entry.getKey()).getPrimaryOutConfig().isMirroring());
        ...
        Futures.addCallback(surfaceEdge3.createSurfaceOutputFuture(((DualOutConfig) entry.getKey()).getPrimaryOutConfig().getFormat(), cameraInputInfoM38of, SurfaceOutput.CameraInputInfo.m38of(resolution2, cropRect2, cameraInternal2, ((DualOutConfig) entry.getKey()).getSecondaryOutConfig().getRotationDegrees(), ((DualOutConfig) entry.getKey()).getSecondaryOutConfig().isMirroring())), new FutureCallback() { // from class: androidx.camera.core.processing.concurrent.DualSurfaceProcessorNode.1
            ...
            public void onSuccess(SurfaceOutput surfaceOutput) {
                ...
                try {
                    ...
                    Logger.m31e("DualSurfaceProcessorNode", "Failed to send SurfaceOutput to SurfaceProcessor.", e);
                }
            }
            ...
            public void onFailure(Throwable th) {
                if (surfaceEdge3.getTargets() == 2 && (th instanceof CancellationException)) {
                    Logger.m28d("DualSurfaceProcessorNode", "Downstream VideoCapture failed to provide Surface.");
                    return;
                }
                Logger.m35w("DualSurfaceProcessorNode", "Downstream node failed to provide Surface. Target: " + TargetUtils.getHumanReadableName(surfaceEdge3.getTargets()), th);
            }
    ...
    @AutoValue
    public static abstract class AbstractC0280In {
        @NonNull
        ...
        public static AbstractC0280In m62of(@NonNull SurfaceEdge surfaceEdge, @NonNull SurfaceEdge surfaceEdge2, @NonNull List<DualOutConfig> list) {
