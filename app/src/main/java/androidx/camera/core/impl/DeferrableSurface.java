...
public abstract class DeferrableSurface {
    ...
    public /* synthetic */ void lambda$new$2(String str) {
        try {
            ...
            Logger.m30e("DeferrableSurface", "Unexpected surface termination for " + this + "\nStack Trace:\n" + str);
            synchronized (this.mLock) {
    ...
    private void printGlobalDebugCounts(String str, int i, int i2) {
        if (!DEBUG && Logger.isDebugEnabled("DeferrableSurface")) {
            Logger.m28d("DeferrableSurface", "DeferrableSurface usage statistics may be inaccurate since debug logging was not enabled at static initialization time. App restart may be required to enable accurate usage statistics.");
        }
        Logger.m28d("DeferrableSurface", str + "[total_surfaces=" + i + ", used_surfaces=" + i2 + "](" + this + "}");
    }
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (Logger.isDebugEnabled("DeferrableSurface")) {
                    ...
                    Logger.m28d("DeferrableSurface", "use count+1, useCount=" + this.mUseCount + " " + this);
                }
            } catch (Throwable th) {
                throw th;
            }
        ...
        synchronized (this.mLock) {
            try {
                if (this.mClosed) {
                    ...
                    if (Logger.isDebugEnabled("DeferrableSurface")) {
                        Logger.m28d("DeferrableSurface", "surface closed,  useCount=" + this.mUseCount + " closed=true " + this);
                    }
                }
            } catch (Throwable th) {
                throw th;
    ...
    public void decrementUseCount() {
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (Logger.isDebugEnabled("DeferrableSurface")) {
                    Logger.m28d("DeferrableSurface", "use count-1,  useCount=" + this.mUseCount + " closed=" + this.mClosed + " " + this);
                    if (this.mUseCount == 0) {
