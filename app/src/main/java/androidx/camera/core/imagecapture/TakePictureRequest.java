...
public abstract class TakePictureRequest {
    ...
    void markFormatProcessStatusInSimultaneousCapture(int i, boolean z) {
        if (!this.mFormatCaptureStatus.containsKey(Integer.valueOf(i))) {
            Logger.m30e("TakePictureRequest", "The format is not supported in simultaneous capture");
        } else {
    ...
    public static TakePictureRequest m50of(@NonNull Executor executor, @Nullable ImageCapture.OnImageCapturedCallback onImageCapturedCallback, @Nullable ImageCapture.OnImageSavedCallback onImageSavedCallback, @Nullable ImageCapture.OutputFileOptions outputFileOptions, @Nullable ImageCapture.OutputFileOptions outputFileOptions2, @NonNull Rect rect, @NonNull Matrix matrix, int i, int i2, int i3, boolean z, @NonNull List<CameraCaptureCallback> list) {
