        ...
        synchronized (this.mLock) {
            try {
                if (this.mMatchedImageProxies.size() < getMaxImages()) {
                    ...
                    Logger.m28d("TAG", "Maximum image number reached.");
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (size >= imageReaderProxy.getMaxImages()) {
                    Logger.m28d("MetadataImageReader", "Skip to acquire the next image because the acquired image count has reached the max images count.");
                    return;
                }
                do {
                    try {
                        ...
                        Logger.m29d("MetadataImageReader", "Failed to acquire next image.", e);
