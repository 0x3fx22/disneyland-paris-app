...
public final class ImageProcessingUtil {
    ...
    public static ImageProxy convertJpegBytesToImage(@NonNull ImageReaderProxy imageReaderProxy, @NonNull byte[] bArr) {
        ...
        if (nativeWriteJpegToSurface(bArr, surface) != 0) {
            Logger.m30e("ImageProcessingUtil", "Failed to enqueue JPEG image.");
            return null;
        }
        ImageProxy imageProxyAcquireLatestImage = imageReaderProxy.acquireLatestImage();
        if (imageProxyAcquireLatestImage == null) {
            Logger.m30e("ImageProcessingUtil", "Failed to get acquire JPEG image.");
        }
    ...
    public static boolean convertYuvToJpegBytesIntoSurface(@NonNull Image image, @IntRange(from = 1, m4to = WebToNativeBridgeBase.CLOSE_WAIT_DURATION_MILLISECONDS) int i, int i2, @NonNull Surface surface) {
        return convertYuvToJpegBytesIntoSurface(new AndroidImageProxy(image), i, i2, surface);
    }

    public static boolean convertYuvToJpegBytesIntoSurface(@NonNull ImageProxy imageProxy, @IntRange(from = 1, m4to = WebToNativeBridgeBase.CLOSE_WAIT_DURATION_MILLISECONDS) int i, int i2, @NonNull Surface surface) {
        try {
            return writeJpegBytesToSurface(surface, ImageUtil.yuvImageToJpegByteArray(imageProxy, null, i, i2));
        } catch (ImageUtil.CodecFailedException e) {
            Logger.m31e("ImageProcessingUtil", "Failed to encode YUV to JPEG", e);
            return false;
        }
    }

    @Nullable
    public static ImageProxy convertYUVToRGB(@NonNull final ImageProxy imageProxy, @NonNull ImageReaderProxy imageReaderProxy, @Nullable ByteBuffer byteBuffer, @IntRange(from = 0, m4to = 359) int i, boolean z) {
        if (!isSupportedYUVFormat(imageProxy)) {
            Logger.m30e("ImageProcessingUtil", "Unsupported format for YUV to RGB");
            return null;
        }
        long jCurrentTimeMillis = System.currentTimeMillis();
        if (!isSupportedRotationDegrees(i)) {
            Logger.m30e("ImageProcessingUtil", "Unsupported rotation degrees for rotate RGB");
            return null;
        }
        if (convertYUVToRGBInternal(imageProxy, imageReaderProxy.getSurface(), byteBuffer, i, z) == Result.ERROR_CONVERSION) {
            Logger.m30e("ImageProcessingUtil", "YUV to RGB conversion failure");
            return null;
        }
        if (Log.isLoggable("MH", 3)) {
            Logger.m28d("ImageProcessingUtil", String.format(Locale.US, "Image processing performance profiling, duration: [%d], image count: %d", Long.valueOf(System.currentTimeMillis() - jCurrentTimeMillis), Integer.valueOf(sImageCount)));
            sImageCount++;
        }
        final ImageProxy imageProxyAcquireLatestImage = imageReaderProxy.acquireLatestImage();
        if (imageProxyAcquireLatestImage == null) {
            Logger.m30e("ImageProcessingUtil", "YUV to RGB acquireLatestImage failure");
            return null;
        }
    ...
    public static boolean applyPixelShiftForYUV(@NonNull ImageProxy imageProxy) {
        ...
        Logger.m30e("ImageProcessingUtil", "One pixel shift for YUV failure");
        return false;
    }

    @Nullable
    public static ImageProxy rotateYUV(@NonNull final ImageProxy imageProxy, @NonNull ImageReaderProxy imageReaderProxy, @NonNull ImageWriter imageWriter, @NonNull ByteBuffer byteBuffer, @NonNull ByteBuffer byteBuffer2, @NonNull ByteBuffer byteBuffer3, @IntRange(from = 0, m4to = 359) int i) {
        if (!isSupportedYUVFormat(imageProxy)) {
            Logger.m30e("ImageProcessingUtil", "Unsupported format for rotate YUV");
            return null;
        }
        if (!isSupportedRotationDegrees(i)) {
            Logger.m30e("ImageProcessingUtil", "Unsupported rotation degrees for rotate YUV");
            return null;
        }
        ...
        if ((i > 0 ? rotateYUVInternal(imageProxy, imageWriter, byteBuffer, byteBuffer2, byteBuffer3, i) : result) == result) {
            Logger.m30e("ImageProcessingUtil", "rotate YUV failure");
            return null;
        }
        final ImageProxy imageProxyAcquireLatestImage = imageReaderProxy.acquireLatestImage();
        if (imageProxyAcquireLatestImage == null) {
            Logger.m30e("ImageProcessingUtil", "YUV rotation acquireLatestImage failure");
            return null;
        }
        SingleCloseImageProxy singleCloseImageProxy = new SingleCloseImageProxy(imageProxyAcquireLatestImage);
