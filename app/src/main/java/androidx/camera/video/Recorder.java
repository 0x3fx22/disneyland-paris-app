...
public final class Recorder implements VideoOutput {
    ...
    Recorder(Executor executor, MediaSpec mediaSpec, int i, EncoderFactory encoderFactory, EncoderFactory encoderFactory2, OutputStorage.Factory factory, long j) {
        ...
        this.mStreamInfo = MutableStateObservable.withInitialState(StreamInfo.m78of(this.mStreamId, internalStateToStreamState(this.mState)));
        ...
        Logger.m28d("Recorder", "mRequiredFreeStorageBytes = " + StorageUtil.formatSize(j));
    }
    ...
    public void onSurfaceRequested(@NonNull final SurfaceRequest surfaceRequest, @NonNull final Timebase timebase) {
        synchronized (this.mLock) {
            try {
                Logger.m28d("Recorder", "Surface is requested in state: " + this.mState + ", Current surface: " + this.mStreamId);
    ...
    Recording start(PendingRecording pendingRecording) {
        ...
        if (i != 0) {
            Logger.m30e("Recorder", "Recording was started when the Recorder had encountered error " + e);
        ...
        synchronized (this.mLock) {
            try {
                if (!isSameRecording(recording, this.mPendingRecordingRecord) && !isSameRecording(recording, this.mActiveRecordingRecord)) {
                    Logger.m28d("Recorder", "pause() called on a recording that is no longer active: " + recording.getOutputOptions());
                    return;
                }
    ...
    void resume(Recording recording) {
        synchronized (this.mLock) {
            try {
                if (!isSameRecording(recording, this.mPendingRecordingRecord) && !isSameRecording(recording, this.mActiveRecordingRecord)) {
                    Logger.m28d("Recorder", "resume() called on a recording that is no longer active: " + recording.getOutputOptions());
                    return;
                }
                int iOrdinal = this.mState.ordinal();
                if (iOrdinal != 0) {
    ...
    void stop(Recording recording, final int i, final Throwable th) {
        synchronized (this.mLock) {
            try {
                if (!isSameRecording(recording, this.mPendingRecordingRecord) && !isSameRecording(recording, this.mActiveRecordingRecord)) {
                    Logger.m28d("Recorder", "stop() called on a recording that is no longer active: " + recording.getOutputOptions());
                    return;
                }
                ...
                if (recordingRecord != null) {
                    if (i == 10) {
                        Logger.m30e("Recorder", "Recording was stopped due to recording being garbage collected before any valid data has been produced.");
                    }
    ...
    void mute(Recording recording, final boolean z) {
        synchronized (this.mLock) {
            try {
                if (!isSameRecording(recording, this.mPendingRecordingRecord) && !isSameRecording(recording, this.mActiveRecordingRecord)) {
                    Logger.m28d("Recorder", "mute() called on a recording that is no longer active: " + recording.getOutputOptions());
                    return;
                }
    ...
    private void finalizePendingRecording(RecordingRecord recordingRecord, int i, Throwable th) {
        ...
        recordingRecord.updateVideoRecordEvent(VideoRecordEvent.finalizeWithError(recordingRecord.getOutputOptions(), RecordingStats.m77of(0L, 0L, AudioStats.m73of(1, this.mAudioErrorCause, AudioStats.AUDIO_AMPLITUDE_NONE)), OutputResults.m74of(uri), i, th));
    }
    ...
    public void lambda$onSourceStateChanged$1(VideoOutput.SourceState sourceState) {
        ...
        if (sourceState2 != sourceState) {
            Logger.m28d("Recorder", "Video source has transitioned to state: " + sourceState);
        ...
        Logger.m28d("Recorder", "Video source transitions to the same state: " + sourceState);
    }
    ...
    private void configureInternal(SurfaceRequest surfaceRequest, Timebase timebase, boolean z) {
        if (surfaceRequest.isServiced()) {
            Logger.m34w("Recorder", "Ignore the SurfaceRequest since it is already served.");
            return;
        }
        ...
        Logger.m28d("Recorder", "Using supported quality of " + qualityFindNearestHigherSupportedQualityFor + " for surface size " + resolution);
    ...
    class SetupVideoTask {
        ...
        public /* synthetic */ void lambda$setupVideo$0(SurfaceRequest surfaceRequest, Timebase timebase) {
            if (!surfaceRequest.isServiced() && (!Recorder.this.mVideoEncoderSession.isConfiguredSurfaceRequest(surfaceRequest) || Recorder.this.isPersistentRecordingInProgress())) {
                ...
                Futures.addCallback(listenableFutureConfigure, new C03201(videoEncoderSession), Recorder.this.mSequentialExecutor);
                return;
            }
            Logger.m34w("Recorder", "Ignore the SurfaceRequest " + surfaceRequest + " isServiced: " + surfaceRequest.isServiced() + " VideoEncoderSession: " + Recorder.this.mVideoEncoderSession + " has been configured with a persistent in-progress recording.");
        }

        /* JADX INFO: renamed from: androidx.camera.video.Recorder$SetupVideoTask$1 */
        class C03201 implements FutureCallback {
            final /* synthetic */ VideoEncoderSession val$videoEncoderSession;

            C03201(VideoEncoderSession videoEncoderSession) {
                this.val$videoEncoderSession = videoEncoderSession;
            }

            @Override // androidx.camera.core.impl.utils.futures.FutureCallback
            public void onSuccess(Encoder encoder) {
                Logger.m28d("Recorder", "VideoEncoder is created. " + encoder);
                if (encoder == null) {
                    return;
                }
            ...
            public void onFailure(Throwable th) {
                Logger.m35w("Recorder", "VideoEncoder Setup error: " + th, th);
            ...
            public /* synthetic */ void lambda$onFailure$0() {
                ...
                Logger.m28d("Recorder", "Retry setupVideo #" + SetupVideoTask.this.mRetryCount);
    ...
    public ListenableFuture safeToCloseVideoEncoder() {
        Logger.m28d("Recorder", "Try to safely release video encoder: " + this.mVideoEncoder);
    ...
    void onVideoEncoderReady(final VideoEncoderSession videoEncoderSession) {
        ...
        Futures.addCallback(videoEncoderSession.getReadyToReleaseFuture(), new FutureCallback() { // from class: androidx.camera.video.Recorder.1
            ...
            public void onSuccess(Encoder encoder) {
                Encoder encoder2;
                Logger.m28d("Recorder", "VideoEncoder can be released: " + encoder);
                if (encoder == null) {
                    return;
                }
            ...
            @Override // androidx.camera.core.impl.utils.futures.FutureCallback
            public void onFailure(Throwable th) {
                Logger.m28d("Recorder", "Error in ReadyToReleaseFuture: " + th);
            }
            ...
            try {
                ...
                switch (this.mState) {
                    ...
                    case STOPPING:
                        ...
                        break;
                    case ERROR:
                        Logger.m30e("Recorder", "onConfigured() was invoked when the Recorder had encountered error");
                        recordingRecord = null;
                        exc = null;
                        z = false;
                        i = 0;
    ...
    private void setupAudio(RecordingRecord recordingRecord) throws InvalidConfigException {
        ...
        Logger.m28d("Recorder", String.format("Set up new audio source: 0x%x", Integer.valueOf(audioSource.hashCode())));
    ...
    private void releaseCurrentAudioSource() {
        ...
        this.mAudioSource = null;
        Logger.m28d("Recorder", String.format("Releasing audio source: 0x%x", Integer.valueOf(audioSource.hashCode())));
        Futures.addCallback(audioSource.release(), new FutureCallback() { // from class: androidx.camera.video.Recorder.2
            @Override // androidx.camera.core.impl.utils.futures.FutureCallback
            public void onSuccess(Void r1) {
                Logger.m28d("Recorder", String.format("Released audio source successfully: 0x%x", Integer.valueOf(audioSource.hashCode())));
            }

            @Override // androidx.camera.core.impl.utils.futures.FutureCallback
            public void onFailure(Throwable th) {
                Logger.m28d("Recorder", String.format("An error occurred while attempting to release audio source: 0x%x", Integer.valueOf(audioSource.hashCode())));
            }
    ...
    void setupAndStartMediaMuxer(RecordingRecord recordingRecord) {
        ...
        try {
            ...
            if (j != 0 && size > j) {
                Logger.m28d("Recorder", String.format("Initial data exceeds file size limit %d > %d", Long.valueOf(size), Long.valueOf(this.mFileSizeLimitInBytes)));
    ...
    private void startInternal(RecordingRecord recordingRecord) {
        ...
        Logger.m28d("Recorder", "availableBytes = " + StorageUtil.formatSize(availableBytes));
        ...
        if (recordingRecord.getOutputOptions().getFileSizeLimit() > 0) {
            ...
            Logger.m28d("Recorder", "File size limit in bytes: " + this.mFileSizeLimitInBytes);
        } else {
            this.mFileSizeLimitInBytes = 0L;
        }
        if (recordingRecord.getOutputOptions().getDurationLimitMillis() > 0) {
            ...
            Logger.m28d("Recorder", "Duration limit in nanoseconds: " + this.mDurationLimitNs);
        } else {
            this.mDurationLimitNs = 0L;
        }
        ...
        if (iOrdinal != 0) {
            ...
            try {
                ...
                Logger.m31e("Recorder", "Unable to create audio resource with error: ", e);
    ...
    private void updateEncoderCallbacks(final RecordingRecord recordingRecord, boolean z) {
        ...
        Futures.addCallback(Futures.allAsList(this.mEncodingFutures), new FutureCallback() { // from class: androidx.camera.video.Recorder.6
            ...
            public void onSuccess(List list) {
                Logger.m28d("Recorder", "Encodings end successfully.");
                Recorder recorder = Recorder.this;
            ...
            public void onFailure(Throwable th) {
                ...
                Logger.m28d("Recorder", "Encodings end with error: " + th);
                Recorder recorder = Recorder.this;
                    ...
                    if (!recorder.mInProgressRecordingStopping) {
                        ...
                        if (encodedData.isKeyFrame()) {
                            ...
                            if (!recorder2.isAudioEnabled() || !Recorder.this.mPendingAudioRingBuffer.isEmpty()) {
                                Logger.m28d("Recorder", "Received video keyframe. Starting muxer...");
                                Recorder.this.setupAndStartMediaMuxer(recordingRecord);
                                return;
                            } else if (z) {
                                Logger.m28d("Recorder", "Replaced cached video keyframe with newer keyframe.");
                                return;
                            } else {
                                Logger.m28d("Recorder", "Cached video keyframe while we wait for first audio sample before starting muxer.");
                                return;
                            }
                        }
                        if (z) {
                            Logger.m28d("Recorder", "Dropped cached keyframe since we have new video data and have not yet received audio data.");
                        }
                        Logger.m28d("Recorder", "Dropped video data since muxer has not yet started and data is not a keyframe.");
                        Recorder.this.mVideoEncoder.requestKeyFrame();
                        encodedData.close();
                        return;
                    }
                    Logger.m28d("Recorder", "Drop video data since recording is stopping.");
                    encodedData.close();
                    return;
                }
                try {
        ...
        this.mAudioSource.setAudioSourceCallback(this.mSequentialExecutor, new AudioSource.AudioSourceCallback() { // from class: androidx.camera.video.Recorder.4
            ...
            public void onSilenceStateChanged(boolean z) {
                ...
                if (recorder.mIsAudioSourceSilenced != z) {
                    ...
                    Logger.m34w("Recorder", "Audio source silenced transitions to the same state " + z);
                }
            }

            @Override // androidx.camera.video.internal.audio.AudioSource.AudioSourceCallback
            public void onError(Throwable th) {
                Logger.m31e("Recorder", "Error occurred after audio source started.", th);
                ...
                if (recorder.mMediaMuxer == null) {
                    if (!recorder.mInProgressRecordingStopping) {
                        ...
                        if (Recorder.this.mPendingFirstVideoData != null) {
                            Logger.m28d("Recorder", "Received audio data. Starting muxer...");
                            Recorder.this.setupAndStartMediaMuxer(recordingRecord);
                        } else {
                            Logger.m28d("Recorder", "Cached audio data while we wait for video keyframe before starting muxer.");
                        }
                    } else {
                        Logger.m28d("Recorder", "Drop audio data since recording is stopping.");
                    }
                    encodedData.close();
                    return;
                }
    ...
    void writeVideoData(EncodedData encodedData, RecordingRecord recordingRecord) {
        ...
        long j = this.mFileSizeLimitInBytes;
        if (j != 0 && size > j) {
            Logger.m28d("Recorder", String.format("Reach file size limit %d > %d", Long.valueOf(size), Long.valueOf(this.mFileSizeLimitInBytes)));
            onInProgressRecordingInternalError(recordingRecord, 2, null);
            return;
        }
        ...
        if (j2 == Long.MAX_VALUE) {
            ...
            Logger.m28d("Recorder", String.format("First video time: %d (%s)", Long.valueOf(presentationTimeUs), DebugUtils.readableUs(this.mFirstRecordingVideoDataTimeUs)));
            str = "Recorder";
            nanos = 0;
        } else {
            ...
            if (j3 != 0 && nanos2 > j3) {
                Logger.m28d("Recorder", String.format("Video data reaches duration limit %d > %d", Long.valueOf(nanos2), Long.valueOf(this.mDurationLimitNs)));
        ...
        try {
            ...
            if (size > this.mAvailableBytesAboveRequired) {
                ...
                Logger.m28d(str, "availableBytes = " + StorageUtil.formatSize(availableBytes));
    ...
    void writeAudioData(EncodedData encodedData, RecordingRecord recordingRecord) {
        ...
        if (j2 == Long.MAX_VALUE) {
            ...
            Logger.m28d("Recorder", String.format("First audio time: %d (%s)", Long.valueOf(presentationTimeUs), DebugUtils.readableUs(this.mFirstRecordingAudioDataTimeUs)));
        } else {
            TimeUnit timeUnit = TimeUnit.MICROSECONDS;
            ...
            long j3 = this.mDurationLimitNs;
            if (j3 != 0 && nanos2 > j3) {
                Logger.m28d("Recorder", String.format("Audio data reaches duration limit %d > %d", Long.valueOf(nanos2), Long.valueOf(this.mDurationLimitNs)));
                onInProgressRecordingInternalError(recordingRecord, 9, null);
                return;
            }
        }
    ...
    public static /* synthetic */ void lambda$stopInternal$13(Encoder encoder) {
        Logger.m28d("Recorder", "The source didn't become non-streaming before timeout. Waited 1000ms");
    ...
    private void reset() {
        if (this.mAudioEncoder != null) {
            Logger.m28d("Recorder", "Releasing audio encoder.");
    ...
    private void tryReleaseVideoEncoder() {
        ...
        if (videoEncoderSession != null) {
            ...
            Logger.m28d("Recorder", "Releasing video encoder: " + this.mVideoEncoder);
    ...
    private void resetVideo() {
        if (this.mVideoEncoder != null) {
            Logger.m28d("Recorder", "Releasing video encoder.");
    ...
    void finalizeInProgressRecording(int i, Throwable th) {
        ...
        if (mediaMuxer != null) {
            try {
                ...
                Logger.m31e("Recorder", "MediaMuxer failed to stop or release with error: " + e.getMessage(), e);
                if (i == 0) {
        ...
        OutputResults outputResultsM74of = OutputResults.m74of(this.mOutputUri);
        RecordingRecord recordingRecord = this.mInProgressRecording;
        if (i == 0) {
            finalizeFinalizeWithError = VideoRecordEvent.finalize(outputOptions, inProgressRecordingStats, outputResultsM74of);
        } else {
            finalizeFinalizeWithError = VideoRecordEvent.finalizeWithError(outputOptions, inProgressRecordingStats, outputResultsM74of, i, th);
        }
    ...
    /* JADX WARN: Type inference failed for: r2v12, types: [androidx.camera.video.Recorder$RecordingRecord] */
    ...
    private void onRecordingFinalized(RecordingRecord recordingRecord) {
        ...
        synchronized (this.mLock) {
            try {
                RecordingRecord recordingRecord2 = this.mActiveRecordingRecord;
                if (recordingRecord2 != recordingRecord) {
                    throw new AssertionError("Active recording did not match finalized recording on finalize.");
                }
                recordingRecord2.getRecordingState().removeObservers();
                recordingRecordMakePendingRecordingActiveLocked = null;
                exc = null;
                recordingRecordMakePendingRecordingActiveLocked = null;
                recordingRecordMakePendingRecordingActiveLocked = null;
                ...
                switch (this.mState.ordinal()) {
                    ...
                    case 2:
                        z = true;
                        if (this.mSourceState == VideoOutput.SourceState.INACTIVE) {
                            RecordingRecord recordingRecord4 = this.mPendingRecordingRecord;
                            this.mPendingRecordingRecord = null;
                            setState(State.CONFIGURING);
                            exc = PENDING_RECORDING_ERROR_CAUSE_SOURCE_INACTIVE;
                            i2 = 0;
        ...
        if (recordingRecordMakePendingRecordingActiveLocked != null) {
            ...
            finalizePendingRecording(r2, i3, exc);
        }
    }
    ...
    void tryServicePendingRecording() {
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (iOrdinal != 1) {
                    ...
                    th = recordingRecordMakePendingRecordingActiveLocked;
                } else {
                    z2 = false;
                }
        ...
        if (recordingRecordMakePendingRecordingActiveLocked != null) {
            ...
            finalizePendingRecording(recordingRecordMakePendingRecordingActiveLocked, i, th);
        }
    }
    ...
    RecordingStats getInProgressRecordingStats() {
        return RecordingStats.m77of(this.mRecordingDurationNs, this.mRecordingBytes, AudioStats.m73of(internalAudioStateToAudioStatsState(this.mAudioState), this.mAudioErrorCause, this.mAudioAmplitude));
    }
    ...
    void setState(State state) {
        ...
        Logger.m28d("Recorder", "Transitioning Recorder internal state: " + this.mState + " --> " + state);
        ...
        this.mStreamInfo.setState(StreamInfo.m79of(this.mStreamId, streamStateInternalStateToStreamState, this.mInProgressTransformationInfo));
    }
    ...
    private void setStreamId(int i) {
        ...
        Logger.m28d("Recorder", "Transitioning streamId: " + this.mStreamId + " --> " + i);
        this.mStreamId = i;
        this.mStreamInfo.setState(StreamInfo.m79of(i, internalStateToStreamState(this.mState), this.mInProgressTransformationInfo));
    }

    void setInProgressTransformationInfo(SurfaceRequest.TransformationInfo transformationInfo) {
        Logger.m28d("Recorder", "Update stream transformation info: " + transformationInfo);
        this.mInProgressTransformationInfo = transformationInfo;
        synchronized (this.mLock) {
            this.mStreamInfo.setState(StreamInfo.m79of(this.mStreamId, internalStateToStreamState(this.mState), transformationInfo));
        }
    }
    ...
    private void updateNonPendingState(State state) {
        ...
        if (this.mNonPendingState != state) {
            ...
            this.mStreamInfo.setState(StreamInfo.m79of(this.mStreamId, internalStateToStreamState(state), this.mInProgressTransformationInfo));
        }
    }
    ...
    void setAudioState(AudioState audioState) {
        Logger.m28d("Recorder", "Transitioning audio state: " + this.mAudioState + " --> " + audioState);
    ...
    static abstract class RecordingRecord implements AutoCloseable {
        ...
        public static /* synthetic */ MediaMuxer lambda$initializeRecording$1(OutputOptions outputOptions, ParcelFileDescriptor parcelFileDescriptor, int i, Consumer consumer) throws IOException {
            ...
            if (outputOptions instanceof FileOutputOptions) {
                ...
                if (!OutputUtil.createParentFolder(file)) {
                    Logger.m34w("Recorder", "Failed to create folder for " + file.getAbsolutePath());
                }
        ...
        public static /* synthetic */ void lambda$initializeRecording$5(ParcelFileDescriptor parcelFileDescriptor, Uri uri) {
            try {
                ...
                Logger.m31e("Recorder", "Failed to close dup'd ParcelFileDescriptor", e);
            }
        }
        ...
        void updateVideoRecordEvent(final VideoRecordEvent videoRecordEvent) {
            ...
            try {
                ...
                Logger.m31e("Recorder", "The callback executor is invalid.", e);
            }
        }
