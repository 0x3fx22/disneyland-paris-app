...
final class VideoEncoderSession {
    ...
    ListenableFuture configure(final SurfaceRequest surfaceRequest, final Timebase timebase, final MediaSpec mediaSpec, final VideoValidatedEncoderProfilesProxy videoValidatedEncoderProfilesProxy) {
        if (this.mVideoEncoderState.ordinal() == 0) {
            ...
            Logger.m28d("VideoEncoderSession", "Create VideoEncoderSession: " + this);
            ...
            Futures.addCallback(future, new FutureCallback() { // from class: androidx.camera.video.VideoEncoderSession.1
                ...
                public void onFailure(Throwable th) {
                    Logger.m35w("VideoEncoderSession", "VideoEncoder configuration failed.", th);
    ...
    void terminateNow() {
        ...
        if (iOrdinal != 1 && iOrdinal != 2 && iOrdinal != 3) {
            if (iOrdinal == 4) {
                Logger.m28d("VideoEncoderSession", "terminateNow in " + this.mVideoEncoderState + ", No-op");
                return;
            }
            throw new IllegalStateException("State " + this.mVideoEncoderState + " is not handled");
        }
        ...
        if (this.mVideoEncoder != null) {
            Logger.m28d("VideoEncoderSession", "VideoEncoder is releasing: " + this.mVideoEncoder);
        ...
        Logger.m34w("VideoEncoderSession", "There's no VideoEncoder to release! Finish release completer.");
    ...
    private void closeInternal() {
        ...
        if (iOrdinal == 2 || iOrdinal == 3) {
            Logger.m28d("VideoEncoderSession", "closeInternal in " + this.mVideoEncoderState + " state");
            this.mVideoEncoderState = VideoEncoderState.PENDING_RELEASE;
            return;
        }
        if (iOrdinal == 4) {
            Logger.m28d("VideoEncoderSession", "closeInternal in RELEASED state, No-op");
            return;
        }
        throw new IllegalStateException("State " + this.mVideoEncoderState + " is not handled");
    }
    ...
    private void configureVideoEncoderInternal(final SurfaceRequest surfaceRequest, Timebase timebase, VideoValidatedEncoderProfilesProxy videoValidatedEncoderProfilesProxy, MediaSpec mediaSpec, final CallbackToFutureAdapter.Completer completer) {
        ...
        try {
            ...
            Logger.m31e("VideoEncoderSession", "Unable to initialize video encoder.", e);
            completer.setException(e);
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$configureVideoEncoderInternal$5(CallbackToFutureAdapter.Completer completer, SurfaceRequest surfaceRequest, final Surface surface) {
        Executor executor;
        int iOrdinal = this.mVideoEncoderState.ordinal();
        if (iOrdinal != 0) {
            if (iOrdinal == 1) {
                if (surfaceRequest.isServiced()) {
                    Logger.m28d("VideoEncoderSession", "Not provide surface, " + Objects.toString(surfaceRequest, "EMPTY") + " is already serviced.");
                    completer.set(null);
                    closeInternal();
                    return;
                }
                this.mActiveSurface = surface;
                Logger.m28d("VideoEncoderSession", "provide surface: " + surface);
            ...
            if (iOrdinal != 2) {
                if (iOrdinal == 3) {
                    ...
                    Logger.m34w("VideoEncoderSession", "Surface is updated in READY state: " + surface);
                    return;
                }
                if (iOrdinal != 4) {
                    throw new IllegalStateException("State " + this.mVideoEncoderState + " is not handled");
        ...
        Logger.m28d("VideoEncoderSession", "Not provide surface in " + this.mVideoEncoderState);
        completer.set(null);
    }
    ...
    public void onSurfaceRequestComplete(SurfaceRequest.Result result) {
        Logger.m28d("VideoEncoderSession", "Surface can be closed: " + result.getSurface().hashCode());
