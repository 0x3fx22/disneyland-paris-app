...
public final class VideoCapture<T extends VideoOutput> extends UseCase {
    ...
    VideoCapture(VideoCaptureConfig videoCaptureConfig) {
        ...
        this.mStreamInfoObserver = new Observable.Observer() { // from class: androidx.camera.video.VideoCapture.1
            ...
            public void onNewData(StreamInfo streamInfo) {
                ...
                Logger.m28d("VideoCapture", "Stream info update: old: " + VideoCapture.this.mStreamInfo + " new: " + streamInfo);
                ...
                if ((streamInfo2.getId() != -1 && streamInfo.getId() == -1) || (streamInfo2.getId() == -1 && streamInfo.getId() != -1)) {
                    ...
                    videoCapture3.updateSessionConfig(ImageAnalysis$$ExternalSyntheticBackport2.m23m(new Object[]{videoCapture3.mSessionConfigBuilder.build()}));
                ...
                if (streamInfo2.getStreamState() != streamInfo.getStreamState()) {
                    ...
                    videoCapture5.updateSessionConfig(ImageAnalysis$$ExternalSyntheticBackport2.m23m(new Object[]{videoCapture5.mSessionConfigBuilder.build()}));
            ...
            public void onError(Throwable th) {
                Logger.m35w("VideoCapture", "Receive onError from StreamState observer", th);
            }
        };
    }
    ...
    protected StreamSpec onSuggestedStreamSpecUpdated(@NonNull StreamSpec streamSpec, @Nullable StreamSpec streamSpec2) {
        Logger.m28d("VideoCapture", "onSuggestedStreamSpecUpdated: " + streamSpec);
        ...
        if (customOrderedResolutions != null && !customOrderedResolutions.contains(streamSpec.getResolution())) {
            Logger.m34w("VideoCapture", "suggested resolution " + streamSpec.getResolution() + " is not in custom ordered resolutions " + customOrderedResolutions);
        }
        return streamSpec;
    }
    ...
    public void onStateAttached() {
        ...
        Logger.m28d("VideoCapture", "VideoCapture#onStateAttached: cameraID = " + getCameraId());
        ...
        updateSessionConfig(ImageAnalysis$$ExternalSyntheticBackport2.m23m(new Object[]{this.mSessionConfigBuilder.build()}));
        notifyActive();
    ...
    public void onStateDetached() {
        Logger.m28d("VideoCapture", "VideoCapture#onStateDetached");
        ...
        if (listenableFuture != null && listenableFuture.cancel(false)) {
            Logger.m28d("VideoCapture", "VideoCapture is detached from the camera. Surface update cancelled.");
        }
        clearPipeline();
    }
    ...
    private SessionConfig.Builder createPipeline(final VideoCaptureConfig videoCaptureConfig, StreamSpec streamSpec) {
        ...
        Logger.m28d("VideoCapture", "camera timebase = " + cameraInternal.getCameraInfoInternal().getTimebase() + ", processing timebase = " + timebaseResolveTimebase);
        ...
        if (this.mNode != null) {
            OutConfig outConfigM65of = OutConfig.m65of(this.mCameraEdge);
            final SurfaceEdge surfaceEdge2 = this.mNode.transform(SurfaceProcessorNode.AbstractC0278In.m60of(this.mCameraEdge, Collections.singletonList(outConfigM65of))).get(outConfigM65of);
    ...
    static class SourceStreamRequirementObserver implements Observable.Observer {
        ...
        @Override // androidx.camera.core.impl.Observable.Observer
        public void onError(Throwable th) {
            Logger.m35w("VideoCapture", "SourceStreamRequirementObserver#onError", th);
        }
        ...
        private void updateVideoUsageInCamera(boolean z) {
            ...
            if (cameraControlInternal == null) {
                Logger.m28d("VideoCapture", "SourceStreamRequirementObserver#isSourceStreamRequired: Received new data despite being closed already");
            } else if (z) {
        ...
        public void close() {
            ...
            Logger.m28d("VideoCapture", "SourceStreamRequirementObserver#close: mIsSourceStreamRequired = " + this.mIsSourceStreamRequired);
            if (this.mCameraControl == null) {
                Logger.m28d("VideoCapture", "SourceStreamRequirementObserver#close: Already closed!");
            } else {
    ...
    private SurfaceProcessorNode createNodeIfNeeded(CameraInternal cameraInternal, VideoCaptureConfig videoCaptureConfig, Rect rect, Size size, DynamicRange dynamicRange) {
        ...
        Logger.m28d("VideoCapture", "Surface processing is enabled.");
        CameraInternal camera = getCamera();
    ...
    private static Rect adjustCropRectToValidSize(final Rect rect, Size size, VideoEncoderInfo videoEncoderInfo) {
        Logger.m28d("VideoCapture", String.format("Adjust cropRect %s by width/height alignment %d/%d and supported widths %s / supported heights %s", TransformUtils.rectToString(rect), Integer.valueOf(videoEncoderInfo.getWidthAlignment()), Integer.valueOf(videoEncoderInfo.getHeightAlignment()), videoEncoderInfo.getSupportedWidths(), videoEncoderInfo.getSupportedHeights()));
        ...
        if (hashSet.isEmpty()) {
            Logger.m34w("VideoCapture", "Can't find valid cropped size");
            return rect;
        }
        ArrayList arrayList = new ArrayList(hashSet);
        Logger.m28d("VideoCapture", "candidatesList = " + arrayList);
        ...
        Logger.m28d("VideoCapture", "sorted candidatesList = " + arrayList);
        Size size2 = (Size) arrayList.get(0);
        int width = size2.getWidth();
        int height = size2.getHeight();
        if (width == rect.width() && height == rect.height()) {
            Logger.m28d("VideoCapture", "No need to adjust cropRect because crop size is valid.");
            return rect;
        }
        ...
        Logger.m28d("VideoCapture", String.format("Adjust cropRect from %s to %s", TransformUtils.rectToString(rect), TransformUtils.rectToString(rect2)));
        return rect2;
    }
    ...
    private static void addBySupportedSize(Set set, int i, int i2, Size size, VideoEncoderInfo videoEncoderInfo) {
        ...
        try {
            ...
            Logger.m35w("VideoCapture", "No supportedHeights for width: " + i, e);
        }
        try {
            set.add(new Size(((Integer) videoEncoderInfo.getSupportedWidthsFor(i2).clamp(Integer.valueOf(i))).intValue(), i2));
        } catch (IllegalArgumentException e2) {
            Logger.m35w("VideoCapture", "No supportedWidths for height: " + i2, e2);
        }
    }
    ...
    private static VideoEncoderInfo resolveVideoEncoderInfo(Function function, VideoValidatedEncoderProfilesProxy videoValidatedEncoderProfilesProxy, MediaSpec mediaSpec, Size size, DynamicRange dynamicRange, Range range) {
        ...
        if (videoEncoderInfo == null) {
            Logger.m34w("VideoCapture", "Can't find videoEncoderInfo");
            return null;
        }
    ...
    private void setupSurfaceUpdateNotifier(final SessionConfig.Builder builder, final boolean z) {
        ListenableFuture listenableFuture = this.mSurfaceUpdateFuture;
        if (listenableFuture != null && listenableFuture.cancel(false)) {
            Logger.m28d("VideoCapture", "A newer surface update is requested. Previous surface update cancelled.");
        }
        ...
        Futures.addCallback(future, new FutureCallback() { // from class: androidx.camera.video.VideoCapture.3
            ...
            public void onFailure(Throwable th) {
                ...
                Logger.m31e("VideoCapture", "Surface update completed with unexpected exception", th);
            }
        }, CameraXExecutors.mainThreadExecutor());
    }
    ...
    public /* synthetic */ Object lambda$setupSurfaceUpdateNotifier$6(final SessionConfig.Builder builder, CallbackToFutureAdapter.Completer completer) {
        ...
        final C03232 c03232 = new C03232(atomicBoolean, completer, builder);
        completer.addCancellationListener(new Runnable() { // from class: androidx.camera.video.VideoCapture$$ExternalSyntheticLambda8
            @Override // java.lang.Runnable
            public final void run() {
                VideoCapture.lambda$setupSurfaceUpdateNotifier$5(atomicBoolean, builder, c03232);
            }
        }, CameraXExecutors.directExecutor());
        builder.addRepeatingCameraCaptureCallback(c03232);
    ...
    class C03232 extends CameraCaptureCallback {
        ...
        C03232(AtomicBoolean atomicBoolean, CallbackToFutureAdapter.Completer completer, SessionConfig.Builder builder) {
        ...
        public void onCaptureCompleted(int i, CameraCaptureResult cameraCaptureResult) {
            ...
            if (this.mIsFirstCaptureResult) {
                ...
                Logger.m28d("VideoCapture", "cameraCaptureResult timestampNs = " + cameraCaptureResult.getTimestamp() + ", current system uptimeMs = " + SystemClock.uptimeMillis() + ", current system realtimeMs = " + SystemClock.elapsedRealtime());
            }
    ...
    private void updateCustomOrderedResolutionsByQuality(CameraInfoInternal cameraInfoInternal, UseCaseConfig.Builder builder) {
        ...
        if (supportedQualities.isEmpty()) {
            Logger.m34w("VideoCapture", "Can't find any supported quality on the device.");
            return;
        }
        ...
        Logger.m28d("VideoCapture", "Found selectedQualities " + prioritizedQualities + " by " + qualitySelector);
        ...
        Logger.m28d("VideoCapture", "Set custom ordered resolutions = " + listFilterOutEncoderUnsupportedResolutions);
