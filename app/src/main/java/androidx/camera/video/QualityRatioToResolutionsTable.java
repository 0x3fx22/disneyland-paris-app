...
class QualityRatioToResolutionsTable {
    ...
    static {
        ...
        map.put(Quality.f29SD, Range.create(241, Integer.valueOf(AdaptiveTrackSelection.DEFAULT_MAX_HEIGHT_TO_DISCARD)));
    ...
    QualityRatioToResolutionsTable(List list, Map map) {
        for (Quality quality : sQualityRangeMap.keySet()) {
            this.mTable.put(QualityRatio.m76of(quality, -1), new ArrayList());
            Iterator it = sAspectRatioMap.keySet().iterator();
            while (it.hasNext()) {
                this.mTable.put(QualityRatio.m76of(quality, ((Integer) it.next()).intValue()), new ArrayList());
            }
        }
    ...
    private List getQualityRatioRow(Quality quality, int i) {
        return (List) this.mTable.get(QualityRatio.m76of(quality, i));
    }
    ...
    static abstract class QualityRatio {
        ...
        static QualityRatio m76of(Quality quality, int i) {
