...
public class VideoEncoderConfigVideoProfileResolver implements Supplier<VideoEncoderConfig> {
    ...
    public VideoEncoderConfig get() {
        ...
        Logger.m28d("VidEncVdPrflRslvr", "Resolved VIDEO frame rate: " + iResolveFrameRate + "fps");
        Range<Integer> bitrate = this.mVideoSpec.getBitrate();
        Logger.m28d("VidEncVdPrflRslvr", "Using resolved VIDEO bitrate from EncoderProfiles");
    ...
    private int resolveFrameRate() {
        ...
        Logger.m28d("VidEncVdPrflRslvr", String.format("Resolved frame rate %dfps [Video profile frame rate: %dfps, Expected operating range: %s]", Integer.valueOf(iIntValue), Integer.valueOf(frameRate), Objects.equals(this.mExpectedFrameRateRange, range2) ? this.mExpectedFrameRateRange : "<UNSPECIFIED>"));
        return iIntValue;
    }
}
