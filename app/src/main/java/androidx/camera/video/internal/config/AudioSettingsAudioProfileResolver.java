...
public final class AudioSettingsAudioProfileResolver implements Supplier<AudioSettings> {
    ...
    public AudioSettings get() {
        ...
        if (channelCount == -1) {
            Logger.m28d("AudioSrcAdPrflRslvr", "Resolved AUDIO channel count from AudioProfile: " + channels);
            channelCount = channels;
        } else {
            Logger.m28d("AudioSrcAdPrflRslvr", "Media spec AUDIO channel count overrides AudioProfile [AudioProfile channel count: " + channels + ", Resolved Channel Count: " + channelCount + "]");
        }
        ...
        Logger.m28d("AudioSrcAdPrflRslvr", "Using resolved AUDIO sample rate or nearest supported from AudioProfile: " + iSelectSampleRateOrNearestSupported + "Hz. [AudioProfile sample rate: " + sampleRate2 + "Hz]");
