...
public final class AudioSource {
    ...
    public AudioSource(@NonNull AudioSettings audioSettings, @NonNull Executor executor, @Nullable Context context) throws AudioSourceAccessException {
        ...
        }, C0740C.DEFAULT_MAX_SEEK_TO_PREVIOUS_POSITION_MS);
    }
    ...
    public /* synthetic */ void lambda$stop$3() {
        ...
        if (iOrdinal == 1) {
            ...
            Logger.m34w("AudioSource", "AudioSource is released. Calling stop() is a no-op.");
        }
    }
    ...
    private void resetBufferProvider(final BufferProvider bufferProvider) {
            ...
            this.mStateObserver = new Observable.Observer() { // from class: androidx.camera.video.internal.audio.AudioSource.1
                ...
                public void onNewData(BufferProvider.State state) {
                    ...
                    if (AudioSource.this.mBufferProvider == bufferProvider) {
                        Logger.m28d("AudioSource", "Receive BufferProvider state change: " + AudioSource.this.mBufferProviderState + " to " + state);
                        AudioSource audioSource = AudioSource.this;
            ...
            this.mAcquireBufferCallback = new FutureCallback() { // from class: androidx.camera.video.internal.audio.AudioSource.2
                ...
                public void onSuccess(InputBuffer inputBuffer) {
                    ...
                    if (packetInfo.getSizeInBytes() > 0) {
                        ...
                        Logger.m34w("AudioSource", "Unable to read data from AudioStream.");
                        inputBuffer.cancel();
                    }
                ...
                public void onFailure(Throwable th) {
                    ...
                    Logger.m28d("AudioSource", "Unable to get input buffer, the BufferProvider could be transitioning to INACTIVE state.");
    ...
    void retryStartAudioStream() {
        ...
        try {
            ...
            Logger.m28d("AudioSource", "Retry start AudioStream succeed");
            this.mSilentAudioStream.stop();
            this.mInSilentStartState = false;
        } catch (AudioStream.AudioStreamException e) {
            Logger.m35w("AudioSource", "Retry start AudioStream failed", e);
    ...
    private void startSendingAudio() {
        ...
        try {
            Logger.m28d("AudioSource", "startSendingAudio");
            this.mAudioStream.start();
            this.mInSilentStartState = false;
        } catch (AudioStream.AudioStreamException e) {
            Logger.m35w("AudioSource", "Failed to start AudioStream", e);
    ...
    private void stopSendingAudio() {
        if (this.mIsSendingAudio) {
            ...
            Logger.m28d("AudioSource", "stopSendingAudio");
    ...
    void setState(InternalState internalState) {
        Logger.m28d("AudioSource", "Transitioning internal state: " + this.mState + " --> " + internalState);
