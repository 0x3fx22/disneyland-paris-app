...
public final class SharedByteBuffer implements Closeable {
    ...
    protected void finalize() throws Throwable {
        try {
            if (closeInternal()) {
                Logger.m34w("SharedByteBuffer", String.format(Locale.US, "SharedByteBuffer closed by finalizer, but should have been closed manually with SharedByteBuffer.close() [%s]", toString()));
            }
        } finally {
            super.finalize();
        }
        ...
        synchronized (this.mCloseLock) {
            try {
                ...
                if (Logger.isDebugEnabled("SharedByteBuffer")) {
                    ...
                    Logger.m28d("SharedByteBuffer", String.format(Locale.US, "Ref count decremented: %d [%s]", Integer.valueOf(iDecrementAndGet), toString()));
                }
                if (iDecrementAndGet == 0) {
                    if (Logger.isDebugEnabled("SharedByteBuffer")) {
                        Logger.m28d("SharedByteBuffer", String.format(Locale.US, "Final reference released. Running final close action. [%s]", toString()));
                    }
                    try {
                        ...
                        Logger.m31e("SharedByteBuffer", String.format(Locale.US, "Unable to execute final close action. [%s]", toString()), e);
                    }
                }
                return true;
            } catch (Throwable th) {
