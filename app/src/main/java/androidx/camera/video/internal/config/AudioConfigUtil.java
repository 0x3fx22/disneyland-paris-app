...
public final class AudioConfigUtil {
    ...
    public static AudioMimeInfo resolveAudioMimeInfo(@NonNull MediaSpec mediaSpec, @Nullable VideoValidatedEncoderProfilesProxy videoValidatedEncoderProfilesProxy) {
        ...
        if (videoValidatedEncoderProfilesProxy == null || videoValidatedEncoderProfilesProxy.getDefaultAudioProfile() == null) {
            ...
            if (Objects.equals(mediaType, EncoderProfilesProxy.AudioProfileProxy.MEDIA_TYPE_NONE)) {
                Logger.m28d("AudioConfigUtil", "EncoderProfiles contains undefined AUDIO mime type so cannot be used. May rely on fallback defaults to derive settings [chosen mime type: " + strOutputFormatToAudioMime + "(profile: " + iOutputFormatToAudioProfile + ")]");
            } else if (mediaSpec.getOutputFormat() == -1) {
                Logger.m28d("AudioConfigUtil", "MediaSpec contains OUTPUT_FORMAT_AUTO. Using EncoderProfiles to derive AUDIO settings [mime type: " + mediaType + "(profile: " + profile + ")]");
                ...
                Logger.m28d("AudioConfigUtil", "MediaSpec audio mime/profile matches EncoderProfiles. Using EncoderProfiles to derive AUDIO settings [mime type: " + mediaType + "(profile: " + iOutputFormatToAudioProfile + ")]");
                strOutputFormatToAudioMime = mediaType;
            } else {
                Logger.m28d("AudioConfigUtil", "MediaSpec audio mime or profile does not match EncoderProfiles, so EncoderProfiles settings cannot be used. May rely on fallback defaults to derive AUDIO settings [EncoderProfiles mime type: " + mediaType + "(profile: " + profile + "), chosen mime type: " + strOutputFormatToAudioMime + "(profile: " + iOutputFormatToAudioProfile + ")]");
            }
            defaultAudioProfile = null;
        }
    ...
    static int resolveAudioSource(AudioSpec audioSpec) {
        ...
        if (source == -1) {
            Logger.m28d("AudioConfigUtil", "Using default AUDIO source: 5");
            return 5;
        }
        Logger.m28d("AudioConfigUtil", "Using provided AUDIO source: " + source);
        return source;
    }
    ...
    static int resolveAudioSourceFormat(AudioSpec audioSpec) {
        ...
        if (sourceFormat == -1) {
            Logger.m28d("AudioConfigUtil", "Using default AUDIO source format: 2");
            return 2;
        }
        Logger.m28d("AudioConfigUtil", "Using provided AUDIO source format: " + sourceFormat);
        return sourceFormat;
    }
    ...
    static int selectSampleRateOrNearestSupported(Range range, int i, int i2, final int i3) {
        ...
        while (true) {
            if (range.contains(Integer.valueOf(iIntValue))) {
                ...
                Logger.m28d("AudioConfigUtil", "Sample rate " + iIntValue + "Hz is not supported by audio source with channel count " + i + " and source format " + i2);
            } else {
                Logger.m28d("AudioConfigUtil", "Sample rate " + iIntValue + "Hz is not in target range " + range);
            }
            if (arrayList == null) {
                Logger.m28d("AudioConfigUtil", "Trying common sample rates in proximity order to target " + i3 + "Hz");
            ...
            if (i4 < arrayList.size()) {
                ...
                Logger.m28d("AudioConfigUtil", "No sample rate found in target range or supported by audio source. Falling back to default sample rate of 44100Hz");
                return 44100;
            }
        }
    }
    ...
    static int scaleAndClampBitrate(int i, int i2, int i3, int i4, int i5, Range range) {
        ...
        Logger.m28d("AudioConfigUtil", str);
        return iDoubleValue;
    }
}
