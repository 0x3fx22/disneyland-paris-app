...
public final class AudioSettingsDefaultResolver implements Supplier<AudioSettings> {
    ...
    public AudioSettings get() {
        ...
        if (channelCount == -1) {
            Logger.m28d("DefAudioResolver", "Using fallback AUDIO channel count: 1");
            channelCount = 1;
        } else {
            Logger.m28d("DefAudioResolver", "Using supplied AUDIO channel count: " + channelCount);
        }
        ...
        if (AudioSpec.SAMPLE_RATE_RANGE_AUTO.equals(sampleRate)) {
            ...
            sb.append(44100);
            sb.append("Hz");
            Logger.m28d("DefAudioResolver", sb.toString());
        } else {
            ...
            Logger.m28d("DefAudioResolver", "Using AUDIO sample rate resolved from AudioSpec: " + iSelectSampleRateOrNearestSupported + "Hz");
        }
