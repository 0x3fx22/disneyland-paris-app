...
public abstract class CameraController {
    ...
    private void setTargetOutputSize(ImageOutputConfig.Builder builder, OutputSize outputSize) {
        ...
        Logger.m30e("CameraController", "Invalid target surface size. " + outputSize);
    }
    ...
    public void updateScreenFlashToImageCapture() {
        ...
        if (screenFlashUiInfoByPriority == null) {
            Logger.m28d("CameraController", "No ScreenFlash instance set yet, need to wait for controller to be set to either ScreenFlashView or PreviewView");
        ...
        Logger.m28d("CameraController", "Set ScreenFlash instance to ImageCapture, provided by " + screenFlashUiInfoByPriority.getProviderType().name());
    }
    ...
    private Consumer wrapListenerToDeactivateRecordingOnFinalized(Consumer consumer) {
        return new C03392(ContextCompat.getMainExecutor(this.mAppContext), consumer);
    }

    /* JADX INFO: renamed from: androidx.camera.view.CameraController$2 */
    class C03392 implements Consumer {
        final /* synthetic */ Consumer val$listener;
        final /* synthetic */ Executor val$mainExecutor;

        C03392(Executor executor, Consumer consumer) {
    ...
    public boolean hasCamera(@NonNull CameraSelector cameraSelector) {
        ...
        try {
            ...
            Logger.m35w("CameraController", "Failed to check camera availability", e);
            return false;
        }
    }
    ...
    void onPinchToZoom(float f) {
        if (!isCameraAttached()) {
            Logger.m34w("CameraController", "Use cases not attached to camera.");
            return;
        }
        if (!this.mPinchToZoomEnabled) {
            Logger.m28d("CameraController", "Pinch to zoom disabled.");
            return;
        }
        Logger.m28d("CameraController", "Pinch to zoom with scale: " + f);
    ...
    void onTapToFocus(MeteringPointFactory meteringPointFactory, float f, float f2) {
        ...
        if (!this.mTapToFocusEnabled) {
            Logger.m28d("CameraController", "Tap to focus disabled. ");
            return;
        }
        Logger.m28d("CameraController", "Tap to focus started: " + f + ", " + f2);
        ...
        Futures.addCallback(this.mCamera.getCameraControl().startFocusAndMetering(new FocusMeteringAction.Builder(meteringPointFactory.createPoint(f, f2, 0.16666667f), 1).addPoint(meteringPointFactory.createPoint(f, f2, 0.25f), 2).build()), new FutureCallback() { // from class: androidx.camera.view.CameraController.3
            ...
            public void onSuccess(FocusMeteringResult focusMeteringResult) {
                ...
                Logger.m28d("CameraController", "Tap to focus onSuccess: " + focusMeteringResult.isFocusSuccessful());
            ...
            public void onFailure(Throwable th) {
                if (th instanceof CameraControl.OperationCanceledException) {
                    Logger.m28d("CameraController", "Tap-to-focus is canceled by new action.");
                } else {
                    Logger.m29d("CameraController", "Tap to focus failed.", th);
    ...
    @NonNull
    @MainThread
    public ListenableFuture<Void> setLinearZoom(@FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 1.0d) float f) {
        Threads.checkMainThread();
        if (!isCameraAttached()) {
    ...
    void startCameraAndTrackStates(Runnable runnable) {
        try {
            ...
            if (!isCameraAttached()) {
                Logger.m28d("CameraController", "Use cases not attached to camera.");
                return;
            }
    ...
    protected UseCaseGroup createUseCaseGroup() {
        if (!isCameraInitialized()) {
            Logger.m28d("CameraController", "Camera not initialized.");
            return null;
        }
        if (!isPreviewViewAttached()) {
            Logger.m28d("CameraController", "PreviewView not attached to CameraController.");
            return null;
        }
