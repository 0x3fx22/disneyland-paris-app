...
final class Camera2CameraImpl implements CameraInternal {
    ...
    private void addOrRemoveMeteringRepeatingUseCase() {
        ...
        if (repeatingCaptureConfig.getSurfaces().isEmpty()) {
            ...
            if (isSurfaceCombinationWithMeteringRepeatingSupported()) {
                ...
                Logger.m30e("Camera2CameraImpl", "Failed to add a repeating surface, CameraControl and ImageCapture may encounter issues due to the absence of repeating surface. Please add a UseCase (Preview or ImageAnalysis) that can provide a repeating surface for CameraControl and ImageCapture to function properly.");
                return;
            }
        }
        if (size2 == 1 && size == 1) {
        ...
        Logger.m28d("Camera2CameraImpl", "No need to remove a previous mMeteringRepeating, SessionConfig Surfaces: " + size2 + ", CaptureConfig Surfaces: " + size);
    }
        ...
        for (UseCaseAttachState.UseCaseAttachInfo useCaseAttachInfo : this.mUseCaseAttachState.getAttachedUseCaseInfo()) {
            if (useCaseAttachInfo.getCaptureTypes() == null || useCaseAttachInfo.getCaptureTypes().get(0) != UseCaseConfigFactory.CaptureType.METERING_REPEATING) {
                if (useCaseAttachInfo.getStreamSpec() == null || useCaseAttachInfo.getCaptureTypes() == null) {
                    Logger.m34w("Camera2CameraImpl", "Invalid stream spec or capture types in " + useCaseAttachInfo);
                    return false;
                }
    ...
    void openCaptureSession() {
        ...
        Futures.addCallback(captureSessionInterface.open(attachedBuilder.build(), (CameraDevice) Preconditions.checkNotNull(this.mCameraDevice), this.mCaptureSessionOpenerBuilder.build()), new FutureCallback() { // from class: androidx.camera.camera2.internal.Camera2CameraImpl.4
            ...
            public void onFailure(Throwable th) {
                ...
                Logger.m31e("Camera2CameraImpl", "Unable to configure camera " + Camera2CameraImpl.this, th);
                Camera2CameraImpl camera2CameraImpl = Camera2CameraImpl.this;
    ...
    private boolean checkAndAttachRepeatingSurface(CaptureConfig.Builder builder) {
        if (!builder.getSurfaces().isEmpty()) {
            Logger.m34w("Camera2CameraImpl", "The capture config builder already has surface inside.");
            return false;
        }
        ...
        Logger.m34w("Camera2CameraImpl", "Unable to find a repeating surface to attach to CaptureConfig");
        return false;
    }
    ...
    private void debugLog(String str, Throwable th) {
        Logger.m29d("Camera2CameraImpl", String.format("{%s} %s", toString(), str), th);
    }
    ...
    final class StateCallback extends CameraDevice.StateCallback {
        ...
        public void onError(CameraDevice cameraDevice, int i) {
            ...
            if (iOrdinal != 1) {
                switch (iOrdinal) {
                    ...
                    case 8:
                    case 9:
                        Logger.m28d("Camera2CameraImpl", String.format("CameraDevice.onError(): %s failed with %s while in %s state. Will attempt recovering from error.", cameraDevice.getId(), Camera2CameraImpl.getErrorMessage(i), Camera2CameraImpl.this.mState.name()));
            ...
            Logger.m30e("Camera2CameraImpl", String.format("CameraDevice.onError(): %s failed with %s while in %s state. Will finish closing camera.", cameraDevice.getId(), Camera2CameraImpl.getErrorMessage(i), Camera2CameraImpl.this.mState.name()));
        ...
        private void handleErrorOnOpen(CameraDevice cameraDevice, int i) {
            ...
            if (i == 1 || i == 2 || i == 4) {
                Logger.m28d("Camera2CameraImpl", String.format("Attempt to reopen camera[%s] after error[%s]", cameraDevice.getId(), Camera2CameraImpl.getErrorMessage(i)));
                reopenCameraAfterError(i);
                return;
            }
            Logger.m30e("Camera2CameraImpl", "Error observed on open (or opening) camera device " + cameraDevice.getId() + ": " + Camera2CameraImpl.getErrorMessage(i) + " closing camera.");
        ...
        void scheduleCameraReopen() {
            ...
            Logger.m30e("Camera2CameraImpl", "Camera reopening attempted for " + this.mCameraReopenMonitor.getReopenLimitMs() + "ms without success.");
