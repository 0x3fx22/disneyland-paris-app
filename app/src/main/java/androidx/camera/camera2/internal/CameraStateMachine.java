...
class CameraStateMachine {
    ...
    public void updateState(CameraInternal.State state, CameraState.StateError stateError) {
        ...
        switch (C02011.$SwitchMap$androidx$camera$core$impl$CameraInternal$State[state.ordinal()]) {
            case 1:
        ...
        Logger.m28d("CameraStateMachine", "New public camera state " + cameraStateOnCameraPendingOpen + " from " + state + " and " + stateError);
        if (Objects.equals((CameraState) this.mCameraStates.getValue(), cameraStateOnCameraPendingOpen)) {
            return;
        }
        Logger.m28d("CameraStateMachine", "Publishing new public camera state " + cameraStateOnCameraPendingOpen);
