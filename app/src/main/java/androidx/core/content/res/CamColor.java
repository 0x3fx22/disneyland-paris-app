...
public class CamColor {
    ...
    private final float f81mJ;
    private final float mJstar;

    /* JADX INFO: renamed from: mM */
    private final float f82mM;

    /* JADX INFO: renamed from: mQ */
    private final float f83mQ;

    /* JADX INFO: renamed from: mS */
    private final float f84mS;
    ...
    public static int toColor(@FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 360.0d) float f, @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = DoubleCompanionObject.POSITIVE_INFINITY, toInclusive = false) float f2, @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 100.0d) float f3) {
    ...
    int viewed(ViewingConditions viewingConditions) {
        float fPow = (float) Math.pow(((double) ((((double) getChroma()) == AudioStats.AUDIO_AMPLITUDE_NONE || ((double) getJ()) == AudioStats.AUDIO_AMPLITUDE_NONE) ? BitmapDescriptorFactory.HUE_RED : getChroma() / ((float) Math.sqrt(((double) getJ()) / 100.0d)))) / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d), 1.1111111111111112d);
        ...
        float fSignum3 = Math.signum(f7) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((float) Math.max(AudioStats.AUDIO_AMPLITUDE_NONE, (((double) Math.abs(f7)) * 27.13d) / (400.0d - ((double) Math.abs(f7)))), 2.380952380952381d));
