...
public final class WindowInsetsAnimationCompat {
    ...
    public void setFraction(@FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 1.0d) float f) {
    ...
    public void setAlpha(@FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 1.0d) float f) {
    ...
    private static class Impl21 extends Impl {
        ...
        static BoundsCompat computeAnimationBounds(WindowInsetsCompat windowInsetsCompat, WindowInsetsCompat windowInsetsCompat2, int i) {
            ...
            return new BoundsCompat(Insets.m101of(Math.min(insets.left, insets2.left), Math.min(insets.top, insets2.top), Math.min(insets.right, insets2.right), Math.min(insets.bottom, insets2.bottom)), Insets.m101of(Math.max(insets.left, insets2.left), Math.max(insets.top, insets2.top), Math.max(insets.right, insets2.right), Math.max(insets.bottom, insets2.bottom)));
        }
    ...
    private static class Impl30 extends Impl {
        ...
        Impl30(int i, Interpolator interpolator, long j) {
            this(AbstractC0520x7721ea9a.m162m(i, interpolator, j));
        }
        ...
        private static class ProxyCallback extends WindowInsetsAnimation$Callback {
            ...
            public WindowInsets onProgress(WindowInsets windowInsets, List list) {
                ...
                for (int size = list.size() - 1; size >= 0; size--) {
                    WindowInsetsAnimation windowInsetsAnimationM173m = AbstractC0531x30864cc7.m173m(list.get(size));
                    WindowInsetsAnimationCompat windowInsetsAnimationCompat = getWindowInsetsAnimationCompat(windowInsetsAnimationM173m);
                    windowInsetsAnimationCompat.setFraction(windowInsetsAnimationM173m.getFraction());
        ...
        public static WindowInsetsAnimation.Bounds createPlatformBounds(BoundsCompat boundsCompat) {
            AbstractC0522x7721ea9c.m164m();
            return AbstractC0521x7721ea9b.m163m(boundsCompat.getLowerBound().toPlatformInsets(), boundsCompat.getUpperBound().toPlatformInsets());
        }
