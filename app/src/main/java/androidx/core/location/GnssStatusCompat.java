...
public abstract class GnssStatusCompat {
    ...
    @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 360.0d)
    public abstract float getAzimuthDegrees(@IntRange(from = 0) int i);

    @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 63.0d)
    ...
    @FloatRange(from = RegionEvent.MIN_LATITUDE, m3to = RegionEvent.MAX_LATITUDE)
    public abstract float getElevationDegrees(@IntRange(from = 0) int i);

    @IntRange(from = 0)
    public abstract int getSatelliteCount();

    @IntRange(from = 1, m4to = 200)
