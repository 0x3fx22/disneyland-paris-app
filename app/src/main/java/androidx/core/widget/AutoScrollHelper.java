...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long j) {
            ...
            if (j3 < 0 || j < j3) {
                return AutoScrollHelper.constrain((j - j2) / this.mRampUpDuration, BitmapDescriptorFactory.HUE_RED, 1.0f) * 0.5f;
            }
            float f = this.mStopValue;
            return (1.0f - f) + (f * AutoScrollHelper.constrain((j - j3) / this.mEffectiveRampDown, BitmapDescriptorFactory.HUE_RED, 1.0f));
        }
