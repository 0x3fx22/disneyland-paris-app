...
public final class ViewFinderImpl implements ViewFinder {
    ...
    public View getView() throws NoMatchingViewException, AmbiguousViewMatcherException {
        ...
        throw new NoMatchingViewException.Builder().withViewMatcher(this.viewMatcher).withRootView(view).withAdapterViews(arrayListNewArrayList).withAdapterViewWarning(EspressoOptional.m396of(String.format(Locale.ROOT, "\nIf the target view is not part of the view hierarchy, you may need to use Espresso.onData to load it from one of the following AdapterViews:%s", Joiner.m384on("\n- ").join(arrayListNewArrayList)))).build();
    }
}
