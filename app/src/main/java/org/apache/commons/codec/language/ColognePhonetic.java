...
public class ColognePhonetic implements StringEncoder {
    ...
    private static final char[] f3972SZ = {'S', Matrix.MATRIX_TYPE_ZERO};
            ...
            if (cRemoveNext != 'H' && cRemoveNext >= 'A' && cRemoveNext <= 'Z') {
                if (arrayContains(AEIJOUY, cRemoveNext)) {
                    ...
                    if (cRemoveNext == 'X' && !arrayContains(CKQ, c3)) {
                        ...
                        if (cRemoveNext == 'C') {
                            if (c2 != '/' ? arrayContains(f3972SZ, c3) || !arrayContains(AHOUKQX, nextChar) : !arrayContains(AHKLOQRUX, nextChar)) {
                            }
