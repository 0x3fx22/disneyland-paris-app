...
public class KDFFeedbackBytesGenerator implements MacDerivationFunction {
    ...
    public KDFFeedbackBytesGenerator(Mac mac) {
        ...
        this.f4436h = macSize;
        this.f4438k = new byte[macSize];
    }

    private void generateNext() {
        if (this.generatedBytes == 0) {
            Mac mac = this.prf;
            byte[] bArr = this.f4437iv;
            mac.update(bArr, 0, bArr.length);
        } else {
            Mac mac2 = this.prf;
            byte[] bArr2 = this.f4438k;
            mac2.update(bArr2, 0, bArr2.length);
        }
        if (this.useCounter) {
            int i = (this.generatedBytes / this.f4436h) + 1;
            byte[] bArr3 = this.ios;
        ...
        this.prf.doFinal(this.f4438k, 0);
    }
    ...
    public int generateBytes(byte[] bArr, int i, int i2) throws DataLengthException, IllegalArgumentException {
        ...
        System.arraycopy(this.f4438k, i7, bArr, i, iMin);
        ...
        while (true) {
            ...
            generateNext();
            iMin = Math.min(this.f4436h, i8);
            System.arraycopy(this.f4438k, 0, bArr, i, iMin);
            this.generatedBytes += iMin;
            i8 -= iMin;
        }
    }
    ...
    public void init(DerivationParameters derivationParameters) {
        ...
        if (kDFFeedbackParameters.useCounter()) {
            BigInteger bigIntegerMultiply = TWO.pow(r).multiply(BigInteger.valueOf(this.f4436h));
            if (bigIntegerMultiply.compareTo(INTEGER_MAX) != 1) {
                iIntValue = bigIntegerMultiply.intValue();
            }
        }
        this.maxSizeExcl = iIntValue;
        this.f4437iv = kDFFeedbackParameters.getIV();
