...
public class KGCMBlockCipher implements AEADBlockCipher {
    ...
    public KGCMBlockCipher(BlockCipher blockCipher) {
        ...
        this.f4504iv = new byte[blockSize];
        this.multiplier = createDefaultMultiplier(blockSize);
        this.f4503b = new long[blockSize >>> 3];
        this.macBlock = null;
    }
    ...
    private void calculateMac(byte[] bArr, int i, int i2, int i3) {
        int i4 = i + i2;
        while (i < i4) {
            xorWithInput(this.f4503b, bArr, i);
            this.multiplier.multiplyH(this.f4503b);
            i += this.blockSize;
        }
    ...
    public void init(boolean z, CipherParameters cipherParameters) throws IOException, IllegalArgumentException {
        ...
        if (cipherParameters instanceof AEADParameters) {
            ...
            byte[] bArr = this.f4504iv;
            int length = bArr.length - nonce.length;
            Arrays.fill(bArr, (byte) 0);
            System.arraycopy(nonce, 0, this.f4504iv, length, nonce.length);
            ...
            byte[] bArr3 = this.f4504iv;
            int length2 = bArr3.length - iv.length;
            Arrays.fill(bArr3, (byte) 0);
            System.arraycopy(iv, 0, this.f4504iv, length2, iv.length);
            this.initialAssociatedText = null;
            this.macSize = this.blockSize;
            key = (KeyParameter) parametersWithIV.getParameters();
        }
        this.macBlock = new byte[this.blockSize];
        this.ctrEngine.init(true, new ParametersWithIV(key, this.f4504iv));
