...
public class SM2KeyExchange {
    ...
    private byte[] m1993S1(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    private byte[] m1994S2(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    private BigInteger reduce(BigInteger bigInteger) {
        return bigInteger.and(BigInteger.valueOf(1L).shiftLeft(this.f4129w).subtract(BigInteger.valueOf(1L))).setBit(this.f4129w);
    }
    ...
    public byte[][] calculateKeyWithConfirmation(int i, byte[] bArr, CipherParameters cipherParameters) {
        ...
        if (!this.initiator) {
            ...
            return new byte[][]{bArrKdf, m1993S1(this.digest, eCPointCalculateU, bArrCalculateInnerHash), m1994S2(this.digest, eCPointCalculateU, bArrCalculateInnerHash)};
        }
        ...
        if (Arrays.constantTimeAreEqual(m1993S1(this.digest, eCPointCalculateU, bArrCalculateInnerHash2), bArr)) {
            return new byte[][]{bArrKdf2, m1994S2(this.digest, eCPointCalculateU, bArrCalculateInnerHash2)};
        }
    ...
    public void init(CipherParameters cipherParameters) {
        ...
        this.f4129w = (this.ecParams.getCurve().getFieldSize() / 2) - 1;
    }
}
