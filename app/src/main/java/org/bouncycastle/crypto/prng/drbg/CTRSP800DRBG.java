...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private void CTR_DRBG_Reseed_algorithm(byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), bArr), this._seedLength), this._Key, this.f4598_V);
        this._reseedCounter = 1L;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArrBlock_Cipher_df, this._Key, this.f4598_V);
        } else {
            bArrBlock_Cipher_df = new byte[this._seedLength / 8];
        }
        int length = this.f4598_V.length;
        byte[] bArr3 = new byte[length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i = 0; i <= bArr.length / length; i++) {
            int i2 = i * length;
            int length2 = bArr.length - i2 > length ? length : bArr.length - (this.f4598_V.length * i);
            if (length2 != 0) {
                addOneTo(this.f4598_V);
                this._engine.processBlock(this.f4598_V, 0, bArr3, 0);
