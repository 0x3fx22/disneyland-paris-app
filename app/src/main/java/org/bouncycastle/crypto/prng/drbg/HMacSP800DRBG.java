...
public class HMacSP800DRBG implements SP80090DRBG {
    ...
    private byte[] f4604_K;

    /* JADX INFO: renamed from: _V */
    private byte[] f4605_V;
    ...
    private void hmac_DRBG_Update_Func(byte[] bArr, byte b) {
        this._hMac.init(new KeyParameter(this.f4604_K));
        Mac mac = this._hMac;
        byte[] bArr2 = this.f4605_V;
        mac.update(bArr2, 0, bArr2.length);
        this._hMac.update(b);
        if (bArr != null) {
            this._hMac.update(bArr, 0, bArr.length);
        }
        this._hMac.doFinal(this.f4604_K, 0);
        this._hMac.init(new KeyParameter(this.f4604_K));
        Mac mac2 = this._hMac;
        byte[] bArr3 = this.f4605_V;
        mac2.update(bArr3, 0, bArr3.length);
        this._hMac.doFinal(this.f4605_V, 0);
    }
