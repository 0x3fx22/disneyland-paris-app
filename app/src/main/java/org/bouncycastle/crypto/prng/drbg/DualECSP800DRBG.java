...
import org.bouncycastle.math.p183ec.ECMultiplier;
import org.bouncycastle.math.p183ec.ECPoint;
import org.bouncycastle.math.p183ec.FixedPointCombMultiplier;
...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    private ECPoint f4601_P;

    /* JADX INFO: renamed from: _Q */
    private ECPoint f4602_Q;
    ...
    static {
        ...
        nistPoints = new DualECPoints[]{new DualECPoints(128, c7835Fp.createPoint(bigInteger, bigInteger2), c7835Fp.createPoint(bigInteger3, bigInteger4), 1), new DualECPoints(192, c7835Fp.createPoint(bigInteger5, bigInteger6), c7835Fp.createPoint(bigInteger7, bigInteger8), 1), new DualECPoints(256, c7835Fp.createPoint(bigInteger9, bigInteger10), c7835Fp.createPoint(bigInteger11, bigInteger12), 1)};
        ECCurve.C7835Fp c7835Fp = (ECCurve.C7835Fp) NISTNamedCurves.getByName("P-256").getCurve();
        ECCurve.C7835Fp c7835Fp2 = (ECCurve.C7835Fp) NISTNamedCurves.getByName("P-384").getCurve();
        ECCurve.C7835Fp c7835Fp3 = (ECCurve.C7835Fp) NISTNamedCurves.getByName("P-521").getCurve();
    }
    ...
    public DualECSP800DRBG(DualECPoints[] dualECPointsArr, Digest digest, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        for (int i2 = 0; i2 != dualECPointsArr.length; i2++) {
            if (i <= dualECPointsArr[i2].getSecurityStrength()) {
                ...
                this.f4601_P = dualECPointsArr[i2].getP();
                this.f4602_Q = dualECPointsArr[i2].getQ();
                break;
            }
        }
        if (this.f4601_P == null) {
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        BigInteger bigInteger = bArr2 != null ? new BigInteger(1, xor(this.f4603_s, Utils.hash_df(this._digest, bArr2, this._seedlen))) : new BigInteger(1, this.f4603_s);
        int length3 = 0;
        Arrays.fill(bArr, (byte) 0);
        int length4 = 0;
        for (int i = 0; i < length2; i++) {
            bigInteger = getScalarMultipleXCoord(this.f4601_P, bigInteger);
            byte[] byteArray = getScalarMultipleXCoord(this.f4602_Q, bigInteger).toByteArray();
        ...
        if (length4 < bArr.length) {
            bigInteger = getScalarMultipleXCoord(this.f4601_P, bigInteger);
            byte[] byteArray2 = getScalarMultipleXCoord(this.f4602_Q, bigInteger).toByteArray();
        ...
        this.f4603_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f4601_P, bigInteger));
        return length;
    }
    ...
    public void reseed(byte[] bArr) {
        ...
        this.f4603_s = Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f4603_s, this._seedlen), getEntropy(), bArr), this._seedlen);
        this._reseedCounter = 0L;
    }
}
