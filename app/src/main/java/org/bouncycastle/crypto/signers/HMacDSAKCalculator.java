...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    private final byte[] f4608K;

    /* JADX INFO: renamed from: V */
    private final byte[] f4609V;
    private final HMac hMac;

    /* JADX INFO: renamed from: n */
    private BigInteger f4610n;

    public HMacDSAKCalculator(Digest digest) {
        HMac hMac = new HMac(digest);
        this.hMac = hMac;
        this.f4609V = new byte[hMac.getMacSize()];
        this.f4608K = new byte[hMac.getMacSize()];
    }

    private BigInteger bitsToInt(byte[] bArr) {
        BigInteger bigInteger = new BigInteger(1, bArr);
        return bArr.length * 8 > this.f4610n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f4610n.bitLength()) : bigInteger;
    }

    @Override // org.bouncycastle.crypto.signers.DSAKCalculator
    public void init(BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr) {
        this.f4610n = bigInteger;
        Arrays.fill(this.f4609V, (byte) 1);
        Arrays.fill(this.f4608K, (byte) 0);
        ...
        this.hMac.init(new KeyParameter(this.f4608K));
        HMac hMac = this.hMac;
        byte[] bArr4 = this.f4609V;
        ...
        this.hMac.doFinal(this.f4608K, 0);
        this.hMac.init(new KeyParameter(this.f4608K));
        HMac hMac2 = this.hMac;
        byte[] bArr5 = this.f4609V;
        hMac2.update(bArr5, 0, bArr5.length);
        this.hMac.doFinal(this.f4609V, 0);
        HMac hMac3 = this.hMac;
        byte[] bArr6 = this.f4609V;
    ...
    public BigInteger nextK() {
        int unsignedByteLength = BigIntegers.getUnsignedByteLength(this.f4610n);
        byte[] bArr = new byte[unsignedByteLength];
        while (true) {
            int i = 0;
            while (i < unsignedByteLength) {
                HMac hMac = this.hMac;
                byte[] bArr2 = this.f4609V;
                hMac.update(bArr2, 0, bArr2.length);
                this.hMac.doFinal(this.f4609V, 0);
                int iMin = Math.min(unsignedByteLength - i, this.f4609V.length);
                System.arraycopy(this.f4609V, 0, bArr, i, iMin);
                i += iMin;
            }
            BigInteger bigIntegerBitsToInt = bitsToInt(bArr);
            if (bigIntegerBitsToInt.compareTo(ZERO) > 0 && bigIntegerBitsToInt.compareTo(this.f4610n) < 0) {
                return bigIntegerBitsToInt;
            }
            HMac hMac2 = this.hMac;
            byte[] bArr3 = this.f4609V;
