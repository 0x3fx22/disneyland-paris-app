...
public class IESEngine {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        byte[] bArr3;
        int iProcessBytes;
        if (i2 < this.f4374V.length + this.mac.getMacSize()) {
            throw new InvalidCipherTextException("Length of input must be greater than the MAC and V combined");
        }
        if (this.cipher == null) {
            int length = (i2 - this.f4374V.length) - this.mac.getMacSize();
            ...
            for (int i4 = 0; i4 != length; i4++) {
                bArr3[i4] = (byte) (bArr[(this.f4374V.length + i) + i4] ^ bArr4[i4]);
            }
            iProcessBytes = 0;
        } else {
    ...
    private byte[] encryptBlock(byte[] bArr, int i, int i2) {
        ...
        if (this.cipher == null) {
            ...
            if (this.f4373IV != null) {
                bufferedBlockCipher = this.cipher;
                keyParameter = new ParametersWithIV(new KeyParameter(bArr6), this.f4373IV);
            } else {
                bufferedBlockCipher = this.cipher;
        ...
        byte[] bArr10 = this.f4374V;
        byte[] bArr11 = new byte[bArr10.length + i2 + macSize];
        System.arraycopy(bArr10, 0, bArr11, 0, bArr10.length);
        System.arraycopy(bArr2, 0, bArr11, this.f4374V.length, i2);
        System.arraycopy(bArr9, 0, bArr11, this.f4374V.length + i2, macSize);
        return bArr11;
    }
    ...
    private void extractParams(CipherParameters cipherParameters) {
        if (cipherParameters instanceof ParametersWithIV) {
            ...
            this.f4373IV = parametersWithIV.getIV();
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.forEncryption) {
            ...
            if (ephemeralKeyPairGenerator != null) {
                ...
                this.f4374V = ephemeralKeyPairGenerate.getEncodedPublicKey();
            }
        } else if (this.keyParser != null) {
            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bArr, i, i2);
            try {
                this.pubParam = this.keyParser.readKey(byteArrayInputStream);
                this.f4374V = Arrays.copyOfRange(bArr, i, (i2 - byteArrayInputStream.available()) + i);
            } catch (IOException e) {
