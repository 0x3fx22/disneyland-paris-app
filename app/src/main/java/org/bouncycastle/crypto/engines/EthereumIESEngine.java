...
public class EthereumIESEngine {
    ...
    public static class HandshakeKDFFunction implements DigestDerivationFunction {
        ...
        public void init(DerivationParameters derivationParameters) {
            if (derivationParameters instanceof KDFParameters) {
                ...
                this.f4364iv = kDFParameters.getIV();
            } else {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        byte[] bArr3;
        int iProcessBytes;
        if (i2 < this.f4363V.length + this.mac.getMacSize()) {
            throw new InvalidCipherTextException("length of input must be greater than the MAC and V combined");
        }
        if (this.cipher == null) {
            int length = (i2 - this.f4363V.length) - this.mac.getMacSize();
            ...
            for (int i4 = 0; i4 != length; i4++) {
                bArr3[i4] = (byte) (bArr[(this.f4363V.length + i) + i4] ^ bArr4[i4]);
            }
            iProcessBytes = 0;
        } else {
        ...
        Mac mac = this.mac;
        byte[] bArr12 = this.f4362IV;
        mac.update(bArr12, 0, bArr12.length);
        Mac mac2 = this.mac;
        byte[] bArr13 = this.f4363V;
    ...
    private byte[] encryptBlock(byte[] bArr, int i, int i2) {
        ...
        if (this.cipher == null) {
            ...
            if (this.f4362IV != null) {
                bufferedBlockCipher = this.cipher;
                keyParameter = new ParametersWithIV(new KeyParameter(bArr6), this.f4362IV);
            } else {
                bufferedBlockCipher = this.cipher;
        ...
        System.arraycopy(bArr2, 0, bArr14, this.f4363V.length, i2);
        System.arraycopy(bArr9, 0, bArr14, this.f4363V.length + i2, macSize);
        return bArr14;
    }
    ...
    private void extractParams(CipherParameters cipherParameters) {
        if (cipherParameters instanceof ParametersWithIV) {
            ...
            this.f4362IV = parametersWithIV.getIV();
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.forEncryption) {
            ...
            if (ephemeralKeyPairGenerator != null) {
                ...
                this.f4363V = ephemeralKeyPairGenerate.getEncodedPublicKey();
            }
        } else if (this.keyParser != null) {
            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bArr, i, i2);
            try {
                this.pubParam = this.keyParser.readKey(byteArrayInputStream);
                this.f4363V = Arrays.copyOfRange(bArr, i, (i2 - byteArrayInputStream.available()) + i);
            } catch (IOException e) {
