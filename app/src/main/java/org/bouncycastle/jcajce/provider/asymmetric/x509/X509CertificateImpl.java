...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException {
        if (!isAlgIdEqual(this.f4638c.getSignatureAlgorithm(), this.f4638c.getTBSCertificate().getSignature())) {
        ...
        try {
            ...
            this.f4638c.getTBSCertificate().encodeTo(bufferedOutputStream, ASN1Encoding.DER);
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException {
        ...
        int i = 0;
        if (z && X509SignatureUtil.isCompositeAlgorithm(this.f4638c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence aSN1Sequence = ASN1Sequence.getInstance(this.f4638c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence2 = ASN1Sequence.getInstance(DERBitString.getInstance(this.f4638c.getSignature()).getBytes());
            boolean z2 = false;
        ...
        if (!X509SignatureUtil.isCompositeAlgorithm(this.f4638c.getSignatureAlgorithm())) {
            Signature signatureCreateSignature = signatureCreator.createSignature(X509SignatureUtil.getSignatureName(this.f4638c.getSignatureAlgorithm()));
            if (!z) {
                checkSignature(publicKey, signatureCreateSignature, this.f4638c.getSignatureAlgorithm().getParameters(), getSignature());
                return;
            }
            List<PublicKey> publicKeys2 = ((CompositePublicKey) publicKey).getPublicKeys();
            while (i != publicKeys2.size()) {
                try {
                    checkSignature(publicKeys2.get(i), signatureCreateSignature, this.f4638c.getSignatureAlgorithm().getParameters(), getSignature());
                    return;
        ...
        ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(this.f4638c.getSignatureAlgorithm().getParameters());
        ASN1Sequence aSN1Sequence4 = ASN1Sequence.getInstance(DERBitString.getInstance(this.f4638c.getSignature()).getBytes());
        boolean z3 = false;
    ...
    public void checkValidity(Date date) throws CertificateNotYetValidException, CertificateExpiredException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f4638c.getEndDate().getTime());
        }
        if (date.getTime() >= getNotBefore().getTime()) {
            return;
        }
        throw new CertificateNotYetValidException("certificate not valid till " + this.f4638c.getStartDate().getTime());
    }
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f4638c.getTBSCertificate().getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            return this.f4638c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extensionOctets = getExtensionOctets(this.f4638c, "2.5.29.37");
        if (extensionOctets == null) {
            return null;
        }
        try {
    ...
    public byte[] getExtensionValue(String str) {
        ASN1OctetString extensionValue = getExtensionValue(this.f4638c, str);
        if (extensionValue == null) {
            return null;
        }
        try {
    ...
    public Collection getIssuerAlternativeNames() {
        return getAlternativeNames(this.f4638c, Extension.issuerAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f4638c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f4638c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Name getIssuerX500Name() {
        return this.f4638c.getIssuer();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f4638c.getIssuer().getEncoded(ASN1Encoding.DER));
    ...
    public Date getNotAfter() {
        return this.f4638c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotBefore() {
        return this.f4638c.getStartDate().getDate();
    }

    @Override // java.security.cert.Certificate
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f4638c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public BigInteger getSerialNumber() {
        return this.f4638c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f4638c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f4638c.getSignature().getOctets();
    }

    @Override // java.security.cert.X509Certificate
    public Collection getSubjectAlternativeNames() {
        return getAlternativeNames(this.f4638c, Extension.subjectAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getSubjectDN() {
        return new X509Principal(this.f4638c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f4638c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Name getSubjectX500Name() {
        return this.f4638c.getSubject();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f4638c.getSubject().getEncoded(ASN1Encoding.DER));
        } catch (IOException unused) {
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f4638c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public TBSCertificate getTBSCertificateNative() {
        return this.f4638c.getTBSCertificate();
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f4638c.getVersionNumber();
    }

    @Override // java.security.cert.X509Extension
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f4638c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration enumerationOids = extensions.oids();
        while (enumerationOids.hasMoreElements()) {
