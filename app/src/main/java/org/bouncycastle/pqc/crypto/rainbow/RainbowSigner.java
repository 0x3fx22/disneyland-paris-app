...
public class RainbowSigner implements MessageSigner {
    ...
    private ComputeInField f4912cf = new ComputeInField();
    RainbowKeyParameters key;
    private SecureRandom random;
    int signableDocumentLength;

    /* JADX INFO: renamed from: x */
    private short[] f4913x;

    private short[] initSign(Layer[] layerArr, short[] sArr) {
        short[] sArr2 = new short[sArr.length];
        short[] sArrMultiplyMatrix = this.f4912cf.multiplyMatrix(((RainbowPrivateKeyParameters) this.key).getInvA1(), this.f4912cf.addVect(((RainbowPrivateKeyParameters) this.key).getB1(), sArr));
        for (int i = 0; i < layerArr[0].getVi(); i++) {
            this.f4913x[i] = (short) this.random.nextInt();
            short[] sArr3 = this.f4913x;
    ...
    public byte[] generateSignature(byte[] bArr) {
        ...
        this.f4913x = new short[((RainbowPrivateKeyParameters) this.key).getInvA2().length];
        ...
        do {
            try {
                ...
                for (int i3 = 0; i3 < length; i3++) {
                    ...
                    short[] sArrSolveEquation = this.f4912cf.solveEquation(layers[i3].plugInVinegars(this.f4913x), sArr);
                    if (sArrSolveEquation == null) {
                        throw new Exception("LES is not solveable!");
                    }
                    for (int i5 = 0; i5 < sArrSolveEquation.length; i5++) {
                        this.f4913x[layers[i3].getVi() + i5] = sArrSolveEquation[i5];
                    }
                }
                short[] sArrMultiplyMatrix = this.f4912cf.multiplyMatrix(((RainbowPrivateKeyParameters) this.key).getInvA2(), this.f4912cf.addVect(((RainbowPrivateKeyParameters) this.key).getB2(), this.f4913x));
