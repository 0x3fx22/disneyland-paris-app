...
public class GMSSRootCalc {
    ...
    public GMSSRootCalc(int i, int i2, GMSSDigestProvider gMSSDigestProvider) {
        ...
        this.retain = new Vector[this.f4828K - 1];
    ...
    public void initialize(Vector vector) {
        int i;
        this.treehash = new Treehash[this.heightOfTree - this.f4828K];
        int i2 = 0;
        while (true) {
            i = this.heightOfTree;
            if (i2 >= i - this.f4828K) {
        ...
        this.retain = new Vector[this.f4828K - 1];
        for (int i4 = 0; i4 < this.f4828K - 1; i4++) {
    ...
    public void update(byte[] bArr) {
        ...
        } else if (i == 3 && this.heightOfTree > this.f4828K) {
            this.treehash[0].setFirstNode(bArr);
        }
        int i2 = this.index[0];
        if ((i2 - 3) % 2 == 0 && i2 >= 3 && this.heightOfTree == this.f4828K) {
        ...
        while (this.tailStack.size() > 0 && i5 == ((Integer) this.heightOfNodes.lastElement()).intValue()) {
            ...
            if (i5 < this.heightOfTree) {
                ...
                if (i5 >= this.heightOfTree - this.f4828K) {
                    if (i5 == 0) {
                        System.out.println("M���P");
                    }
                    int i8 = this.index[i5];
                    if ((i8 - 3) % 2 == 0 && i8 >= 3) {
                        this.retain[i5 - (this.heightOfTree - this.f4828K)].insertElementAt(bArr2, 0);
                    }
    ...
    public void update(byte[] bArr, byte[] bArr2) {
        ...
        if (i < this.heightOfTree - this.f4828K && this.indexForNextSeed - 2 == this.index[0]) {
