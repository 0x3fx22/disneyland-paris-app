...
public class JcaPublicKeyConverter {
    ...
    private static ECCurve convertCurve(EllipticCurve ellipticCurve, BigInteger bigInteger, int i) {
        ...
        if (field instanceof ECFieldFp) {
            return new ECCurve.C7835Fp(((ECFieldFp) field).getP(), a2, b, bigInteger, BigInteger.valueOf(i));
        }
    ...
    private ECParameterSpec getParams(ECDSAPublicKey eCDSAPublicKey) {
        ...
        ECCurve.C7835Fp c7835Fp = new ECCurve.C7835Fp(eCDSAPublicKey.getPrimeModulusP(), eCDSAPublicKey.getFirstCoefA(), eCDSAPublicKey.getSecondCoefB(), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF());
        ECPoint eCPointDecodePoint = c7835Fp.decodePoint(eCDSAPublicKey.getBasePointG());
        return new ECParameterSpec(convertCurve(c7835Fp), new java.security.spec.ECPoint(eCPointDecodePoint.getAffineXCoord().toBigInteger(), eCPointDecodePoint.getAffineYCoord().toBigInteger()), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF().intValue());
    }
    ...
    private java.security.spec.ECPoint getPublicPoint(ECDSAPublicKey eCDSAPublicKey) {
        ...
        ECPoint.C7838Fp c7838Fp = (ECPoint.C7838Fp) new ECCurve.C7835Fp(eCDSAPublicKey.getPrimeModulusP(), eCDSAPublicKey.getFirstCoefA(), eCDSAPublicKey.getSecondCoefB(), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF()).decodePoint(eCDSAPublicKey.getPublicPointY());
        return new java.security.spec.ECPoint(c7838Fp.getAffineXCoord().toBigInteger(), c7838Fp.getAffineYCoord().toBigInteger());
    }

    public PublicKey getKey(PublicKeyDataObject publicKeyDataObject) throws EACException, InvalidKeySpecException {
        if (publicKeyDataObject.getUsage().m1988on(EACObjectIdentifiers.id_TA_ECDSA)) {
