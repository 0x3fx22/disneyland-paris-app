...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature) throws SignatureException, InvalidKeyException, CertificateException {
        if (!isAlgIdEqual(this.f4667c.getSignatureAlgorithm(), this.f4667c.getTBSCertificate().getSignature())) {
            throw new CertificateException("signature algorithm in TBS cert not same as outer cert");
        }
        X509SignatureUtil.setSignatureParameters(signature, this.f4667c.getSignatureAlgorithm().getParameters());
    ...
    private byte[] getExtensionBytes(String str) {
        Extension extension;
        Extensions extensions = this.f4667c.getTBSCertificate().getExtensions();
    ...
    public void checkValidity(Date date) throws CertificateNotYetValidException, CertificateExpiredException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f4667c.getEndDate().getTime());
        }
        if (date.getTime() >= getNotBefore().getTime()) {
            return;
        }
        throw new CertificateNotYetValidException("certificate not valid till " + this.f4667c.getStartDate().getTime());
    }
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            return this.f4667c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public Principal getIssuerDN() {
        return new X509Principal(this.f4667c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f4667c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f4667c.getIssuer().getEncoded());
    ...
    public Date getNotAfter() {
        return this.f4667c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotBefore() {
        return this.f4667c.getStartDate().getDate();
    }

    @Override // java.security.cert.Certificate
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f4667c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public BigInteger getSerialNumber() {
        return this.f4667c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f4667c.getSignatureAlgorithm().getAlgorithm().getId();
    }

    @Override // java.security.cert.X509Certificate
    public byte[] getSigAlgParams() {
        if (this.f4667c.getSignatureAlgorithm().getParameters() != null) {
            try {
                return this.f4667c.getSignatureAlgorithm().getParameters().toASN1Primitive().getEncoded(ASN1Encoding.DER);
            } catch (IOException unused) {
            }
        }
        return null;
    ...
    public byte[] getSignature() {
        return this.f4667c.getSignature().getOctets();
    }
    ...
    public Principal getSubjectDN() {
        return new X509Principal(this.f4667c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f4667c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f4667c.getSubject().getEncoded());
        } catch (IOException unused) {
            throw new IllegalStateException("can't encode issuer DN");
        }
    }
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f4667c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public int getVersion() {
        return this.f4667c.getVersionNumber();
    }

    @Override // java.security.cert.X509Extension
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f4667c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration enumerationOids = extensions.oids();
        while (enumerationOids.hasMoreElements()) {
    ...
    @Override // java.security.cert.Certificate
    public final void verify(PublicKey publicKey) throws SignatureException, NoSuchAlgorithmException, InvalidKeyException, CertificateException, NoSuchProviderException {
        Signature signature;
        String signatureName = X509SignatureUtil.getSignatureName(this.f4667c.getSignatureAlgorithm());
        try {
