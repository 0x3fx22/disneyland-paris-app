...
public class PKIXNameConstraintValidator implements NameConstraintValidator {
    ...
    private Set intersectIPRange(byte[] bArr, byte[] bArr2) {
        ...
        return compareTo(max(bArrMinMaxIPs[0], bArrMinMaxIPs[2]), min(bArrMinMaxIPs[1], bArrMinMaxIPs[3])) == 1 ? Collections.EMPTY_SET : Collections.singleton(ipWithSubnetMask(m1989or(bArrMinMaxIPs[0], bArrMinMaxIPs[2]), m1989or(bArr4, bArr6)));
    }
    ...
    private static byte[] m1989or(byte[] bArr, byte[] bArr2) {
    ...
    */
    private void unionEmail(String str, String str2, Set set) {
        if (str.indexOf(64) != -1) {
            String strSubstring = str.substring(str.indexOf(64) + 1);
            if (str2.indexOf(64) == -1) {
                if (!str2.startsWith(InstructionFileId.DOT) ? !strSubstring.equalsIgnoreCase(str2) : !withinDomain(strSubstring, str2)) {
                }
            }
            set.add(str);
