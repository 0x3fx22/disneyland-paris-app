...
public class Blob {
    ...
    /* JADX INFO: renamed from: org.bouncycastle.gpg.keybox.Blob$1 */
    ...
    static Blob getInstance(Object obj, KeyFingerPrintCalculator keyFingerPrintCalculator, BlobVerifier blobVerifier) throws IOException {
        ...
        BlobType blobTypeFromByte = BlobType.fromByte(keyBoxByteBufferWrap.m2095u8());
        int iM2095u8 = keyBoxByteBufferWrap.m2095u8();
        int i = C77731.$SwitchMap$org$bouncycastle$gpg$keybox$BlobType[blobTypeFromByte.ordinal()];
        if (i == 2) {
            return FirstBlob.parseContent(iPosition, jU32, blobTypeFromByte, iM2095u8, keyBoxByteBufferWrap);
        }
        if (i == 3) {
            return CertificateBlob.parseContent(iPosition, jU32, blobTypeFromByte, iM2095u8, keyBoxByteBufferWrap, blobVerifier);
        }
        if (i != 4) {
            return null;
        }
        return PublicKeyRingBlob.parseContent(iPosition, jU32, blobTypeFromByte, iM2095u8, keyBoxByteBufferWrap, keyFingerPrintCalculator, blobVerifier);
    }
