...
public class UserAttributeSubpacketInputStream extends InputStream implements UserAttributeSubpacketTags {
    ...
    private void readFully(byte[] bArr, int i, int i2) throws IOException {
        ...
        while (i2 > 0) {
            int i4 = this.f4115in.read(bArr, i, i2);
            if (i4 < 0) {
                throw new EOFException();
            }
            i += i4;
    ...
    public int available() throws IOException {
        return this.f4115in.available();
    }

    @Override // java.io.InputStream
    public int read() throws IOException {
        return this.f4115in.read();
    }
    ...
    public UserAttributeSubpacket readPacket() throws IOException {
        ...
        if (i < 192) {
            ...
            i = ((i - 192) << 8) + this.f4115in.read() + 192;
            z = false;
        } else {
            if (i != 255) {
                throw new IOException("unrecognised length reading user attribute sub packet");
            }
            i = (this.f4115in.read() << 24) | (this.f4115in.read() << 16) | (this.f4115in.read() << 8) | this.f4115in.read();
            z = true;
        }
        int i2 = this.f4115in.read();
        if (i2 < 0) {
