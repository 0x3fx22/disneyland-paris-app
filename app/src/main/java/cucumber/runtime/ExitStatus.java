...
public class ExitStatus implements EventListener {
    ...
    public byte exitStatus() {
        ...
        return this.runtimeOptions.isWip() ? ((Result) Collections.min(this.results, Result.SEVERITY)).m1796is(Result.Type.PASSED) ? (byte) 1 : (byte) 0 : !((Result) Collections.max(this.results, Result.SEVERITY)).isOk(this.runtimeOptions.isStrict()) ? (byte) 1 : (byte) 0;
    }
}
