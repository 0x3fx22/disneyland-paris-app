...
public class StatusUtil {
    ...
    StatusManager f199sm;

    public StatusUtil(Context context) {
        this.f199sm = context.getStatusManager();
    }

    public StatusUtil(StatusManager statusManager) {
        this.f199sm = statusManager;
    }
    ...
    public void addStatus(Status status) {
        StatusManager statusManager = this.f199sm;
    ...
    public boolean containsException(Class<?> cls) {
        Iterator<Status> it = this.f199sm.getCopyOfStatusList().iterator();
        while (it.hasNext()) {
    ...
    public boolean containsMatch(long j, int i, String str) {
        List<Status> listFilterStatusListByTimeThreshold = filterStatusListByTimeThreshold(this.f199sm.getCopyOfStatusList(), j);
    ...
    public int getHighestLevel(long j) {
        int level = 0;
        for (Status status : filterStatusListByTimeThreshold(this.f199sm.getCopyOfStatusList(), j)) {
    ...
    public long timeOfLastReset() {
        List<Status> copyOfStatusList = this.f199sm.getCopyOfStatusList();
