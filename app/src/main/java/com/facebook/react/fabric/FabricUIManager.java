...
public class FabricUIManager implements UIManager, LifecycleEventListener, UIBlockViewResolver, SynchronousEventReceiver {
    ...
    public <T extends View> int addRootView(T t, WritableMap writableMap) {
        ...
        if (ReactNativeFeatureFlags.enableFabricLogs()) {
            FLog.m1281d(str, "Starting surface for module: %s and reactTag: %d", jSModuleName, Integer.valueOf(rootViewTag));
        }
    ...
    public <T extends View> int startSurface(T t, String str, WritableMap writableMap, int i, int i2) {
        ...
        if (ReactNativeFeatureFlags.enableFabricLogs()) {
            FLog.m1281d(TAG, "Starting surface for module: %s and reactTag: %d", str, Integer.valueOf(rootViewTag));
        }
    ...
    public void invalidate() {
        String str = TAG;
        FLog.m1303i(str, "FabricUIManager.invalidate");
    ...
    public boolean getThemeData(int i, float[] fArr) {
        ...
        if (context == null) {
            FLog.m1333w(TAG, "Couldn't get context for surfaceId %d in getThemeData", Integer.valueOf(i));
            return false;
        }
    ...
    public void synchronouslyUpdateViewOnUIThread(final int i, @NonNull final ReadableMap readableMap) {
        ...
        if (ReactNativeFeatureFlags.enableFabricLogs()) {
            FLog.m1281d(TAG, "SynchronouslyUpdateViewOnUIThread for tag %d: %s", Integer.valueOf(i), IS_DEVELOPMENT_ENVIRONMENT ? readableMap.toHashMap().toString() : "<hidden>");
        }
    ...
    public void updateRootLayoutSpecs(int i, int i2, int i3, int i4, int i5) {
        ...
        if (ReactNativeFeatureFlags.enableFabricLogs()) {
            FLog.m1280d(TAG, "Updating Root Layout Specs for [%d]", Integer.valueOf(i));
        }
        SurfaceMountingManager surfaceManager = this.mMountingManager.getSurfaceManager(i);
        if (surfaceManager == null) {
    ...
    public void receiveEvent(int i, int i2, @NonNull String str, boolean z, @Nullable WritableMap writableMap, int i3, boolean z2) {
        if (ReactBuildConfig.DEBUG && i == -1) {
            FLog.m1281d(TAG, "Emitted event without surfaceId: [%d] %s", Integer.valueOf(i2), str);
        }
        if (this.mDestroyed) {
            FLog.m1291e(TAG, "Attempted to receiveEvent after destruction");
            return;
        }
        ...
        if (eventEmitter == null) {
            ...
            FLog.m1303i(TAG, "Unable to invoke event: " + str + " for reactTag: " + i2);
            return;
        }
        if (z2) {
            UiThreadUtil.assertOnUiThread();
    ...
    public void setJSResponder(final int i, final int i2, final int i3, final boolean z) {
        this.mMountItemDispatcher.addMountItem(new MountItem() { // from class: com.facebook.react.fabric.FabricUIManager.4
            ...
            public void execute(@NonNull MountingManager mountingManager) {
                ...
                FLog.m1291e(FabricUIManager.TAG, "setJSResponder skipped, surface no longer available [" + i + "]");
            }
    ...
    private class DispatchUIFrameCallback extends GuardedFrameCallback {
        ...
        public void doFrameGuarded(long j) {
            ...
            if (!this.mIsMountingEnabled) {
                FLog.m1331w(FabricUIManager.TAG, "Not flushing pending UI operations: exception was previously thrown");
                return;
            }
            if (FabricUIManager.this.mDestroyed) {
                FLog.m1331w(FabricUIManager.TAG, "Not flushing pending UI operations: FabricUIManager is destroyed");
                return;
            }
            ...
            try {
                try {
                    ...
                    FLog.m1292e(FabricUIManager.TAG, "Exception thrown when executing UIFrameGuarded", e);
