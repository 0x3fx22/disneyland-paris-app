...
public class SurfaceMountingManager {
    ...
    public static void logViewHierarchy(ViewGroup viewGroup, boolean z) {
        ...
        FLog.m1291e(TAG, "  <ViewGroup tag=" + id + " class=" + viewGroup.getClass().toString() + ">");
        for (int i = 0; i < viewGroup.getChildCount(); i++) {
            FLog.m1291e(TAG, "     <View idx=" + i + " tag=" + viewGroup.getChildAt(i).getId() + " class=" + viewGroup.getChildAt(i).getClass().toString() + ">");
        }
        String str = TAG;
        FLog.m1291e(str, "  </ViewGroup tag=" + id + ">");
        if (z) {
            FLog.m1291e(str, "Displaying Ancestors:");
            for (ViewParent parent = viewGroup.getParent(); parent != null; parent = parent.getParent()) {
                ViewGroup viewGroup2 = parent instanceof ViewGroup ? (ViewGroup) parent : null;
                int id2 = viewGroup2 == null ? -1 : viewGroup2.getId();
                FLog.m1291e(TAG, "<ViewParent tag=" + id2 + " class=" + parent.getClass().toString() + ">");
            }
        }
    }
    ...
    public /* synthetic */ void lambda$addRootView$0(View view) {
        ...
        if (view.getId() == this.mSurfaceId) {
            ...
            String str = TAG;
            FLog.m1293e(str, "Trying to add RootTag to RootView that already has a tag: existing tag: [%d] new tag: [%d]", Integer.valueOf(view.getId()), Integer.valueOf(this.mSurfaceId));
    ...
    public void stopSurface() {
        FLog.m1291e(TAG, "Stopping surface [" + this.mSurfaceId + "]");
        if (isStopped()) {
            return;
        }
    ...
    public /* synthetic */ void lambda$stopSurface$1() {
        ...
        FLog.m1291e(TAG, "Surface [" + this.mSurfaceId + "] was stopped on SurfaceMountingManager.");
    }
    ...
    public void addViewAt(final int i, final int i2, final int i3) {
        ...
        if (z) {
            FLog.m1291e(TAG, "addViewAt: [" + i2 + "] -> [" + i + "] idx: " + i3 + " BEFORE");
        ...
        try {
            ...
            if (z) {
                UiThreadUtil.runOnUiThread(new Runnable() { // from class: com.facebook.react.fabric.mounting.SurfaceMountingManager.1
                    @Override // java.lang.Runnable
                    public void run() {
                        FLog.m1291e(SurfaceMountingManager.TAG, "addViewAt: [" + i2 + "] -> [" + i + "] idx: " + i3 + " AFTER");
    ...
    public void removeViewAt(final int i, final int i2, int i3) {
        ...
        if (SHOW_CHANGED_VIEW_HIERARCHIES) {
            FLog.m1291e(TAG, "removeViewAt: [" + i + "] -> [" + i2 + "] idx: " + i3 + " BEFORE");
            logViewHierarchy(viewGroup, false);
        }
        ...
        if (id != i) {
            ...
            if (i5 == -1) {
                FLog.m1291e(TAG, "removeViewAt: [" + i + "] -> [" + i2 + "] @" + i3 + ": view already removed from parent! Children in parent: " + childCount);
                return;
            }
        ...
        try {
            ...
            if (SHOW_CHANGED_VIEW_HIERARCHIES) {
                UiThreadUtil.runOnUiThread(new Runnable() { // from class: com.facebook.react.fabric.mounting.SurfaceMountingManager.2
                    @Override // java.lang.Runnable
                    public void run() {
                        FLog.m1291e(SurfaceMountingManager.TAG, "removeViewAt: [" + i + "] -> [" + i2 + "] idx: " + i4 + " AFTER");
                        SurfaceMountingManager.logViewHierarchy(viewGroup, false);
                    }
                });
            }
    ...
    public void printSurfaceState() {
        FLog.m1293e(TAG, "Views created for surface {%d}:", Integer.valueOf(getSurfaceId()));
        for (ViewState viewState : this.mTagToViewState.values()) {
            ViewManager viewManager = viewState.mViewManager;
            ...
            FLog.m1293e(TAG, "<%s id=%d parentTag=%s isRoot=%b />", name, Integer.valueOf(viewState.mReactTag), numValueOf, Boolean.valueOf(viewState.mIsRoot));
        }
    }
