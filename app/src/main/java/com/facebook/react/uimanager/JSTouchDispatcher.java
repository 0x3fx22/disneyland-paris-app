...
public class JSTouchDispatcher {
    ...
    public void handleTouchEvent(MotionEvent motionEvent, EventDispatcher eventDispatcher, @Nullable ReactContext reactContext) {
        ...
        if (action == 0) {
            if (this.mTargetTag != -1) {
                FLog.m1291e(ReactConstants.TAG, "Got DOWN touch before receiving UP or CANCEL from last gesture");
            }
            this.mChildIsHandlingNativeGesture = false;
        ...
        if (this.mTargetTag == -1) {
            FLog.m1291e(ReactConstants.TAG, "Unexpected state: received touch event but didn't get starting ACTION_DOWN for this gesture before");
            return;
        }
        if (action == 1) {
        ...
        if (action == 3) {
            if (!this.mTouchEventCoalescingKeyHelper.hasCoalescingKey(motionEvent.getDownTime())) {
                FLog.m1291e(ReactConstants.TAG, "Received an ACTION_CANCEL touch event for which we have no corresponding ACTION_DOWN");
            } else {
                dispatchCancelEvent(motionEvent, eventDispatcher);
            }
        ...
        FLog.m1331w(ReactConstants.TAG, "Warning : touch event was ignored. Action=" + action + " Target=" + this.mTargetTag);
    }
    ...
    private void dispatchCancelEvent(MotionEvent motionEvent, EventDispatcher eventDispatcher) {
        if (this.mTargetTag == -1) {
            FLog.m1331w(ReactConstants.TAG, "Can't cancel already finished gesture. Is a child View trying to start a gesture from an UP/CANCEL event?");
            return;
        }
