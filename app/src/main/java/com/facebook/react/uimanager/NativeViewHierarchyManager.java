...
public class NativeViewHierarchyManager {
    ...
    public synchronized void updateInstanceHandle(int i, long j) {
        ...
        try {
            ...
            FLog.m1292e(TAG, "Unable to update properties for view tag " + i, e);
        }
    }
    ...
    public synchronized void updateProperties(int i, ReactStylesDiffMap reactStylesDiffMap) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1281d(TAG, "updateProperties[%d]: %s", Integer.valueOf(i), reactStylesDiffMap.toString());
            }
            UiThreadUtil.assertOnUiThread();
            try {
    ...
    public synchronized void updateViewExtraData(int i, Object obj) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1281d(TAG, "updateViewExtraData[%d]: %s", Integer.valueOf(i), obj.toString());
            }
            UiThreadUtil.assertOnUiThread();
    ...
    public synchronized void updateLayout(int i, int i2, int i3, int i4, int i5, int i6, YogaDirection yogaDirection) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1285d(TAG, "updateLayout[%d]->[%d]: %d %d %d %d", Integer.valueOf(i2), Integer.valueOf(i), Integer.valueOf(i3), Integer.valueOf(i4), Integer.valueOf(i5), Integer.valueOf(i6));
            }
            UiThreadUtil.assertOnUiThread();
    ...
    private void updateInstanceHandle(View view, long j) {
        UiThreadUtil.assertOnUiThread();
        view.setTag(C3142R.id.view_tag_instance_handle, Long.valueOf(j));
    }
    ...
    public synchronized long getInstanceHandle(int i) {
        ...
        l = (Long) view.getTag(C3142R.id.view_tag_instance_handle);
        if (l == null) {
    ...
    public synchronized void createView(ThemedReactContext themedReactContext, int i, String str, @Nullable ReactStylesDiffMap reactStylesDiffMap) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1282d(TAG, "createView[%d]: %s %s", Integer.valueOf(i), str, reactStylesDiffMap != null ? reactStylesDiffMap.toString() : "<null>");
            }
            UiThreadUtil.assertOnUiThread();
    ...
    public synchronized void manageChildren(final int i, @Nullable int[] iArr, @Nullable ViewAtIndex[] viewAtIndexArr, @Nullable int[] iArr2) {
        ...
        synchronized (this) {
            try {
                if (this.DEBUG_MODE) {
                    FLog.m1283d(TAG, "createView[%d]: %s %s %s", Integer.valueOf(i), iArr3 != null ? iArr.toString() : "<null>", viewAtIndexArr != null ? viewAtIndexArr.toString() : "<null>", iArr2 != null ? iArr2.toString() : "<null>");
                }
                UiThreadUtil.assertOnUiThread();
    ...
    public synchronized void setChildren(int i, ReadableArray readableArray) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1281d(TAG, "setChildren[%d]: %s", Integer.valueOf(i), readableArray != null ? readableArray.toString() : "<null>");
            }
            UiThreadUtil.assertOnUiThread();
    ...
    protected final synchronized void addRootViewGroup(int i, View view) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1281d(TAG, "addRootViewGroup[%d]: %s", Integer.valueOf(i), view != null ? view.toString() : "<null>");
            }
            if (view.getId() != -1) {
                FLog.m1291e(TAG, "Trying to add a root view with an explicit id (" + view.getId() + ") already set. React Native uses the id field to track react tags and will overwrite this field. If that is fine, explicitly overwrite the id field to View.NO_ID before calling addRootView.");
            }
    ...
    protected synchronized void dropView(View view) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1280d(TAG, "dropView[%d]", Integer.valueOf(view != null ? view.getId() : -1));
            }
            UiThreadUtil.assertOnUiThread();
            if (view == null) {
                return;
            ...
            if ((view instanceof ViewGroup) && (viewManager instanceof ViewGroupManager)) {
                ...
                for (int childCount = viewGroupManager.getChildCount(viewGroup) - 1; childCount >= 0; childCount--) {
                    ...
                    if (childAt == null) {
                        FLog.m1291e(TAG, "Unable to drop null child view");
    ...
    public synchronized void removeRootView(int i) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1280d(TAG, "removeRootView[%d]", Integer.valueOf(i));
            }
            UiThreadUtil.assertOnUiThread();
            if (!this.mRootTags.get(i)) {
    ...
    public synchronized void measure(int i, int[] iArr) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1280d(TAG, "measure[%d]", Integer.valueOf(i));
            }
            UiThreadUtil.assertOnUiThread();
            View view = this.mTagsToViews.get(i);
            if (view == null) {
    ...
    public synchronized void measureInWindow(int i, int[] iArr) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1280d(TAG, "measureInWindow[%d]", Integer.valueOf(i));
            }
            UiThreadUtil.assertOnUiThread();
            View view = this.mTagsToViews.get(i);
            if (view == null) {
    ...
    public synchronized int findTargetTagForTouch(int i, float f, float f2) {
        ...
        try {
            if (this.DEBUG_MODE) {
                FLog.m1282d(TAG, "findTargetTagForTouch[%d]: %f %f", Integer.valueOf(i), Float.valueOf(f), Float.valueOf(f2));
            }
            UiThreadUtil.assertOnUiThread();
            view = this.mTagsToViews.get(i);
            if (view == null) {
    ...
    public synchronized void dispatchCommand(int i, int i2, @Nullable ReadableArray readableArray) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1282d(TAG, "dispatchCommand[%d]: %d %s", Integer.valueOf(i), Integer.valueOf(i2), readableArray != null ? readableArray.toString() : "<null>");
            }
            UiThreadUtil.assertOnUiThread();
            View view = this.mTagsToViews.get(i);
            if (view == null) {
    ...
    public synchronized void dispatchCommand(int i, String str, @Nullable ReadableArray readableArray) {
        try {
            if (this.DEBUG_MODE) {
                FLog.m1282d(TAG, "dispatchCommand[%d]: %s %s", Integer.valueOf(i), str, readableArray != null ? readableArray.toString() : "<null>");
            }
            UiThreadUtil.assertOnUiThread();
            View view = this.mTagsToViews.get(i);
            if (view == null) {
