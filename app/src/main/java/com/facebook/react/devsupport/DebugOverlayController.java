...
class DebugOverlayController {
    ...
    public static void requestPermission(Context context) {
        ...
        FLog.m1331w(ReactConstants.TAG, "Overlay permissions needs to be granted in order for react native apps to run in dev mode");
    ...
    private static boolean hasPermission(Context context, String str) {
        try {
            ...
            FLog.m1292e(ReactConstants.TAG, "Error while retrieving package info", e);
        }
        return false;
    }
            ...
            public void run() {
                if (z && DebugOverlayController.this.mFPSDebugViewContainer == null) {
                    if (!DebugOverlayController.permissionCheck(DebugOverlayController.this.mReactContext)) {
                        FLog.m1279d(ReactConstants.TAG, "Wait for overlay permission to be set");
                        return;
                    }
