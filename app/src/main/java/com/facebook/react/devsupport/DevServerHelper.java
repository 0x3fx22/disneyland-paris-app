...
public class DevServerHelper {
    ...
    public void openPackagerConnection(final String str, final PackagerCommandListener packagerCommandListener) {
        if (this.mPackagerClient != null) {
            FLog.m1331w(ReactConstants.TAG, "Packager connection already open, nooping.");
        } else {
    ...
    public void openInspectorConnection() {
        if (this.mInspectorPackagerConnection != null) {
            FLog.m1331w(ReactConstants.TAG, "Inspector connection already open, nooping.");
        } else {
    ...
    private String createBundleURL(String str, BundleType bundleType, String str2, boolean z, boolean z2) {
        ...
        sb2.append(String.format(Locale.US, "http://%s/%s.%s?platform=android&dev=%s&lazy=%s&minify=%s&app=%s&modulesOnly=%s&runModule=%s", str2, str, bundleType.typeID(), Boolean.valueOf(devMode), Boolean.valueOf(devMode), Boolean.valueOf(getJSMinifyMode()), this.mPackageName, z ? "true" : "false", z2 ? "true" : "false"));
    ...
    public void isPackagerRunning(PackagerStatusCallback packagerStatusCallback) {
        ...
        if (debugServerHost == null) {
            FLog.m1331w(ReactConstants.TAG, "No packager host configured.");
    ...
    public File downloadBundleResourceFromUrlSync(String str, File file) {
        ...
        try {
            ...
            FLog.m1293e(ReactConstants.TAG, "Failed to fetch resource synchronously - resourcePath: \"%s\", outputFile: \"%s\"", str, file.getAbsolutePath(), e);
            return null;
        }
        FLog.m1293e(ReactConstants.TAG, "Failed to fetch resource synchronously - resourcePath: \"%s\", outputFile: \"%s\"", str, file.getAbsolutePath(), e);
        return null;
