...
class JSDebuggerWebSocketClient extends WebSocketListener {
    ...
    public void prepareJSRuntime(JSDebuggerCallback jSDebuggerCallback) {
        ...
        try {
            ...
            new JsonWriter(stringWriter).beginObject().name("id").value(andIncrement).name(TCEventPropertiesNames.TCE_METHOD).value("prepareJSRuntime").endObject().close();
    ...
    public void loadBundle(String str, HashMap<String, String> map, JSDebuggerCallback jSDebuggerCallback) {
        ...
        try {
            StringWriter stringWriter = new StringWriter();
            JsonWriter jsonWriterBeginObject = new JsonWriter(stringWriter).beginObject().name("id").value(andIncrement).name(TCEventPropertiesNames.TCE_METHOD).value("executeApplicationScript").name("url").value(str).name("inject").beginObject();
    ...
    public void executeJSCall(String str, String str2, JSDebuggerCallback jSDebuggerCallback) {
        ...
        try {
            ...
            jsonWriter.beginObject().name("id").value(andIncrement).name(TCEventPropertiesNames.TCE_METHOD).value(str);
    ...
    public void onMessage(@Nullable WebSocket webSocket, String str) {
    ...
    void abort(String str, Throwable th) {
        FLog.m1292e(TAG, "Error occurred, shutting down websocket connection: " + str, th);
        closeQuietly();
