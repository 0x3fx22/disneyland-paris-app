...
public class CheckOnDiskStateDataApp implements RecoveryStrategy {
    ...
    public boolean recover(UnsatisfiedLinkError unsatisfiedLinkError, SoSource[] soSourceArr) {
        ...
        LogUtil.m1405e(SoLoader.TAG, "Checking /data/app missing libraries.");
        File file = new File(this.mContext.getApplicationInfo().nativeLibraryDir);
        if (!file.exists()) {
            LogUtil.m1405e(SoLoader.TAG, "Native library directory " + file + " does not exist, exiting /data/app recovery.");
            return false;
        }
            ...
            if (soSource instanceof BackupSoSource) {
                ...
                try {
                    ...
                    if (arrayList.isEmpty()) {
                        LogUtil.m1405e(SoLoader.TAG, "No libraries missing from " + file);
                        return false;
                    }
                    LogUtil.m1405e(SoLoader.TAG, "Missing libraries from " + file + ": " + arrayList.toString() + ", will run prepare on tbe backup so source");
                    backupSoSource.prepare(0);
                    break;
                } catch (Exception e) {
                    LogUtil.m1406e(SoLoader.TAG, "Encountered an exception while recovering from /data/app failure ", e);
                    return false;
                }
            }
        }
        ...
        LogUtil.m1405e(SoLoader.TAG, "Successfully recovered from /data/app disk failure.");
        return true;
    }
}
