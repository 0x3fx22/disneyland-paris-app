...
public class DirectorySoSource extends SoSource {
    ...
    protected int loadLibraryFrom(String str, int i, File file, StrictMode.ThreadPolicy threadPolicy) throws IOException {
        ...
        if (this.denyList.contains(str)) {
            LogUtil.m1403d(SoLoader.TAG, str + " is on the denyList, skip loading from " + file.getCanonicalPath());
            return 0;
        }
        File soFileByName = getSoFileByName(str);
        if (soFileByName == null) {
            LogUtil.m1409v(SoLoader.TAG, str + " file not found on " + file.getCanonicalPath());
            return 0;
        }
        String canonicalPath = soFileByName.getCanonicalPath();
        LogUtil.m1403d(SoLoader.TAG, str + " file found at " + canonicalPath);
        if ((i & 1) != 0 && (this.flags & 2) != 0) {
            LogUtil.m1403d(SoLoader.TAG, str + " loaded implicitly");
            return 2;
        }
        if ((this.flags & 1) != 0) {
            ...
            LogUtil.m1403d(SoLoader.TAG, "Not resolving dependencies for " + str);
        }
        try {
    ...
    public void addToLdLibraryPath(Collection<String> collection) {
        try {
            ...
            LogUtil.m1405e(SoLoader.TAG, "Failed to get canonical path for " + this.soDirectory.getName() + " due to " + e.toString() + ", falling to the absolute one");
