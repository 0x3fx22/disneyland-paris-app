...
import com.facebook.fresco.p027ui.common.ControllerListener2;
import com.facebook.fresco.p027ui.common.ForwardingControllerListener2;
import com.facebook.fresco.p027ui.common.LegacyOnFadeListener;
import com.facebook.fresco.p027ui.common.OnFadeListener;
...
public abstract class AbstractDraweeController<T, INFO> implements DraweeController, DeferredReleaser.Releasable, GestureDetector.ClickListener {
    private static final Map COMPONENT_EXTRAS = ImmutableMap.m1263of("component_tag", "drawee");
    private static final Map SHORTCUT_EXTRAS = ImmutableMap.m1264of("origin", "memory_bitmap", "origin_sub", "shortcut");
    ...
    private synchronized void init(String str, Object obj) {
        ...
        try {
            ...
            if (FLog.isLoggable(2)) {
                FLog.m1314v((Class<?>) TAG, "controller %x %s -> %s: initialize", Integer.valueOf(System.identityHashCode(this)), this.mId, str);
            }
            this.mId = str;
    ...
    public void setHierarchy(@Nullable DraweeHierarchy draweeHierarchy) {
        if (FLog.isLoggable(2)) {
            FLog.m1314v((Class<?>) TAG, "controller %x %s: setHierarchy: %s", Integer.valueOf(System.identityHashCode(this)), this.mId, draweeHierarchy);
        }
    ...
    private void setUpLoggingListener() {
        ...
        if (settableDraweeHierarchy instanceof GenericDraweeHierarchy) {
            ((GenericDraweeHierarchy) settableDraweeHierarchy).setOnFadeListener(new OnFadeListener() { // from class: com.facebook.drawee.controller.AbstractDraweeController.1
                @Override // com.facebook.fresco.p027ui.common.OnFadeListener
    ...
    public void onAttach() {
        ...
        if (FLog.isLoggable(2)) {
            FLog.m1314v((Class<?>) TAG, "controller %x %s: onAttach: %s", Integer.valueOf(System.identityHashCode(this)), this.mId, this.mIsRequestSubmitted ? "request already submitted" : "request needs submit");
        }
    ...
    public void onDetach() {
        ...
        if (FLog.isLoggable(2)) {
            FLog.m1313v((Class<?>) TAG, "controller %x %s: onDetach", Integer.valueOf(System.identityHashCode(this)), this.mId);
        }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        if (FLog.isLoggable(2)) {
            FLog.m1314v((Class<?>) TAG, "controller %x %s: onTouchEvent %s", Integer.valueOf(System.identityHashCode(this)), this.mId, motionEvent);
        }
        GestureDetector gestureDetector = this.mGestureDetector;
        if (gestureDetector == null) {
            return false;
    ...
    public boolean onClick() {
        if (FLog.isLoggable(2)) {
            FLog.m1313v((Class<?>) TAG, "controller %x %s: onClick", Integer.valueOf(System.identityHashCode(this)), this.mId);
        }
        if (!shouldRetryOnTap()) {
            return false;
        }
    ...
    protected void submitRequest() {
        ...
        if (FLog.isLoggable(2)) {
            FLog.m1314v((Class<?>) TAG, "controller %x %s: submitRequest: dataSource: %x", Integer.valueOf(System.identityHashCode(this)), this.mId, Integer.valueOf(System.identityHashCode(this.mDataSource)));
        }
    ...
    private void logMessageAndImage(String str, Object obj) {
        if (FLog.isLoggable(2)) {
            FLog.m1317v((Class<?>) TAG, "controller %x %s: %s: image: %s %x", Integer.valueOf(System.identityHashCode(this)), this.mId, str, getImageClass(obj), Integer.valueOf(getImageHash(obj)));
        }
    }

    private void logMessageAndFailure(String str, Throwable th) {
        if (FLog.isLoggable(2)) {
            FLog.m1315v((Class<?>) TAG, "controller %x %s: %s: failure: %s", Integer.valueOf(System.identityHashCode(this)), this.mId, str, th);
        }
    }
