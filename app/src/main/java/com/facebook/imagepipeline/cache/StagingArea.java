...
public class StagingArea {
        ...
        if (encodedImageCloneOrNull != null) {
            synchronized (encodedImageCloneOrNull) {
                if (!EncodedImage.isValid(encodedImageCloneOrNull)) {
                    this.mMap.remove(cacheKey);
                    FLog.m1329w((Class<?>) TAG, "Found closed reference %d for key %s (%d)", Integer.valueOf(System.identityHashCode(encodedImageCloneOrNull)), cacheKey.getUriString(), Integer.valueOf(System.identityHashCode(cacheKey)));
                    return null;
                }
    ...
    public synchronized boolean containsKey(CacheKey cacheKey) {
        ...
        synchronized (encodedImage) {
            ...
            this.mMap.remove(cacheKey);
            FLog.m1329w((Class<?>) TAG, "Found closed reference %d for key %s (%d)", Integer.valueOf(System.identityHashCode(encodedImage)), cacheKey.getUriString(), Integer.valueOf(System.identityHashCode(cacheKey)));
            return false;
        }
    }

    private synchronized void logStats() {
        FLog.m1312v((Class<?>) TAG, "Count = %d", Integer.valueOf(this.mMap.size()));
    }
}
