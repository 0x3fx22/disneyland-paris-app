...
public abstract class AbstractAdaptiveCountingMemoryCache<K, V> implements CountingMemoryCache<K, V> {
    ...
    public AbstractAdaptiveCountingMemoryCache(Supplier<MemoryCacheParams> supplier, MemoryCache.CacheTrimStrategy cacheTrimStrategy, ValueDescriptor<V> valueDescriptor, int i, int i2, int i3, int i4) {
        FLog.m1279d("AbstractArcCountingMemoryCache", "Create Adaptive Replacement Cache");
        ...
        synchronized (this) {
            try {
                ...
                if (canCacheNewValue(closeableReference.get())) {
                    CountingMemoryCache.Entry entryM1357of = CountingMemoryCache.Entry.m1357of(k, closeableReference, entryStateObserver);
                    Integer value = this.mLeastFrequentlyUsedKeysGhostList.getValue(k);
                    entryM1357of.accessCount = value != null ? value.intValue() : 0;
                    this.mCachedEntries.put(k, entryM1357of);
                    closeableReferenceNewClientReference = newClientReference(entryM1357of);
                }
            } catch (Throwable th) {
                throw th;
            }
    ...
    private synchronized CloseableReference newClientReference(final CountingMemoryCache.Entry entry) {
        ...
        return CloseableReference.m1347of(entry.valueRef.get(), (ResourceReleaser<V>) new ResourceReleaser() { // from class: com.facebook.imagepipeline.cache.AbstractAdaptiveCountingMemoryCache.2
