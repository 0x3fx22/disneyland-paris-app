...
public final class BufferedDiskCache {
    ...
    public final void put(@NotNull final CacheKey key, @NotNull EncodedImage encodedImage) {
        ...
        if (!FrescoSystrace.isTracing()) {
            ...
            try {
                ...
                FLog.m1330w((Class<?>) TAG, e, "Failed to schedule disk-cache write for %s", key.getUriString());
        ...
        try {
            ...
            try {
                ...
                FLog.m1330w((Class<?>) TAG, e2, "Failed to schedule disk-cache write for %s", key.getUriString());
                this.stagingArea.remove(key, encodedImage);
    ...
    private final Task containsAsync(final CacheKey key) {
        try {
            ...
            FLog.m1330w((Class<?>) TAG, e, "Failed to schedule disk-cache read for %s", key.getUriString());
    ...
    public final Task<Void> probe(@NotNull final CacheKey key) {
        ...
        try {
            ...
            FLog.m1330w((Class<?>) TAG, e, "Failed to schedule disk-cache probe for %s", key.getUriString());
            return Task.forError(e);
        }
    }
    ...
    private final boolean checkInStagingAreaAndFileCache(CacheKey key) {
        ...
        if (encodedImage != null) {
            encodedImage.close();
            FLog.m1312v((Class<?>) TAG, "Found image for %s in staging area", key.getUriString());
            this.imageCacheStatsTracker.onStagingAreaHit(key);
            return true;
        }
        FLog.m1312v((Class<?>) TAG, "Did not find image for %s in staging area", key.getUriString());
        ...
        try {
            ...
            if (encodedImage != null) {
                ...
                try {
                    ...
                    CloseableReference closeableReferenceM1345of = CloseableReference.m1345of(fromDiskCache);
                    Intrinsics.checkNotNullExpressionValue(closeableReferenceM1345of, "of(...)");
                    try {
                        EncodedImage encodedImage2 = new EncodedImage((CloseableReference<PooledByteBuffer>) closeableReferenceM1345of);
                        CloseableReference.closeSafely((CloseableReference<?>) closeableReferenceM1345of);
            ...
            FLog.m1311v((Class<?>) TAG, "Host thread was interrupted, decreasing reference count");
            encodedImage.close();
    ...
    public final Task<Void> remove(@NotNull final CacheKey key) {
        ...
        try {
            ...
            FLog.m1330w((Class<?>) TAG, e, "Failed to schedule disk-cache remove for %s", key.getUriString());
            return Task.forError(e);
        }
    }
    ...
    public final Task<Void> clearAll() {
        ...
        try {
            ...
            FLog.m1330w((Class<?>) TAG, e, "Failed to schedule disk-cache clear", new Object[0]);
            return Task.forError(e);
        }
    }
    ...
    private final PooledByteBuffer readFromDiskCache(CacheKey key) throws IOException {
        try {
            Class cls = TAG;
            FLog.m1312v((Class<?>) cls, "Disk cache read for %s", key.getUriString());
            BinaryResource resource = this.fileCache.getResource(key);
            if (resource == null) {
                FLog.m1312v((Class<?>) cls, "Disk cache miss for %s", key.getUriString());
                this.imageCacheStatsTracker.onDiskCacheMiss(key);
                return null;
            }
            FLog.m1312v((Class<?>) cls, "Found entry in disk cache for %s", key.getUriString());
            ...
            try {
                ...
                FLog.m1312v((Class<?>) cls, "Successful read from disk cache for %s", key.getUriString());
            ...
            FLog.m1330w((Class<?>) TAG, e, "Exception reading from cache for %s", key.getUriString());
    ...
    private final void writeToDiskCache(CacheKey key, final EncodedImage encodedImage) {
        Class cls = TAG;
        FLog.m1312v((Class<?>) cls, "About to write to disk-cache for key %s", key.getUriString());
        try {
            ...
            FLog.m1312v((Class<?>) cls, "Successful disk-cache write for key %s", key.getUriString());
        } catch (IOException e) {
            FLog.m1330w((Class<?>) TAG, e, "Failed to write to disk-cache for key %s", key.getUriString());
        }
    }
