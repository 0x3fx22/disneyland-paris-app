...
public class HeadingManager extends ReactContextBaseJavaModule implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        if (sensorEvent.sensor.getType() == 11) {
            ...
            int degrees = (int) (((((Math.toDegrees(SensorManager.getOrientation(this.rMat, this.orientation)[0]) + 360.0d) % 360.0d) - Math.toDegrees(SensorManager.getOrientation(this.rMat, this.orientation)[2])) + 360.0d) % 360.0d);
            this.newAzimuth = degrees;
