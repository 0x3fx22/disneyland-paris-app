...
public class RemoteModelLoader {
    ...
    public synchronized MappedByteBuffer load() throws MlKitException {
        ...
        try {
            ...
            gmsLogger.m1441d("RemoteModelLoader", "Try to load newly downloaded model file.");
            ...
            if (downloadingId == null || downloadingModelHash == null) {
                gmsLogger.m1441d("RemoteModelLoader", "No new model is downloading.");
                ...
                if (downloadingModelStatusCode == null) {
                    ...
                    gmsLogger.m1441d("RemoteModelLoader", "Download Status code: ".concat(downloadingModelStatusCode.toString()));
                    if (downloadingModelStatusCode.intValue() == 8) {
                        File fileZzi = this.zze.zzi(downloadingModelHash);
                        if (fileZzi != null) {
                            mappedByteBufferZzb = zzb(fileZzi);
                            gmsLogger.m1441d("RemoteModelLoader", "Moved the downloaded model to private folder successfully: ".concat(String.valueOf(fileZzi.getParent())));
                            this.zze.updateLatestModelHashAndType(downloadingModelHash);
                            if (z && this.zzf.zzd(fileZzi)) {
                                gmsLogger.m1441d("RemoteModelLoader", "All old models are deleted.");
                                mappedByteBufferZzb = zzb(this.zzf.zza(fileZzi));
                            }
                        }
                        if (mappedByteBufferZzb == null) {
                            gmsLogger.m1441d("RemoteModelLoader", "Loading existing model file.");
                            strZzb = this.zzf.zzb();
                            if (strZzb == null) {
                                gmsLogger.m1441d("RemoteModelLoader", "No existing model file");
                            } else {
                                try {
