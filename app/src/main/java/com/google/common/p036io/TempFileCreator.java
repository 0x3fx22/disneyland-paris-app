...
abstract class TempFileCreator {
    ...
    static final class JavaNioCreator extends TempFileCreator {
        ...
        @Override // com.google.common.p036io.TempFileCreator
        File createTempDir() {
            try {
        ...
        File createTempFile(String str) {
            return Files.createTempFile(Paths.get(StandardSystemProperty.JAVA_IO_TMPDIR.value(), new String[0]), str, null, filePermissions.get()).toFile();
        }
        ...
        private static PermissionSupplier userPermissions() {
            try {
                final ImmutableList immutableListM1517of = ImmutableList.m1517of(AclEntry.newBuilder().setType(AclEntryType.ALLOW).setPrincipal(FileSystems.getDefault().getUserPrincipalLookupService().lookupPrincipalByName(getUsername())).setPermissions(EnumSet.allOf(AclEntryPermission.class)).setFlags(AclEntryFlag.DIRECTORY_INHERIT, AclEntryFlag.FILE_INHERIT).build());
