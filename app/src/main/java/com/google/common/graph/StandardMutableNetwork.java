...
final class StandardMutableNetwork extends StandardNetwork implements MutableNetwork {
    ...
    public boolean addEdge(Object obj, Object obj2, Object obj3) {
        ...
        if (containsEdge(obj3)) {
            ...
            EndpointPair endpointPairM1630of = EndpointPair.m1630of(this, obj, obj2);
            Preconditions.checkArgument(endpointPairIncidentNodes.equals(endpointPairM1630of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", obj3, endpointPairIncidentNodes, endpointPairM1630of);
            return false;
        }
    ...
    private NetworkConnections newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m1627of();
            }
            return DirectedNetworkConnections.m1628of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m1634of();
        }
        return UndirectedNetworkConnections.m1635of();
    }
}
