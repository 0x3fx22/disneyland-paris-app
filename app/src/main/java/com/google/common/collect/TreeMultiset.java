...
public final class TreeMultiset<E> extends AbstractSortedMultiset implements Serializable {
    ...
    private long aggregateBelowRange(Aggregate aggregate, AvlNode avlNode) {
        ...
        if (iCompare == 0) {
            int i = C39774.$SwitchMap$com$google$common$collect$BoundType[this.range.getLowerBoundType().ordinal()];
            if (i != 1) {
                if (i != 2) {
            ...
            jTreeAggregate = aggregate.treeAggregate(avlNode.left) + ((long) aggregate.nodeAggregate(avlNode));
    ...
    private long aggregateAboveRange(Aggregate aggregate, AvlNode avlNode) {
        ...
        if (iCompare == 0) {
            int i = C39774.$SwitchMap$com$google$common$collect$BoundType[this.range.getUpperBoundType().ordinal()];
            if (i != 1) {
                if (i != 2) {
                    throw new AssertionError();
                }
            ...
            jTreeAggregate = aggregate.treeAggregate(avlNode.right) + ((long) aggregate.nodeAggregate(avlNode));
