...
public final class zzer {
    ...
    public static int zzc(int i, int i2, String str) {
        ...
        if (i < 0) {
            strZza = zzes.zza("%s (%s) must not be negative", ArrayContainsMatcher.INDEX_KEY, Integer.valueOf(i));
        } else {
            if (i2 < 0) {
            ...
            strZza = zzes.zza("%s (%s) must be less than size (%s)", ArrayContainsMatcher.INDEX_KEY, Integer.valueOf(i), Integer.valueOf(i2));
        }
    ...
    public static int zzd(int i, int i2, String str) {
        if (i < 0 || i > i2) {
            throw new IndexOutOfBoundsException(zzf(i, i2, ArrayContainsMatcher.INDEX_KEY));
        }
        return i;
    }
