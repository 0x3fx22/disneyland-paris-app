...
public final class zzt {
    public static int zza(int i, int i2, String str) {
        ...
        if (i < 0) {
            strZza = zzu.zza("%s (%s) must not be negative", ArrayContainsMatcher.INDEX_KEY, Integer.valueOf(i));
        } else {
            if (i2 < 0) {
                throw new IllegalArgumentException("negative size: " + i2);
            }
            strZza = zzu.zza("%s (%s) must be less than size (%s)", ArrayContainsMatcher.INDEX_KEY, Integer.valueOf(i), Integer.valueOf(i2));
        }
