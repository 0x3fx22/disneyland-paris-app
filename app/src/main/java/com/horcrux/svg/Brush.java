...
class Brush {
    ...
    void setupPaint(Paint paint, RectF rectF, float f, float f2) {
        ...
        if (size == 0) {
            FLog.m1331w(ReactConstants.TAG, "Gradient contains no stops");
            return;
        }
        int i = size / 2;
        int[] iArr3 = new int[i];
        ...
        if (i == 1) {
            ...
            FLog.m1331w(ReactConstants.TAG, "Gradient contains only one stop");
            iArr = iArr4;
            fArr = fArr4;
        } else {
            iArr = iArr3;
        ...
        if (brushType == BrushType.RADIAL_GRADIENT) {
            ...
            double d10 = val7 / d;
            RadialGradient radialGradient = new RadialGradient((float) (getVal(this.mPoints[4], d8, f, textSize) + ((double) f3)), (float) (getVal(this.mPoints[5], d9 / d10, f, textSize) + (((double) f4) / d10)), (float) d, iArr2, fArr2, Shader.TileMode.CLAMP);
