...
public class Analytics extends AbstractAppCenterService {
    ...
    protected synchronized void applyEnabledState(boolean z) {
        try {
            if (z) {
                this.mChannel.addGroup("group_analytics_critical", getTriggerCount(), C0740C.DEFAULT_MAX_SEEK_TO_PREVIOUS_POSITION_MS, getTriggerMaxParallelRequests(), null, getChannelListener());
