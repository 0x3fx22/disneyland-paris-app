...
public final class WebpDownsampler {
        ...
        if (shouldUsePool(type)) {
            if (z2) {
                ...
                if (Log.isLoggable("WebpDownsampler", 2)) {
                    Log.v("WebpDownsampler", "Calculated target [" + iRound + "x" + iRound2 + "] for source [" + i3 + "x" + i4 + "], sampleSize: " + i7 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            }
    ...
    */
    private static Bitmap decodeStream(InputStream inputStream, BitmapFactory.Options options, Downsampler.DecodeCallbacks decodeCallbacks, BitmapPool bitmapPool) throws IOException {
        ...
        try {
            try {
                Bitmap bitmapDecodeStream = WebpBitmapFactory.decodeStream(inputStream, null, options);
                ...
                if (Log.isLoggable("WebpDownsampler", 3)) {
                    Log.d("WebpDownsampler", "Failed to decode with inBitmap, trying again without Bitmap re-use", iOExceptionNewIoExceptionForInBitmapAssertion);
                }
