...
public class WebpHeaderParser {
    ...
    private static final class StreamReader implements Reader {
        ...
        private final InputStream f1020is;

        StreamReader(InputStream inputStream) {
            this.f1020is = inputStream;
        }

        @Override // com.bumptech.glide.integration.webp.WebpHeaderParser.Reader
        public int getUInt16() {
            return (this.f1020is.read() & 255) | ((this.f1020is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK);
        }
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long jSkip = this.f1020is.skip(j2);
                if (jSkip <= 0) {
                    if (this.f1020is.read() == -1) {
                        break;
                    }
                    jSkip = 1;
                }
