...
class EngineJob implements DecodeJob.Callback, FactoryPools.Poolable {
        ...
        synchronized (this) {
            try {
                ...
                for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                    resourceCallbackAndExecutor.executor.execute(new CallResourceReady(resourceCallbackAndExecutor.f1025cb));
                }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            try {
                ...
                for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                    resourceCallbackAndExecutor.executor.execute(new CallLoadFailed(resourceCallbackAndExecutor.f1025cb));
                }
                decrementPendingCallbacks();
            } catch (Throwable th) {
                throw th;
    ...
    private class CallLoadFailed implements Runnable {
        ...
        private final ResourceCallback f1023cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f1023cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f1023cb.getLock()) {
                synchronized (EngineJob.this) {
                    try {
                        if (EngineJob.this.cbs.contains(this.f1023cb)) {
                            EngineJob.this.callCallbackOnLoadFailed(this.f1023cb);
                        }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* JADX INFO: renamed from: cb */
        private final ResourceCallback f1024cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f1024cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f1024cb.getLock()) {
                synchronized (EngineJob.this) {
                    try {
                        if (EngineJob.this.cbs.contains(this.f1024cb)) {
                            EngineJob.this.engineResource.acquire();
                            EngineJob.this.callCallbackOnResourceReady(this.f1024cb);
                            EngineJob.this.removeCallback(this.f1024cb);
                        }
                        EngineJob.this.decrementPendingCallbacks();
                    } catch (Throwable th) {
                        throw th;
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        /* JADX INFO: renamed from: cb */
        final ResourceCallback f1025cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f1025cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f1025cb.equals(((ResourceCallbackAndExecutor) obj).f1025cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f1025cb.hashCode();
        }
    }
