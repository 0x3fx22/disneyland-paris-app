...
public class MapUtil {
    public static void convertAndAddToCustomVarList(ReadableMap readableMap, List<CustomVar> list) {
        try {
            list.add(new CustomVar(readableMap.getInt(ArrayContainsMatcher.INDEX_KEY), readableMap.getString("key"), readableMap.getString("value")));
        } catch (Exception e) {
    ...
    public static Object getValueForKey(ReadableMap readableMap, String str) {
        ...
        int i = C29741.$SwitchMap$com$facebook$react$bridge$ReadableType[readableMap.getType(str).ordinal()];
        if (i == 2) {
    ...
    /* JADX INFO: renamed from: com.contentsquare.rn.utils.MapUtil$1 */
