...
import com.contentsquare.p025rn.externalbridge.ExternalBridgeInterfaceImpl;
import com.contentsquare.p025rn.externalbridge.XpfInterfaceBridge;
import com.contentsquare.p025rn.masking.ViewMasker;
import com.contentsquare.p025rn.utils.LogMonitorUtil;
import com.contentsquare.p025rn.utils.MapUtil;
...
public class ContentsquareModule extends ReactContextBaseJavaModule {
    ...
    public static /* synthetic */ void lambda$initComponents$17(ReadableMap readableMap) {
        byte b;
        try {
            WritableMap writableMapCreateMap = Arguments.createMap();
            writableMapCreateMap.merge(readableMap);
            writableMapCreateMap.putBoolean("xpf_rn_new_arch_enabled", true);
            Object objNewInstance = TelemetryInterface.class.newInstance();
            ReadableMapKeySetIterator readableMapKeySetIteratorKeySetIterator = writableMapCreateMap.keySetIterator();
            while (readableMapKeySetIteratorKeySetIterator.hasNextKey()) {
                String strNextKey = readableMapKeySetIteratorKeySetIterator.nextKey();
                int iHashCode = strNextKey.hashCode();
                if (iHashCode != -1983476629) {
                    if (iHashCode != 264356371) {
                        if (iHashCode == 1612596743 && strNextKey.equals("xpf_version")) {
                            b = 1;
                        } else {
                            b = -1;
                        }
                    } else if (strNextKey.equals("xpf_bridge_version")) {
                        b = 2;
                    } else {
                        b = -1;
                    }
                } else if (strNextKey.equals("xpf_type")) {
                    b = 0;
                } else {
                    b = -1;
                }
                if (b == 0) {
                    try {
                        TelemetryInterface.class.getDeclaredMethod("setXPFType", Object.class).invoke(objNewInstance, writableMapCreateMap.getString(strNextKey));
                    } catch (Exception e) {
                        Log.d("CSLIB", "Exception failure while calling setXPFType", e);
                    }
                } else if (b == 1) {
                    try {
                        TelemetryInterface.class.getDeclaredMethod("setXPFVersion", Object.class).invoke(objNewInstance, writableMapCreateMap.getString(strNextKey));
                    } catch (Exception e2) {
                        Log.d("CSLIB", "Exception failure while calling setXPFVersion", e2);
                    }
                } else if (b == 2) {
                    try {
                        TelemetryInterface.class.getDeclaredMethod("setXPFBridgeVersion", Object.class).invoke(objNewInstance, writableMapCreateMap.getString(strNextKey));
                    } catch (Exception e3) {
                        Log.d("CSLIB", "Exception failure while calling setXPFBridgeVersion", e3);
                    }
                } else {
                    try {
                        TelemetryInterface.class.getDeclaredMethod("telemetryCollect", String.class, Object.class).invoke(objNewInstance, strNextKey, MapUtil.getValueForKey(writableMapCreateMap, strNextKey));
                    } catch (Exception e4) {
                        Log.d("CSLIB", "Exception failure while calling telemetryCollect", e4);
                    }
                }
            }
        } catch (Exception e5) {
            Log.e("CSLIB", "Exception failure while initializing components", e5);
            e5.printStackTrace();
        }
    }
