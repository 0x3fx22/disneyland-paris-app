...
public class ConfigRetrieverTask extends AsyncTask<String, Void, Void> {
    ...
    private String fetchConfig(String str, String str2, int i) {
        ...
        if (httpResponsePerformHttpGet.success()) {
            this.mLogger.m827d("Got HTTP_OK for endpoint: " + str);
            ...
            if (!TextUtils.isEmpty(stringResponse)) {
                ...
                this.mLogger.m827d("Got remote config " + stringResponse);
                return stringResponse;
            }
        } else if (httpResponsePerformHttpGet.getStatus() == 304) {
            this.mLogger.m827d("Got HTTP Not-Modified: current config still valid, do nothing.");
            return null;
        }
        if (httpResponsePerformHttpGet.getStatus() == 404) {
            this.mLogger.m829e("Got HTTP_NOT_FOUND for endpoint " + str);
            this.mLogger.m833p("Config for package name '" + str2 + "' could not be retrieved. A Contentsquare project might not have been created for you yet. Send your package name to your Contentsquare contact.");
        }
        if (httpResponsePerformHttpGet.getStatus() >= 400) {
            this.mLogger.m831i("Failed to fetch config for " + str2 + ". Network error: " + httpResponsePerformHttpGet.getStatus());
        }
        int i2 = i - 1;
        ...
        this.mLogger.m834w("retrying the conf fetch for the " + i3 + "th time");
        try {
            int iPow = (int) (((double) this.mExponentialBackOffMsecInit) * Math.pow(i3, 2.0d));
            this.mLogger.m827d("sleeping " + iPow + " msec before the next retry");
            Thread.sleep((long) iPow);
        } catch (InterruptedException e) {
            this.mLogger.m830e(e, "Config fetch interrupted.");
        }
    ...
    public Void doInBackground(String... strArr) {
        ...
        if (rootConfigDecodeFromString == null || !this.mConfiguration.hasConfigChanged(rootConfigDecodeFromString)) {
            this.mLogger.m827d("The config fetched from CS servers is the same as the one saved in the device");
        } else {
    ...
    public void onPostExecute(Void r1) {
        if (!this.mUseCallBackDuringOnPostExecute) {
            this.mLogger.m827d("callback not called");
        } else {
