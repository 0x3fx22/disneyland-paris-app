...
public final class HttpConnection {
    ...
    private final void logPublicError(String endpoint, Throwable exception) {
        this.logger.m833p("[ERROR] Contentsquare failed to execute request with endpoint [" + endpoint + "]: " + exception.getMessage() + AbstractJsonLexerKt.END_LIST);
    }
    ...
    private final HttpResponse performHttpPost(String endpoint, byte[] requestData, Map<String, String> headers) {
        this.logger.m827d("Prepare HTTP POST for " + endpoint);
        ...
        this.logger.m827d("Response HTTP POST: status " + httpResponseHandlePost.getStatus() + " for " + requestOptions.getEndpoint());
    ...
    public final HttpResponse performHttpGet(String endpoint, Map<String, String> headers) {
        ...
        this.logger.m827d("Prepare HTTP GET for " + endpoint);
        RequestOptions requestOptions = new RequestOptions(endpoint, headers, 0, 0, 12, null);
        ...
        this.logger.m827d(httpResponseHandleGet.getStatus() + " HTTP response for " + requestOptions.getEndpoint() + ' ' + strConcat);
        ...
        if ((400 <= status && status < 600) || httpResponseHandleGet.getStatus() == -1) {
            ...
            this.logger.m830e(httpResponseHandleGet.getException(), "Exception while processing Http GET request on $" + requestOptions.getEndpoint());
            String endpoint2 = requestOptions.getEndpoint();
