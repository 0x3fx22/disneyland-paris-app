...
abstract class JsonUtils {
    private static final JsonReader.Options POINT_NAMES = JsonReader.Options.m434of("x", "y");
    ...
    static PointF jsonToPoint(JsonReader jsonReader, float f) {
        int i = C18301.$SwitchMap$com$airbnb$lottie$parser$moshi$JsonReader$Token[jsonReader.peek().ordinal()];
        if (i == 1) {
    ...
    static float valueFromObject(JsonReader jsonReader) throws IOException {
        ...
        int i = C18301.$SwitchMap$com$airbnb$lottie$parser$moshi$JsonReader$Token[tokenPeek.ordinal()];
        if (i == 1) {
