...
public class ShapeData {
    ...
    public void interpolateBetween(ShapeData shapeData, ShapeData shapeData2, @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 1.0d) float f) {
        if (this.initialPoint == null) {
            this.initialPoint = new PointF();
        }
        ...
        for (int size3 = this.curves.size() - 1; size3 >= 0; size3--) {
            ...
            ((CubicCurveData) this.curves.get(size3)).setControlPoint1(MiscUtils.lerp(controlPoint1.x, controlPoint3.x, f), MiscUtils.lerp(controlPoint1.y, controlPoint3.y, f));
            ((CubicCurveData) this.curves.get(size3)).setControlPoint2(MiscUtils.lerp(controlPoint2.x, controlPoint4.x, f), MiscUtils.lerp(controlPoint2.y, controlPoint4.y, f));
