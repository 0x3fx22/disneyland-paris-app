...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    static BaseLayer forModel(CompositionLayer compositionLayer, Layer layer, LottieDrawable lottieDrawable, LottieComposition lottieComposition) {
        switch (C18241.$SwitchMap$com$airbnb$lottie$model$layer$Layer$LayerType[layer.getLayerType().ordinal()]) {
            case 1:
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        Paint paint;
        Integer value;
        C1806L.beginSection(this.drawTraceName);
        if (!this.visible || this.layerModel.isHidden()) {
            C1806L.endSection(this.drawTraceName);
            return;
        }
        buildParentLayerListIfNeeded();
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("Layer#parentMatrix");
        }
        this.matrix.reset();
        ...
        if (C1806L.isTraceEnabled()) {
            C1806L.endSection("Layer#parentMatrix");
        }
        ...
        if (!hasMatteOnThisLayer() && !hasMasksOnThisLayer() && getBlendMode() == LBlendMode.NORMAL) {
            ...
            if (C1806L.isTraceEnabled()) {
                C1806L.beginSection("Layer#drawLayer");
            }
            drawLayer(canvas, this.matrix, iIntValue);
            if (C1806L.isTraceEnabled()) {
                C1806L.endSection("Layer#drawLayer");
            }
            recordRenderTime(C1806L.endSection(this.drawTraceName));
            return;
        }
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("Layer#computeBounds");
        }
        ...
        if (C1806L.isTraceEnabled()) {
            C1806L.endSection("Layer#computeBounds");
        }
        if (this.rect.width() >= 1.0f && this.rect.height() >= 1.0f) {
            if (C1806L.isTraceEnabled()) {
                C1806L.beginSection("Layer#saveLayer");
            }
            ...
            if (C1806L.isTraceEnabled()) {
                C1806L.endSection("Layer#saveLayer");
            }
            ...
            if (hasMatteOnThisLayer()) {
                if (C1806L.isTraceEnabled()) {
                    C1806L.beginSection("Layer#drawMatte");
                    C1806L.beginSection("Layer#saveLayer");
                }
                ...
                if (C1806L.isTraceEnabled()) {
                    C1806L.beginSection("Layer#restoreLayer");
                }
                canvas.restore();
                if (C1806L.isTraceEnabled()) {
                    C1806L.endSection("Layer#restoreLayer");
                    C1806L.endSection("Layer#drawMatte");
                }
            }
            if (C1806L.isTraceEnabled()) {
                C1806L.beginSection("Layer#restoreLayer");
    ...
    private void clearCanvas(Canvas canvas) {
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("Layer#clearLayer");
        }
        RectF rectF = this.rect;
        canvas.drawRect(rectF.left - 1.0f, rectF.top - 1.0f, rectF.right + 1.0f, rectF.bottom + 1.0f, this.clearPaint);
        if (C1806L.isTraceEnabled()) {
            C1806L.endSection("Layer#clearLayer");
        }
    }
        ...
        if (hasMasksOnThisLayer()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (value != null) {
                    ...
                    int i2 = C18241.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask.getMaskMode().ordinal()];
                    if (i2 == 1 || i2 == 2) {
                        return;
                    }
    ...
    void setProgress(float f) {
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("BaseLayer#setProgress");
            C1806L.beginSection("BaseLayer#setProgress.transform");
        }
        this.transform.setProgress(f);
        if (C1806L.isTraceEnabled()) {
            C1806L.endSection("BaseLayer#setProgress.transform");
        }
        if (this.mask != null) {
            if (C1806L.isTraceEnabled()) {
                C1806L.beginSection("BaseLayer#setProgress.mask");
            }
            for (int i = 0; i < this.mask.getMaskAnimations().size(); i++) {
                this.mask.getMaskAnimations().get(i).setProgress(f);
            }
            if (C1806L.isTraceEnabled()) {
                C1806L.endSection("BaseLayer#setProgress.mask");
            }
        }
        if (this.inOutAnimation != null) {
            if (C1806L.isTraceEnabled()) {
                C1806L.beginSection("BaseLayer#setProgress.inout");
            }
            this.inOutAnimation.setProgress(f);
            if (C1806L.isTraceEnabled()) {
                C1806L.endSection("BaseLayer#setProgress.inout");
            }
        }
        if (this.matteLayer != null) {
            if (C1806L.isTraceEnabled()) {
                C1806L.beginSection("BaseLayer#setProgress.matte");
            }
            this.matteLayer.setProgress(f);
            if (C1806L.isTraceEnabled()) {
                C1806L.endSection("BaseLayer#setProgress.matte");
            }
        }
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("BaseLayer#setProgress.animations." + this.animations.size());
        }
        for (int i2 = 0; i2 < this.animations.size(); i2++) {
            ((BaseKeyframeAnimation) this.animations.get(i2)).setProgress(f);
        }
        if (C1806L.isTraceEnabled()) {
            C1806L.endSection("BaseLayer#setProgress.animations." + this.animations.size());
            C1806L.endSection("BaseLayer#setProgress");
        }
    }
