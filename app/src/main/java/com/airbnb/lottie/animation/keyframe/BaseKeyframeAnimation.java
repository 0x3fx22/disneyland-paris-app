...
public abstract class BaseKeyframeAnimation<K, A> {
    ...
    public void setProgress(@FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, m3to = 1.0d) float f) {
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("BaseKeyframeAnimation#setProgress");
        }
        if (this.keyframesWrapper.isEmpty()) {
            if (C1806L.isTraceEnabled()) {
                C1806L.endSection("BaseKeyframeAnimation#setProgress");
                return;
            }
            return;
        }
    ...
    public void notifyListeners() {
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("BaseKeyframeAnimation#notifyListeners");
        }
        for (int i = 0; i < this.listeners.size(); i++) {
            ((AnimationListener) this.listeners.get(i)).onValueChanged();
        }
        if (C1806L.isTraceEnabled()) {
            C1806L.endSection("BaseKeyframeAnimation#notifyListeners");
        }
    }

    protected Keyframe<K> getCurrentKeyframe() {
        if (C1806L.isTraceEnabled()) {
            C1806L.beginSection("BaseKeyframeAnimation#getCurrentKeyframe");
        }
        Keyframe<K> currentKeyframe = this.keyframesWrapper.getCurrentKeyframe();
        if (C1806L.isTraceEnabled()) {
            C1806L.endSection("BaseKeyframeAnimation#getCurrentKeyframe");
        }
