...
public abstract class ValueMatcher implements JsonSerializable, Predicate<JsonSerializable> {
    ...
    public static ValueMatcher parse(@Nullable JsonValue jsonValue) throws JsonException {
        ...
        if (jsonMapOptMap.containsKey(ArrayContainsMatcher.ARRAY_CONTAINS_KEY)) {
            ...
            if (jsonMapOptMap.containsKey(ArrayContainsMatcher.INDEX_KEY)) {
                int i = jsonMapOptMap.opt(ArrayContainsMatcher.INDEX_KEY).getInt(-1);
                if (i == -1) {
                    throw new JsonException("Invalid index for array_contains matcher: " + jsonMapOptMap.get(ArrayContainsMatcher.INDEX_KEY));
                }
