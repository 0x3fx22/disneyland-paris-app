...
public class UrlAllowList {
    ...
    public boolean addEntry(@NonNull String str, int i) {
        ...
        if (uri == null) {
            UALog.m1744e("Invalid URL allow list pattern %s", str);
            return false;
        }
        ...
        if (!UAStringUtil.isEmpty(scheme)) {
            ...
            if (pattern.matcher(scheme).matches()) {
                ...
                if (strNullIfEmpty != null && !HOST_PATTERN.matcher(strNullIfEmpty).matches()) {
                    UALog.m1744e("Invalid host %s in URL allow list pattern %s", strNullIfEmpty, str);
                    return false;
                }
                ...
                if (schemeSpecificPart != null && !pattern.matcher(schemeSpecificPart).matches()) {
                    UALog.m1744e("Invalid path %s in URL allow list pattern %s", schemeSpecificPart, str);
                    return false;
                }
        ...
        UALog.m1744e("Invalid scheme %s in URL allow list pattern %s", scheme, str);
        return false;
    }
