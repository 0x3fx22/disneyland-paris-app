...
public final class PreferenceDataStore {
    ...
    private final PreferenceDataDatabase f3711db;
    ...
    PreferenceDataStore(PreferenceDataDatabase preferenceDataDatabase) {
        this.f3711db = preferenceDataDatabase;
    ...
    private void loadPreferences() {
        try {
            ...
            UALog.m1746e(e, "Failed to load preferences. Retrying with fallback loading.", new Object[0]);
            fallbackLoad();
        }
    }
    ...
    private void fallbackLoad() {
        ...
        try {
            ...
            UALog.m1746e(e, "Failed to load keys.", new Object[0]);
            listQueryKeys = null;
        }
        if (listQueryKeys == null || listQueryKeys.isEmpty()) {
            UALog.m1744e("Unable to load keys, deleting preference store.", new Object[0]);
            try {
                this.dao.deleteAll();
                return;
            } catch (Exception e2) {
                UALog.m1746e(e2, "Failed to delete preferences.", new Object[0]);
                return;
            }
        }
        ArrayList arrayList = new ArrayList();
        for (String str : listQueryKeys) {
            try {
                ...
                if (preferenceDataQueryValue.value == null) {
                    UALog.m1744e("Unable to fetch preference value. Deleting: %s", str);
                    this.dao.delete(str);
                } else {
                    arrayList.add(new Preference(preferenceDataQueryValue.getKey(), preferenceDataQueryValue.getValue()));
                }
            } catch (Exception e3) {
                UALog.m1746e(e3, "Failed to delete preference %s", str);
            }
        }
        finishLoad(arrayList);
    }
    ...
    public JsonValue getJsonValue(@NonNull String str) {
        try {
            ...
            UALog.m1742d(e, "Unable to parse preference value: %s", str);
        ...
        private boolean setValue(String str) {
            synchronized (this) {
                try {
                    ...
                    this.value = str;
                    UALog.m1751v("Preference updated: %s", this.key);
        ...
        public boolean writeValue(String str) {
            synchronized (this) {
                try {
                    try {
                        if (str == null) {
                            UALog.m1751v("Removing preference: %s", this.key);
                            PreferenceDataStore.this.dao.delete(this.key);
                        } else {
                            UALog.m1751v("Saving preference: %s value: %s", this.key, str);
                            PreferenceDataStore.this.dao.upsert(new PreferenceData(this.key, str));
                        }
                    } catch (Exception e) {
                        UALog.m1746e(e, "Failed to write preference %s:%s", this.key, str);
                        return false;
                    }
                } catch (Throwable th) {
                    throw th;
