...
public abstract class EventDao {
    ...
    public void trimDatabase(int i) {
        while (databaseSize() > i) {
            ...
            UALog.m1741d("Event database size exceeded. Deleting oldest session: %s", strOldestSessionId);
            int iDeleteSession = deleteSession(strOldestSessionId);
            UALog.m1741d("Deleted %d rows with session ID %s", Integer.valueOf(iDeleteSession), strOldestSessionId);
            if (iDeleteSession == 0) {
                return;
            }
        }
