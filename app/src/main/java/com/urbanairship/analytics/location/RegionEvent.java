...
public class RegionEvent extends Event implements JsonSerializable {
    ...
    public boolean isValid() {
        ...
        if (str == null || this.source == null) {
            UALog.m1744e("The region ID and source must not be null.", new Object[0]);
            return false;
        }
        if (!regionEventCharacterCountIsValid(str)) {
            UALog.m1744e("The region ID must not be greater than %s or less than %s characters in length.", 255, 1);
            return false;
        }
        if (!regionEventCharacterCountIsValid(this.source)) {
            UALog.m1744e("The source must not be greater than %s or less than %s characters in length.", 255, 1);
            return false;
        }
        ...
        UALog.m1744e("The boundary event must either be an entrance (%s) or an exit (%s).", 1, 2);
        return false;
    }
