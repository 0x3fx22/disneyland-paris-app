...
class AssociateIdentifiersEvent extends Event {
    ...
    public boolean isValid() {
        ...
        if (this.ids.size() > 100) {
            UALog.m1744e("Associated identifiers exceeds %s", 100);
            z = false;
        } else {
            z = true;
        }
        for (Map.Entry entry : this.ids.entrySet()) {
            if (((String) entry.getKey()).length() > 255) {
                UALog.m1744e("Associated identifiers key %s exceeds %s characters.", entry.getKey(), 255);
                z = false;
            }
            if (((String) entry.getValue()).length() > 255) {
                UALog.m1744e("Associated identifiers for key %s exceeds %s characters.", entry.getKey(), 255);
                z = false;
            }
        }
        return z;
