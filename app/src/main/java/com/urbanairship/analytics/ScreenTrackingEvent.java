...
class ScreenTrackingEvent extends Event {
    ...
    public boolean isValid() {
        if (this.screen.length() > 255 || this.screen.length() <= 0) {
            UALog.m1744e("Screen identifier string must be between 1 and 255 characters long.", new Object[0]);
            return false;
        }
        if (this.startTime <= this.stopTime) {
            return true;
        }
        UALog.m1744e("Screen tracking duration must be positive or zero.", new Object[0]);
        return false;
    }
