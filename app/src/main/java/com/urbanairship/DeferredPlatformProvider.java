...
public final class DeferredPlatformProvider implements Provider<Integer> {
    ...
    private final int determinePlatform() {
        ...
        if (bestProvider != null) {
            ...
            UALog.m1748i("Setting platform to %s for push provider: %s", strAsString, bestProvider);
            return platform;
        }
        if (PlayServicesUtils.isGooglePlayStoreAvailable(this.context)) {
            UALog.m1748i("Google Play Store available. Setting platform to Android.", new Object[0]);
        } else {
            if (StringsKt.equals("amazon", Build.MANUFACTURER, true)) {
                UALog.m1748i("Build.MANUFACTURER is AMAZON. Setting platform to Amazon.", new Object[0]);
                return 1;
            }
            UALog.m1748i("Defaulting platform to Android.", new Object[0]);
        }
        return 2;
    }
}
