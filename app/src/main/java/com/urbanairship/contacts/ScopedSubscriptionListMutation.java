...
public class ScopedSubscriptionListMutation implements JsonSerializable {
    ...
    public static ScopedSubscriptionListMutation fromJsonValue(@NonNull JsonValue jsonValue) throws JsonException {
        ...
        Scope scopeFromJson = Scope.fromJson(jsonMapOptMap.opt("scope"));
    ...
    public static List<ScopedSubscriptionListMutation> fromJsonList(@NonNull JsonList jsonList) {
        ...
        while (it.hasNext()) {
            try {
                ...
                UALog.m1746e(e, "Invalid subscription list mutation!", new Object[0]);
            }
        }
        return arrayList;
    }
    ...
    public JsonValue getJsonValue() {
        return JsonMap.newBuilder().put("action", this.action).put("list_id", this.listId).put("scope", this.scope).put("timestamp", this.timestamp).build().getJsonValue();
    }
