...
public class NativeBridge {
    ...
    public boolean onHandleCommand(@Nullable String str, @NonNull JavaScriptExecutor javaScriptExecutor, @NonNull ActionRunRequestExtender actionRunRequestExtender, @NonNull CommandDelegate commandDelegate) {
        ...
        UALog.m1751v("Intercepting: %s", str);
        String host = uri.getHost();
        host.hashCode();
        switch (host) {
            case "run-actions":
                UALog.m1748i("Running run actions command for URL: %s", str);
                runActions(actionRunRequestExtender, decodeActionArguments(uri, false));
                return true;
            case "run-action-cb":
                UALog.m1748i("Running run actions command with callback for URL: %s", str);
                List<String> pathSegments = uri.getPathSegments();
                if (pathSegments.size() == 3) {
                    UALog.m1748i("Action: %s, Args: %s, Callback: %s", pathSegments.get(0), pathSegments.get(1), pathSegments.get(2));
                    runAction(actionRunRequestExtender, javaScriptExecutor, pathSegments.get(0), pathSegments.get(1), pathSegments.get(2));
                } else {
                    UALog.m1744e("Unable to run action, invalid number of arguments.", new Object[0]);
                }
                return true;
            case "named_user":
                UALog.m1748i("Running set Named User command for URL: %s", uri);
            ...
            case "run-basic-actions":
                UALog.m1748i("Running run basic actions command for URL: %s", str);
                runActions(actionRunRequestExtender, decodeActionArguments(uri, true));
                return true;
            case "close":
                UALog.m1748i("Running close command for URL: %s", str);
    ...
    public Cancelable loadJavaScriptEnvironment(@NonNull final Context context, @NonNull final JavaScriptEnvironment javaScriptEnvironment, @NonNull final JavaScriptExecutor javaScriptExecutor) {
        UALog.m1748i("Loading Airship Javascript interface.", new Object[0]);
    ...
    private void runAction(ActionRunRequestExtender actionRunRequestExtender, final JavaScriptExecutor javaScriptExecutor, final String str, String str2, final String str3) {
        try {
            ...
            UALog.m1746e(e, "Unable to parse action argument value: %s", str2);
    ...
    private Map decodeActionArguments(Uri uri, boolean z) {
        ...
        for (String str : queryParameters.keySet()) {
            ...
            if (queryParameters.get(str) == null) {
                UALog.m1754w("No arguments to decode for actionName: %s", str);
                return null;
            }
                ...
                for (String str2 : list) {
                    if (z) {
                        try {
                            ...
                            UALog.m1756w(e, "Invalid json. Unable to create action argument " + str + " with args: " + str2, new Object[0]);
                            return null;
                        }
                    } else {
        ...
        UALog.m1754w("Error no action names are present in the actions key set", new Object[0]);
        return null;
    }
