...
public final class FarmHashFingerprint64 {
    ...
    private static void weakHashLength32WithSeeds(byte[] bArr, int i, long j, long j2, long[] jArr) {
        ...
        long jLoad65 = load64(bArr, i + 8);
        long jLoad66 = load64(bArr, i + 16);
        long jLoad67 = load64(bArr, i + 24);
        long j3 = j + jLoad64;
    ...
    private static long hashLength0to16(byte[] bArr, int i, int i2) {
        if (i2 >= 8) {
            ...
            long jLoad65 = load64(bArr, (i + i2) - 8);
            return hashLength16((Long.rotateRight(jLoad65, 37) * j) + jLoad64, (Long.rotateRight(jLoad64, 25) + jLoad65) * j, j);
        }
        if (i2 >= 4) {
            return hashLength16(((long) i2) + ((load32(bArr, i) & BodyPartID.bodyIdMax) << 3), load32(bArr, (i + i2) - 4) & BodyPartID.bodyIdMax, ((long) (i2 * 2)) - 7286425919675154353L);
        }
        if (i2 <= 0) {
    ...
    private static long hashLength17to32(byte[] bArr, int i, int i2) {
        ...
        long jLoad65 = load64(bArr, i + 8);
        int i3 = i + i2;
        long jLoad66 = load64(bArr, i3 - 8) * j;
        return hashLength16((load64(bArr, i3 - 16) * (-7286425919675154353L)) + Long.rotateRight(jLoad64 + jLoad65, 43) + Long.rotateRight(jLoad66, 30), jLoad66 + jLoad64 + Long.rotateRight(jLoad65 - 7286425919675154353L, 18), j);
    }
    ...
    private static long hashLength33To64(byte[] bArr, int i, int i2) {
        ...
        int i3 = i + i2;
        long jLoad66 = load64(bArr, i3 - 8) * j;
        long jRotateRight = Long.rotateRight(jLoad64 + jLoad65, 43) + Long.rotateRight(jLoad66, 30) + (load64(bArr, i3 - 16) * (-7286425919675154353L));
        long jHashLength16 = hashLength16(jRotateRight, jLoad66 + Long.rotateRight(jLoad65 - 7286425919675154353L, 18) + jLoad64, j);
        long jLoad67 = load64(bArr, i + 16) * j;
        long jLoad68 = load64(bArr, i + 24);
        long jLoad69 = (jRotateRight + load64(bArr, i3 - 32)) * j;
        return hashLength16(((jHashLength16 + load64(bArr, i3 - 24)) * j) + Long.rotateRight(jLoad67 + jLoad68, 43) + Long.rotateRight(jLoad69, 30), jLoad67 + Long.rotateRight(jLoad68 + jLoad64, 18) + jLoad69, j);
    }
    ...
    private static long hashLength65Plus(byte[] bArr, int i, int i2) {
        ...
        while (true) {
            ...
            long jLoad65 = jRotateRight2 + jArr[0] + load64(bArr, i8 + 40);
            ...
            weakHashLength32WithSeeds(bArr, i8 + 32, jRotateRight3 + jArr2[1], jLoad65 + load64(bArr, i8 + 16), jArr2);
            int i9 = i8 + 64;
            if (i9 == i4) {
                ...
                jArr[0] = j6;
                jArr2[0] = jArr2[0] + j6;
                long jRotateRight4 = Long.rotateRight(jRotateRight3 + jLoad65 + jArr[0] + load64(bArr, i6 - 55), 37) * j4;
                long jRotateRight5 = Long.rotateRight(jLoad65 + jArr[1] + load64(bArr, i6 - 15), 42) * j4;
                long j7 = jRotateRight4 ^ (jArr2[1] * 9);
                long jLoad66 = jRotateRight5 + (jArr[0] * 9) + load64(bArr, i6 - 23);
                long jRotateRight6 = Long.rotateRight(j3 + jArr2[0], 33) * j4;
                weakHashLength32WithSeeds(bArr, i7, jArr[1] * j4, j7 + jArr2[0], jArr);
                weakHashLength32WithSeeds(bArr, i6 - 31, jRotateRight6 + jArr2[1], jLoad66 + load64(bArr, i6 - 47), jArr2);
                return hashLength16(hashLength16(jArr[0], jArr2[0], j4) + (shiftMix(jLoad66) * (-4348849565147123417L)) + j7, hashLength16(jArr[1], jArr2[1], j4) + jRotateRight6, j4);
            }
            i8 = i9;
            jShiftMix = j3;
            j2 = jLoad65;
