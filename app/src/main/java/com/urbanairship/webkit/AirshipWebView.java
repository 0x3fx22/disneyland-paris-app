...
public class AirshipWebView extends WebView {
    ...
    private void init(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        if (ManifestUtils.shouldEnableLocalStorage()) {
            UALog.m1751v("Application contains metadata to enable local storage", new Object[0]);
            settings.setDomStorageEnabled(true);
    ...
    public void setWebViewClient(@NonNull WebViewClient webViewClient) {
        if (!(webViewClient instanceof AirshipWebViewClient)) {
            UALog.m1754w("The web view client should extend AirshipWebViewClient to support Airship url overrides and triggering actions from.", new Object[0]);
        }
    ...
    protected void onPreLoad(@NonNull final Runnable runnable) {
        if (getWebViewClientCompat() == null) {
            UALog.m1741d("No web view client set, setting a default AirshipWebViewClient for landing page view.", new Object[0]);
        ...
        UALog.m1741d("Unable to start Safe Browsing. Feature is not supported or disabled in the manifest.", new Object[0]);
