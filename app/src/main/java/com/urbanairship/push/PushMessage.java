...
public class PushMessage implements Parcelable, JsonSerializable {
    ...
    boolean isExpired() {
        ...
        if (!UAStringUtil.isEmpty(str)) {
            UALog.m1751v("Notification expiration time is \"%s\"", str);
            try {
                if (Long.parseLong(str) * 1000 < System.currentTimeMillis()) {
                    return true;
                }
            } catch (NumberFormatException e) {
                UALog.m1742d(e, "Ignoring malformed expiration time.", new Object[0]);
            }
        }
        return false;
    }
    ...
    public Map<String, ActionValue> getActions() {
        ...
        try {
            ...
            UALog.m1744e("Unable to parse action payload: %s", str);
            return map;
        }
    }
    ...
    public Uri getSound(@NonNull Context context) {
        if (this.sound == null && this.data.get(EXTRA_SOUND) != null) {
            ...
            if (identifier != 0) {
                ...
                UALog.m1754w("PushMessage - unable to find notification sound with name: %s", str);
            }
        }
        return this.sound;
    }
    ...
    public int getIconColor(int i) {
        ...
        if (str != null) {
            try {
                ...
                UALog.m1754w("Unrecognized icon color string: %s. Using default color: %s", str, Integer.valueOf(i));
            }
        }
        return i;
    }
    ...
    public int getIcon(@NonNull Context context, int i) {
        ...
        if (str != null) {
            ...
            UALog.m1754w("PushMessage - unable to find icon drawable with name: %s. Using default icon: %s", str, Integer.valueOf(i));
        }
        return i;
    }
    ...
    public static PushMessage fromIntent(@Nullable Intent intent) {
        ...
        try {
            ...
            UALog.m1746e(e, "Failed to parse push message from intent.", new Object[0]);
            return null;
        }
    }
