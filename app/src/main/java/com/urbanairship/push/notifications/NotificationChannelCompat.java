...
public class NotificationChannelCompat implements JsonSerializable {
    ...
    public static NotificationChannelCompat fromJson(@NonNull JsonValue jsonValue) {
        ...
        UALog.m1744e("Unable to deserialize notification channel: %s", jsonValue);
        return null;
    }

    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static List<NotificationChannelCompat> fromXml(@NonNull Context context, @XmlRes int i) {
        XmlResourceParser xml = context.getResources().getXml(i);
        try {
            return parseChannels(context, xml);
        } catch (Exception e) {
            UALog.m1746e(e, "Failed to parse channels", new Object[0]);
        ...
        while (1 != xmlResourceParser.next()) {
            if (2 == xmlResourceParser.getEventType() && "NotificationChannel".equals(xmlResourceParser.getName())) {
                ...
                if (UAStringUtil.isEmpty(string) || UAStringUtil.isEmpty(string2) || i == -1) {
                    UALog.m1744e("Invalid notification channel. Missing name (%s), id (%s), or importance (%s)", string, string2, Integer.valueOf(i));
                } else {
