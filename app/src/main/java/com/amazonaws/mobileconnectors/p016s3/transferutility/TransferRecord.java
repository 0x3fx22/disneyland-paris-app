...
class TransferRecord {
    ...
    public void updateFromDB(Cursor cursor) {
        this.f358id = cursor.getInt(cursor.getColumnIndexOrThrow("_id"));
    ...
    public boolean pause(AmazonS3 amazonS3, TransferStatusUpdater transferStatusUpdater) {
        ...
        transferStatusUpdater.updateState(this.f358id, transferState);
        if (isRunning()) {
    ...
    public boolean cancel(final AmazonS3 amazonS3, TransferStatusUpdater transferStatusUpdater) {
        ...
        transferStatusUpdater.updateState(this.f358id, TransferState.PENDING_CANCEL);
        if (isRunning()) {
            this.submittedTask.cancel(true);
        }
        if (TransferType.UPLOAD.equals(this.type) && this.isMultipart == 1) {
            new Thread(new Runnable() { // from class: com.amazonaws.mobileconnectors.s3.transferutility.TransferRecord.1
                ...
                public void run() {
                    try {
                        AmazonS3 amazonS4 = amazonS3;
                        TransferRecord transferRecord = TransferRecord.this;
                        amazonS4.abortMultipartUpload(new AbortMultipartUploadRequest(transferRecord.bucketName, transferRecord.key, transferRecord.multipartId));
                        TransferRecord.LOGGER.debug("Successfully clean up multipart upload: " + TransferRecord.this.f358id);
                    } catch (AmazonClientException e) {
                        TransferRecord.LOGGER.debug("Failed to abort multiplart upload: " + TransferRecord.this.f358id, e);
                    }
                }
            }).start();
    ...
    private boolean checkPreferredNetworkAvailability(TransferStatusUpdater transferStatusUpdater, ConnectivityManager connectivityManager) {
        ...
        transferStatusUpdater.updateState(this.f358id, TransferState.WAITING_FOR_NETWORK);
        return false;
    }
}
