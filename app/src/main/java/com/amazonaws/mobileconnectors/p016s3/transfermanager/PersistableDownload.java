...
public final class PersistableDownload extends PersistableTransfer {
    ...
    @Override // com.amazonaws.mobileconnectors.p016s3.transfermanager.PersistableTransfer
    public String serialize() {
        ...
        try {
            ...
            if (this.responseHeaders != null) {
                jsonWriter.name("responseHeaders").beginObject().name(CMSAttributeTableGenerator.CONTENT_TYPE).value(this.responseHeaders.getContentType()).name("contentLanguage").value(this.responseHeaders.getContentLanguage()).name(PermissionsResponse.EXPIRES_KEY).value(this.responseHeaders.getExpires()).name("cacheControl").value(this.responseHeaders.getCacheControl()).name("contentDisposition").value(this.responseHeaders.getContentDisposition()).name("contentEncoding").value(this.responseHeaders.getContentEncoding()).endObject();
            }
