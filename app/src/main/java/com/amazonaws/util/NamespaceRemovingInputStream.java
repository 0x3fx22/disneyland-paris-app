...
class NamespaceRemovingInputStream extends SdkFilterInputStream {
    ...
    private static final class StringPrefixSlicer {
        ...
        public boolean removePrefix(String str) {
            if (!this.f396s.startsWith(str)) {
                return false;
            }
            this.f396s = this.f396s.substring(str.length());
            return true;
        }
        ...
        public boolean removeRepeatingPrefix(String str) {
            ...
            while (this.f396s.startsWith(str)) {
                this.f396s = this.f396s.substring(str.length());
            }
            return true;
        }
        ...
        public boolean removePrefixEndingWith(String str) {
            int iIndexOf = this.f396s.indexOf(str);
            if (iIndexOf < 0) {
                return false;
            }
            this.f396s = this.f396s.substring(iIndexOf + str.length());
            return true;
        }
    }
}
