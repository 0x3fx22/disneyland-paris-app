...
import com.amazonaws.services.p017s3.internal.PartCreationEvent;
...
import com.amazonaws.services.p017s3.model.UploadObjectRequest;
...
public class UploadObjectObserver {
    ...
    public UploadObjectObserver init(UploadObjectRequest uploadObjectRequest, S3DirectSpi s3DirectSpi, AmazonS3 amazonS3, ExecutorService executorService) {
        ...
        this.f368s3 = amazonS3;
        this.f367es = executorService;
        return this;
    }
    ...
    public String onUploadInitiation(UploadObjectRequest uploadObjectRequest) {
        String uploadId = this.f368s3.initiateMultipartUpload(newInitiateMultipartUploadRequest(uploadObjectRequest)).getUploadId();
    ...
    public void onPartCreate(PartCreationEvent partCreationEvent) {
        ...
        this.futures.add(this.f367es.submit(new Callable<UploadPartResult>() { // from class: com.amazonaws.services.s3.UploadObjectObserver.1
    ...
    public CompleteMultipartUploadResult onCompletion(List<PartETag> list) {
        return this.f368s3.completeMultipartUpload(new CompleteMultipartUploadRequest(this.req.getBucketName(), this.req.getKey(), this.uploadId, list));
    }
    ...
    public void onAbort() {
        ...
        if (this.uploadId != null) {
            try {
                this.f368s3.abortMultipartUpload(new AbortMultipartUploadRequest(this.req.getBucketName(), this.req.getKey(), this.uploadId));
            } catch (Exception e) {
