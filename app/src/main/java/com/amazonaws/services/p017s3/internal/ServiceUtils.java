...
import com.amazonaws.services.p017s3.S3ClientOptions;
...
import com.amazonaws.services.p017s3.model.SSEAlgorithm;
...
public class ServiceUtils {
    ...
    public static void downloadObjectToFile(S3Object s3Object, File file, boolean z, boolean z2) throws Throwable {
        byte[] bArrComputeMD5Hash;
        byte[] bArr;
        ...
        byte[] bArrFromHex = null;
        byte[] bArr2 = null;
        try {
            BufferedOutputStream bufferedOutputStream2 = new BufferedOutputStream(new FileOutputStream(file, z2));
            try {
                byte[] bArr3 = new byte[Data.MAX_DATA_BYTES];
                while (true) {
                    int i = s3Object.getObjectContent().read(bArr3);
                    if (i <= -1) {
                        try {
                            break;
                ...
                try {
                    if (isMultipartUploadETag(s3Object.getObjectMetadata().getETag())) {
                        ...
                        bArrComputeMD5Hash = Md5Utils.computeMD5Hash(new FileInputStream(file));
                        try {
                            bArrFromHex = BinaryUtils.fromHex(s3Object.getObjectMetadata().getETag());
                        } catch (Exception e) {
                            e = e;
                            log.warn("Unable to calculate MD5 hash to validate download: " + e.getMessage(), e);
                        }
                        bArr = bArrFromHex;
                        bArr2 = bArrComputeMD5Hash;
                    }
                } catch (Exception e2) {
                    e = e2;
                    bArrComputeMD5Hash = null;
                }
                if (!z || bArr2 == null || bArr == null || Arrays.equals(bArr2, bArr)) {
                    return;
                }
