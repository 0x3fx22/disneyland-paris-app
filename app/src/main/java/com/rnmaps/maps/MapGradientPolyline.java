...
public class MapGradientPolyline extends MapFeature {
    ...
    public class AirMapGradientPolylineProvider implements TileProvider {
        ...
        public void renderTrail(Canvas canvas, Matrix matrix, Paint paint, Paint paint2, float f, int i, int i2) {
            ...
            if (this.points.size() == 1) {
                ...
                canvas2.drawCircle((float) mutPoint.f3679x, (float) mutPoint.f3680y, paint2.getStrokeWidth() / 2.0f, paint2);
            ...
            while (i3 < this.points.size()) {
                ...
                canvas2.drawCircle((float) mutPoint2.f3679x, (float) mutPoint2.f3680y, paint2.getStrokeWidth() / 2.0f, paint2);
                paint2.setStyle(Paint.Style.STROKE);
        ...
        public void drawLine(Canvas canvas, Matrix matrix, Paint paint, Paint paint2, MutPoint mutPoint, MutPoint mutPoint2, float f, float f2) {
            ...
            matrix.reset();
            matrix.preRotate((float) Math.toDegrees(Math.atan2(mutPoint2.f3680y - mutPoint.f3680y, mutPoint2.f3679x - mutPoint.f3679x)), (float) mutPoint.f3679x, (float) mutPoint.f3680y);
            matrix.preTranslate((float) mutPoint.f3679x, (float) mutPoint.f3680y);
            float fSqrt = (float) Math.sqrt(Math.pow(mutPoint2.f3679x - mutPoint.f3679x, 2.0d) + Math.pow(mutPoint2.f3680y - mutPoint.f3680y, 2.0d));
            matrix.preScale(fSqrt, fSqrt);
            float f3 = 1.0f / (f2 - f);
            matrix.preScale(f3, f3);
            matrix.preTranslate(-f, BitmapDescriptorFactory.HUE_RED);
            paint.getShader().setLocalMatrix(matrix);
            canvas.drawLine((float) mutPoint.f3679x, (float) mutPoint.f3680y, (float) mutPoint2.f3679x, (float) mutPoint2.f3680y, paint);
        }
    ...
    public static class MutPoint {
        ...
        public double f3679x;

        /* JADX INFO: renamed from: y */
        public double f3680y;

        public MutPoint set(Point point, float f, int i, int i2, int i3) {
            double d = f;
            this.f3679x = (point.f3580x * d) - ((double) (i * i3));
            this.f3680y = (point.f3581y * d) - ((double) (i2 * i3));
            return this;
        }
    }
}
