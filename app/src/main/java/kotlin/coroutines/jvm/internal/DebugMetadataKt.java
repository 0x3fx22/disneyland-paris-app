...
public final class DebugMetadataKt {
    ...
    public static final StackTraceElement getStackTraceElement(@NotNull BaseContinuationImpl baseContinuationImpl) {
        ...
        checkDebugMetadataVersion(1, debugMetadataAnnotation.m1851v());
        int label = getLabel(baseContinuationImpl);
        int i = label < 0 ? -1 : debugMetadataAnnotation.m1847l()[label];
        String moduleName = ModuleNameRetriever.INSTANCE.getModuleName(baseContinuationImpl);
        if (moduleName == null) {
            strM1844c = debugMetadataAnnotation.m1844c();
        } else {
            strM1844c = moduleName + '/' + debugMetadataAnnotation.m1844c();
        }
        return new StackTraceElement(strM1844c, debugMetadataAnnotation.m1848m(), debugMetadataAnnotation.m1845f(), i);
    }
    ...
    public static final String[] getSpilledVariableFieldMapping(@NotNull BaseContinuationImpl baseContinuationImpl) {
        ...
        int label = getLabel(baseContinuationImpl);
        int[] iArrM1846i = debugMetadataAnnotation.m1846i();
        int length = iArrM1846i.length;
        for (int i = 0; i < length; i++) {
            if (iArrM1846i[i] == label) {
                arrayList.add(debugMetadataAnnotation.m1850s()[i]);
                arrayList.add(debugMetadataAnnotation.m1849n()[i]);
            }
        }
