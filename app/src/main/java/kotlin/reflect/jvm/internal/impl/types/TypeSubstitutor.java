...
public class TypeSubstitutor {
    ...
    private TypeProjection unsafeSubstitute(TypeProjection typeProjection, TypeParameterDescriptor typeParameterDescriptor, int i) throws SubstitutionException {
        ...
        if (!DynamicTypesKt.isDynamic(type) && !(type.unwrap() instanceof RawType)) {
            TypeProjection typeProjectionMo5744get = this.substitution.mo5744get(type);
            TypeProjection typeProjectionProjectedTypeForConflictedTypeWithUnsafeVariance = typeProjectionMo5744get != null ? projectedTypeForConflictedTypeWithUnsafeVariance(type, typeProjectionMo5744get, typeParameterDescriptor, typeProjection) : null;
            ...
            if (!KotlinBuiltIns.isNothing(type) && !KotlinTypeKt.isError(type)) {
                if (typeProjectionProjectedTypeForConflictedTypeWithUnsafeVariance != null) {
                    ...
                    if (!CapturedTypeConstructorKt.isCaptured(type)) {
                        int i3 = C71832.f3949x4790888d[varianceConflictTypeConflictType.ordinal()];
                        if (i3 == 1) {
    ...
    static /* synthetic */ class C71832 {

        /* JADX INFO: renamed from: $SwitchMap$org$jetbrains$kotlin$types$TypeSubstitutor$VarianceConflictType */
        static final /* synthetic */ int[] f3949x4790888d;
        ...
        static {
            ...
            try {
                f3949x4790888d[VarianceConflictType.IN_IN_OUT_POSITION.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                f3949x4790888d[VarianceConflictType.NO_CONFLICT.ordinal()] = 3;
    ...
    private TypeProjection substituteCompoundType(TypeProjection typeProjection, int i) {
        KotlinType type = typeProjection.getType();
        Variance projectionKind = typeProjection.getProjectionKind();
        if (type.getConstructor().mo5739getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return typeProjection;
        }
    ...
    private List substituteTypeArguments(List list, List list2, int i) throws SubstitutionException {
        ...
        for (int i2 = 0; i2 < list.size(); i2++) {
            ...
            int i3 = C71832.f3949x4790888d[conflictType(typeParameterDescriptor.getVariance(), typeProjectionUnsafeSubstitute.getProjectionKind()).ordinal()];
            if (i3 == 1 || i3 == 2) {
