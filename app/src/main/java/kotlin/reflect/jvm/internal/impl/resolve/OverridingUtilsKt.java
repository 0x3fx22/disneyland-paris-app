...
public final class OverridingUtilsKt {
    ...
    public static final <H> Collection<H> selectMostSpecificInEachOverridableGroup(@NotNull Collection<? extends H> collection, @NotNull Function1<? super H, ? extends CallableDescriptor> descriptorByHandle) {
        ...
        while (!linkedList.isEmpty()) {
            ...
            Collection<WrappingViewGroup> collectionExtractMembersOverridableInBothWays = OverridingUtil.extractMembersOverridableInBothWays(objFirst, linkedList, descriptorByHandle, new Function1(smartSetCreate2) { // from class: kotlin.reflect.jvm.internal.impl.resolve.OverridingUtilsKt$$Lambda$1
            ...
            if (collectionExtractMembersOverridableInBothWays.size() == 1 && smartSetCreate2.isEmpty()) {
                ...
                for (WrappingViewGroup wrappingViewGroup2 : collectionExtractMembersOverridableInBothWays) {
                    Intrinsics.checkNotNull(wrappingViewGroup2);
                    if (!OverridingUtil.isMoreSpecific(callableDescriptorInvoke, descriptorByHandle.invoke(wrappingViewGroup2))) {
                        smartSetCreate2.add(wrappingViewGroup2);
                    }
                }
