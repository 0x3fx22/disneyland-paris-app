        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (supertypesPolicy2 == null) {
                    ...
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo5743transformType = supertypesPolicy2.mo5743transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(simpleTypeMarkerMo5743transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo5743transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo5743transformType)) {
                            typeCheckerState.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo5743transformType);
                    }
                }
            }
        }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(simpleTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo5743transformType = supertypesPolicy.mo5743transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo5743transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo5743transformType);
