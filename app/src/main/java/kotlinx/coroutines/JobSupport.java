...
public class JobSupport implements Job, ChildJob, ParentJob {
    ...
    public final void removeNode$kotlinx_coroutines_core(@NotNull JobNode node) {
        ...
        } while (!AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state$kotlinx_coroutines_core, JobSupportKt.EMPTY_ACTIVE));
    }
    ...
    private final Object finalizeFinishingState(Finishing state, Object proposedUpdate) throws Throwable {
        ...
        AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, JobSupportKt.boxIncomplete(proposedUpdate));
    ...
    private final boolean tryFinalizeSimpleState(Incomplete state, Object update) throws Throwable {
        if (!AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, JobSupportKt.boxIncomplete(update))) {
            return false;
        }
        onCancelling(null);
    ...
    private final int startInternal(Object state) {
        if (state instanceof Empty) {
            ...
            if (!AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, JobSupportKt.EMPTY_ACTIVE)) {
                return -1;
            }
            onStart();
            return 1;
        ...
        if (!AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, ((InactiveNodeList) state).getList())) {
            return -1;
        }
        onStart();
        return 1;
    ...
    public final DisposableHandle invokeOnCompletionInternal$kotlinx_coroutines_core(boolean invokeImmediately, @NotNull JobNode node) {
        ...
        while (true) {
            ...
            if (state$kotlinx_coroutines_core instanceof Empty) {
                ...
                if (empty.isActive()) {
                    if (AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state$kotlinx_coroutines_core, node)) {
                        break;
                    }
                } else {
    ...
    private final void promoteEmptyToNodeList(Empty state) {
        ...
        AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, inactiveNodeList);
    }

    private final void promoteSingleToNodeList(JobNode state) {
        state.addOneIfEmpty(new NodeList());
        AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, state.getNextNode());
    }
    ...
    private final boolean tryMakeCancelling(Incomplete state, Throwable rootCause) throws Throwable {
        ...
        if (!AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, new Finishing(orPromoteCancellingList, false, rootCause))) {
            return false;
        }
    ...
    private final Object tryMakeCompletingSlowPath(Incomplete state, Object proposedUpdate) throws Throwable {
        ...
        synchronized (finishing) {
            ...
            if (finishing != state && !AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state, finishing)) {
                return JobSupportKt.COMPLETING_RETRY;
            }
    ...
    public final ChildHandle attachChild(@NotNull ChildJob child) {
        ...
        while (true) {
            ...
            if (state$kotlinx_coroutines_core instanceof Empty) {
                Empty empty = (Empty) state$kotlinx_coroutines_core;
                if (empty.isActive()) {
                    if (AbstractC0365xc40028dd.m91m(_state$volatile$FU, this, state$kotlinx_coroutines_core, childHandleNode)) {
                        break;
                    }
                } else {
                    promoteEmptyToNodeList(empty);
    ...
    private final String stateString(Object state) {
        ...
        if (state instanceof Incomplete) {
            return ((Incomplete) state).isActive() ? "Active" : "New";
        }
