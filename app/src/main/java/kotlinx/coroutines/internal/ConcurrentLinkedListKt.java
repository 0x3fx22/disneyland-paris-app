...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(@NotNull S s, long j, @NotNull Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m5993constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m5993constructorimpl(CLOSED);
            }
    ...
    public static final /* synthetic */ <S extends Segment<S>> boolean moveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, S s) {
        while (true) {
            ...
            if (AbstractC0365xc40028dd.m91m(atomicReferenceFieldUpdater, obj, segment, s)) {
    ...
    public static final /* synthetic */ <S extends Segment<S>> boolean moveForward$atomicfu$array(AtomicReferenceArray atomicReferenceArray, int i, S s) {
        while (true) {
            ...
            if (AbstractC4178xa7a47114.m1672m(atomicReferenceArray, i, segment, s)) {
                if (segment.decPointers$kotlinx_coroutines_core()) {
                    segment.remove();
                }
                return true;
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m5998isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (AbstractC0365xc40028dd.m91m(atomicReferenceFieldUpdater, obj, segment, segmentM5996getSegmentimpl)) {
                        if (!segment.decPointers$kotlinx_coroutines_core()) {
                            break loop0;
                        }
                        segment.remove();
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m5998isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (AbstractC4178xa7a47114.m1672m(atomicReferenceArray, i, segment, segmentM5996getSegmentimpl)) {
                        if (!segment.decPointers$kotlinx_coroutines_core()) {
                            break loop0;
                        }
                        segment.remove();
